...
for slug in "${CHAINS[@]}"; do
  ...
  # the alert config is actually live".
  {
    ...
    # Re-checked HERE, inside the loop and immediately before this stamp
    # (Codex #1495 r9 P2). The stamp is WRITTEN during generation, so there is
    # no "after generation" moment at which to check it once — r8 claimed to
    # have moved this past the loop and had not; it still ran before the loop
    # began, which is the one point at which nothing can yet have changed.
    # Per-iteration is the only placement that makes each manifest's marker
    # true of the tree that produced THAT manifest.
