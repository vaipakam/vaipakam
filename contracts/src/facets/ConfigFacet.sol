...
contract ConfigFacet is DiamondAccessControl {
    ...
    // ──────────────────────────────────────────────────────────────
    //  T-090 v1.1 (#389) — intent-based swap-to-repay configuration
    //  See `docs/DesignsAndPlans/SwapToRepayIntentBased.md` §5.6 for
    //  the full surface + rationale.
    //
    //  Storage layout note (Codex round-7 P1 #5): every v1.1 cfg
    //  slot lives on the TOP-LEVEL `LibVaipakam.Storage` struct, NOT
    //  the nested `ProtocolConfig`. Growing `ProtocolConfig` would
    //  shift every subsequent slot on upgrade and corrupt deployed
    //  `borrowerLifRebate` / swap-adapter state.
    // ──────────────────────────────────────────────────────────────
    ...
    /// @dev    ADMIN_ROLE pre-handover; governance-timelock
    ///         post-handover (standard Vaipakam admin path).
    ...
    /// @notice §5.1 step 4 pre-commit lender-protection gate (HF_SCALE-
    ///         scaled; default 1.2e18 = 120%). Blocks already-stressed
    ///         borrowers from using intent as a liquidation-stall
    ///         tactic.
    /// @dev    Codex round-10 P1 #1: HF_SCALE-scaled uint256, NOT
    ///         BPS-scaled.
    /// @param  hfScaleValue HF threshold in HF_SCALE units (1e18 = 1.0).
    ...
    ///         the borrower's takerAmount must clear. Default 200 bps
    ///         = 2%. Capped at `MAX_SLIPPAGE_BPS` (the existing 2500
    ///         BPS ceiling shared with v1 atomic surface).
    ...
    /// @notice §5.1 step 2 auction-window bounds. Defaults: min 60s,
    ///         max 600s (Fusion's typical 5-min window).
    ...
    ///         address. Audited mainnet address is
    ///         `0x111111125421ca6dc452d289314280a0f8842a65` on every
    ///         chain Vaipakam targets at v1.1 launch (Base + Arbitrum +
    ///         their Sepolia testnets); zkSync Era differs (out of
    ///         scope for v1.1).
    /// @dev    Codex round-10 P1 #6 — rotation REFUSED while any
    ///         commit is live. Defense-in-depth alongside the
    ///         per-commit `lopAtCommit` pin so an admin bug or
    ///         mistaken rotation can't orphan a live commit's cancel
    ///         path on the new (and wrong) LOP address.
    ...
    ///         allowlist. Default-OFF on every token; admin adds
    ///         tokens after operator-side transfer round-trip probes
    ///         confirm non-fee-on-transfer + non-rebasing behaviour.
    /// @dev    Replaces the upstream "Liquid leg" check for v1.1; the
    ///         Liquid check verifies oracle freshness + AMM floor
    ///         depth but does NOT probe transfer symmetry, so it
    ///         can't filter fee-on-transfer principals on its own.
    ...
    ///         allowlist. Default-OFF; admin adds tokens after operator
    ///         verification (same probe rationale as the principal-side
    ///         setter).
