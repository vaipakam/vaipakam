...
library LibInteractionRewards {
    ...
    /// @notice #1434 P1-a — install `governorCommitArmedFromDay` AND snapshot
    ///         the fresh paid-out cumulative in one place.
    /// @dev    THE single chokepoint for arming installation. Three sites
    ///         write `governorCommitArmedFromDay`: the canonical one-shot
    ///         setter on Base ({RewardAggregatorFacet}) and the TWO
    ///         mirror-side broadcast-arrival paths in {RewardReporterFacet}.
    ///         Each must also snapshot `interactionPoolPaidOut`, and three
    ///         independent copies of a two-field write is exactly the drift
    ///         class this codebase keeps paying for — so it is one function
    ///         and the call sites do not get to remember the second field.
    ///
    ///         Why a snapshot at all: the delivered-fresh bound measures
    ///         payouts made AGAINST delivered funding. `interactionPoolPaidOut`
    ///         is a lifetime cumulative that also includes UNARMED-day fresh
    ///         payouts drawn from the global schedule, which no remit funded.
    ///         Subtracting the value at arming isolates the post-arming
    ///         payouts, which are the only ones delivered funding owes.
    ///
    ///         Idempotent by the caller's own guard: every site already tests
    ///         `governorCommitArmedFromDay == 0` (the canonical setter reverts
    ///         {GovernorAlreadyArmed}; the mirror paths skip a replay), so
    ///         this is never reached twice and the snapshot cannot be
    ///         re-taken at a later, larger cumulative. Asserted here anyway
    ///         rather than trusted — a future fourth call site that forgets
    ///         the guard would otherwise silently move the baseline forward
    ///         and hand the chain free delivered headroom.
    /// @param  s      Diamond storage.
    /// @param  dayId  The `D*` cutover day being installed.
    function installArmedFromDay(
        LibVaipakam.Storage storage s,
        uint256 dayId
    ) internal {
        if (s.governorCommitArmedFromDay != 0) return;
        s.governorCommitArmedFromDay = dayId;
        s.interactionPaidOutFreshAtArming = s.interactionPoolPaidOut;
    }

    /// @notice #1434 P1-a — VPFI this chain may still pay as FRESH reward out
    ///         of funding actually DELIVERED to it, floored at zero.
    /// @dev    `receivedFresh − (paidOutFresh − paidOutFreshAtArming)`.
    ///
    ///         On the CANONICAL chain this is not the operative bound — Base
    ///         is the funding source, receives no remits, and is bounded by
    ///         the 69M cap less what it has paid and remitted
    ///         ({poolRemaining}). It reads zero there and callers must not
    ///         apply it on Base.
    ///
    ///         READ BY NOTHING YET (P1-a is accounting only). P1-b applies it
    ///         in the ShareOfPool walk, where a shortfall must **DEFER**, not
    ///         truncate. That asymmetry is the whole subtlety: the existing
    ///         fresh path truncates because the 69M headroom is monotone
    ///         NON-INCREASING, so a trimmed remainder is unfundable forever.
    ///         A mirror's delivered budget GROWS with every remit, so the
    ///         same trim would permanently underpay a day whose funding is
    ///         merely still in flight. It needs the RECYCLED side's
    ///         semantics — all-or-nothing, retryable — not the fresh side's.
    /// @param  s Diamond storage.
    /// @return Delivered fresh still available to pay on this chain.
    function deliveredFreshAvailable(
        LibVaipakam.Storage storage s
    ) internal view returns (uint256) {
        uint256 spentSinceArming = s.interactionPoolPaidOut
            > s.interactionPaidOutFreshAtArming
            ? s.interactionPoolPaidOut - s.interactionPaidOutFreshAtArming
            : 0;
        uint256 received = s.rewardBudgetReceivedFresh;
        return received > spentSinceArming ? received - spentSinceArming : 0;
    }
