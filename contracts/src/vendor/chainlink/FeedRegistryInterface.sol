...
// VENDORED — Chainlink's `FeedRegistryInterface`. T-068: the modern
// `chainlink-evm` contracts tree (consumed via `chainlink-local`) no longer
// carries this mainnet Feed-Registry interface, so this single frozen
// interface file is vendored locally. Only change vs upstream: the import
// below is rewritten to the absolute `@chainlink/contracts/...` path (the
// `AggregatorV2V3Interface` it needs IS in `chainlink-evm`).
