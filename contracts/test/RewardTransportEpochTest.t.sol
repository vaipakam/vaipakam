...
contract RewardTransportEpochTest is SetupTest, IVaipakamErrors {
    ...
    function _untyped(uint256 amount, uint256 dayCount, uint256 remitId, bytes32 id)
        ...
        returns (bytes32 h)
    {
        return _deliver(amount, _days(dayCount), remitId, id, false);
    }
    ...
    function test_Untyped_OpensItsEpochBoundedByWhatLanded() public {
        bytes32 h = _untyped(10e18, 3, 1, keccak256("u1"));
        ...
        assertEq(indexedDays, 3, "within the cap: indexed whole at admission");
        assertFalse(oversize, "within the cap");
    ...
    function test_OverCap_IsAdmittedCompactly_ThenIndexedInPages() public {
        ...
        (, uint256 balance, , uint32 dayCount, uint32 indexedDays, bool oversize, ) =
            _epoch().getTransportBatch(h);
        ...
        assertTrue(oversize, "it took the compact admission");
    ...
    /// A within-cap batch was indexed whole at admission, so it has no page.
    function test_Page_RefusesAWithinCapBatch() public {
    ...
    /// An id no delivery opened is refused by name, not treated as an empty
    /// batch — a d5 packet's hash reaches here exactly this way.
    function test_Page_RefusesAnUnknownBatch() public {
        bytes32 typedPacket = _deliver(5e18, _days(2), 6, keccak256("t2"), true);
        vm.expectRevert(abi.encodeWithSelector(TransportBatchUnknown.selector, typedPacket));
        _epoch().materializeTransportBatchPage(typedPacket, _days(2));
    }

    /// A delivery with nothing untyped in it opens NO epoch. §5c refuses a
    /// batch with no balance — its membership could reserve nothing and its
    /// retirement retire nothing — and here it would also hold a valueless
    /// packet shut behind a release that releases nothing.
    function test_NothingUntyped_OpensNoEpoch() public {
        uint256[] memory dayIds = _days(2);
        // Untyped wire, zero delivery: the wire states nothing and nothing
        // lands, so there is no untyped remainder to hold.
        _ingress().onRewardBudgetReceived(
            address(vpfi), 0, dayIds, CHAIN_BASE, 30, REMITTER, 0, 0, keccak256("z1"), false
        );
        bytes32 h = keccak256(abi.encode(uint256(CHAIN_BASE), keccak256("z1")));
        (bytes32 packetHash, , , , , , ) = _epoch().getTransportBatch(h);
        assertEq(packetHash, bytes32(0), "no batch was opened");
        (, uint256 total, ) = _epoch().getTransportDayBatches(1, 0, 4);
        assertEq(total, 0, "and no day indexes one");
    }
    ...
    function test_DayIndex_Paginates() public {
        for (uint256 i; i < 5; ++i) {
            _deliver(1e18, _days(1), 20 + i, keccak256(abi.encode("d", i)), false);
        }
