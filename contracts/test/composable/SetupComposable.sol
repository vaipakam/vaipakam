...
/// @title SetupComposable — standalone deployable helper that mirrors
///         the legacy `SetupTest.setupHelper()` bootstrap as composition
///         instead of inheritance.
/// @notice Tests inherit only `forge-std/Test` and instantiate this contract
///         in their `setUp()`:
///
...
///                   helpers.diamond();      // address of routed diamond
///                   helpers.legalFacet();   // facet deploy address
///                   // ... etc.
///               }
///           }
///
///         The migration target (Stage 6 of the test-suite-audit). With
///         all 50 inheriting tests moved to this pattern, the legacy
///         `SetupTest.t.sol` + `HelperTest.sol` can be deleted.
///
/// @dev    Behavioural parity: the bootstrap body is the same code path
///         the legacy `setupHelper()` runs. The only adjustments are
///         (a) `is Test` removed — uses `Vm` directly via the well-known
///         HEVM cheatcode address; (b) every admin / governance call is
///         pre-pranked as the owner (= the test contract), because
///         `msg.sender` inside `bootstrap` defaults to the composable
///         contract itself; (c) all state fields are `public` so tests
///         can read them via Solidity's auto-generated getters; (d)
///         `makeAddr` is reimplemented internally (forge-std's lives on
///         `Test`).
contract SetupComposable {
    ...
    // Public so inheriting tests can access via auto-generated getters
    // (Stage 6 composition pattern — `helpers.diamond()`,
    // `helpers.owner()`, etc.).
    ...
    /// @notice Idempotent bootstrap. Call once per test contract — typically
    ///         in `setUp()` — passing the test contract's address as
    ///         `_owner`. The diamond is deployed with `_owner` as its
    ///         initial owner; all admin / governance calls run pre-pranked
    ///         as `_owner` so role-gated paths complete inside this body.
    /// @param _owner The test contract's address. After bootstrap, both
    ///         the diamond and the AccessControl role tree treat `_owner`
    ///         as the privileged caller — tests don't need to prank for
    ///         admin calls they own.
    function bootstrap(address _owner) public {
        owner = _owner;
        lender = _makeAddr("lender");
        ...
        // Composition adaptation: in the legacy inheritance-based
        // `SetupTest.setupHelper()`, msg.sender naturally equalled
        // `owner = address(this)`. As a standalone composable,
        // msg.sender inside `bootstrap` is the SetupComposable
        // contract — not the owner — so every admin-gated call below
        // is pre-pranked as `owner` (the test contract).
        vm.prank(owner);
