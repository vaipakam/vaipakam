...
contract SelectorCoverageTest is Test, DeployDiamond, DiamondFacetNames {
    ...
    function test_DeployDiamond_RoutesEveryFacetSelector() public {
        ...
        string[87] memory facets = cutFacetNames();
        uint256 missing;
    ...
    function _populateRoutedSet() private {
        ...
        _addAll(_getOfferParallelSaleSelectors());
        _addAll(_getOfferAcceptSelectors());
        // #1835 — borrower-LIF charge split out of OfferAcceptFacet.
        _addAll(_getOfferAcceptFeeSelectors());
        _addAll(_getOfferPreviewSelectors());
        _addAll(_getLoanSelectors());
        _addAll(_getRepaySelectors());
        // Issue #66 — periodic-interest + NFT-rental daily-deduction
        // cluster split out of RepayFacet.
        _addAll(_getRepayPeriodicFacetSelectors());
        ...
        _addAll(_getSwapToRepayIntentFacetSelectors());
        // T-090 v1.1 (#389) — intent-based swap-to-repay config knobs.
        _addAll(_getIntentConfigSelectors());
        ...
        _addAll(_getEarlyWithdrawalDirectSelectors());
        ...
        // T-087 Sub 2.C — mirror-side Diamond ingress for the cross-
        // chain tier push. Owns the `userTierCache` writer.
        ...
        // #396 v0.5 — gasless signed off-chain offer book.
        _addAll(_getSignedOfferFacetSelectors());
        ...
        _addAll(_getRiskAccessFacetSelectors());
        ...
        _addAll(_getRewardClaimFacetSelectors());
        _addAll(_getRewardHorizonSweepSelectors());
        _addAll(_getPerkSelectors());
        _addAll(_getRewardBroadcastSelectors());
        ...
        _addAll(_getRewardRemittanceLensSelectors());
        _addAll(_getRewardCompensationDispatchSelectors());
        _addAll(_getRewardCommitmentSelectors());
        _addAll(_getRewardCustodySelectors());
        _addAll(_getRewardReconciliationSelectors());
        _addAll(_getRewardIngressSelectors());
        _addAll(_getRewardEpochSelectors());
        _addAll(_getRewardEpochViewSelectors());
        _addAll(_getRewardStagingSelectors());
        _addAll(_getRewardClaimWalkSelectors());
        _addAll(_getRewardSweepWalkSelectors());
        _addAll(_getRewardForfeitWalkSelectors());
        _addAll(_getRewardStagingSettleSelectors());
        _addAll(_getRepatriationSelectors());
        _addAll(_getConfigSelectors());
        _addAll(_getNumeraireConfigSelectors());
        ...
        _addAll(_getRiskSplitLiquidationSelectors());
        ...
        // the FIXED-PRICE Seaport prepay listing flow).
        _addAll(_getNFTPrepayListingSelectors());
        // T-086 Round-5 Block B (#309) — `NFTPrepayDutchListingFacet`
        // (Dutch-decay sibling facet).
        _addAll(_getNFTPrepayDutchListingSelectors());
        // T-086 Round-6 / Block D (#345) — `NFTPrepayListingAtomicFacet`
        // (the new `matchOpenSeaOffer` entry point that kills the
        // v1 English-mode race window via Seaport
        // `matchAdvancedOrders`).
        _addAll(_getNFTPrepayListingAtomicSelectors());
        // T-086 Round-7 (#355) — `NFTPrepayAutoListFacet` (the new
        // permissionless `autoListAtFloorOnGrace` entry point that
        // posts / rotates listings down to the protocol floor while
        // a loan is in its grace window).
