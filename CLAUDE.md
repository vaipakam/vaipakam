...
Same shape as the frontend ABI sync above, but for the Cloudflare
hf-watcher worker. The watcher used to carry hand-typed
`as const` ABI tuples in `ops/hf-watcher/src/diamondAbi.ts`. When
the contract's `Offer` struct gained `periodicInterestCadence`
(T-034 — Periodic Interest Payment), the manual tuple wasn't
updated; viem's positional decoder then read `lendingAsset` from
where the cadence enum actually lives, cascading every subsequent
field by one slot. Symptom: OfferBook rendered garbage values
(5×10²⁹ ETH amounts, 10⁷% rates, 5×10¹⁸ days durations) for
offers whose Dashboard view was correct, because Dashboard reads
via the auto-synced frontend ABI bundle while the OfferBook
indexer pipeline went through the watcher's stale tuple.
[ReleaseNotes-2026-05-05.md](docs/ReleaseNotes/ReleaseNotes-2026-05-05.md)
"Watcher offer-decode drift" has the full incident.

The watcher now imports per-facet JSON files from
`ops/hf-watcher/src/abis/`, generated by `forge inspect <Facet>
abi --json` via:
...
`deploy-chain.sh` and `deploy-mainnet.sh` invoke this script
automatically alongside the frontend + keeper-bot exports — no
manual run needed on a normal deploy. The Solidity compiler is
the single source of truth for the watcher's read-decode shape;
hand-typed positional ABIs are gone and can't recur.

After re-export, redeploy the worker so the live Cloudflare
runtime picks up the corrected ABI:

```bash
cd ops/hf-watcher && npx wrangler deploy
```

The watcher's `refreshStaleOfferDetails` cron loop re-reads every
active offer's details on the next tick (≤ 5 minutes), so existing
D1 rows decoded against the old ABI heal automatically — no
backfill script needed.

**Adding a facet to the watcher's read surface**: append the
contract name to the `FACETS=(...)` array in
`contracts/script/exportWatcherAbis.sh` AND import the resulting
JSON in `ops/hf-watcher/src/diamondAbi.ts`. Today the array is
intentionally minimal (`OfferCancelFacet`, `LoanFacet`) — every
entry is one more place to drift if a facet is renamed; only add
when the watcher gains a new return-decoding code path. Events
parsed via viem's `parseAbi` in `chainIndexer.ts` stay inline
because event routing uses topic-hash matching, not positional
decode.
