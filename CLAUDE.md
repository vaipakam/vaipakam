...
`VaipakamDiamond.sol` is the single entry point. All calls hit its `fallback()`, which routes to the correct facet by function selector. All facets share storage through `LibVaipakam.sol` at the ERC-7201 position `VANGKI_STORAGE_POSITION` = `keccak256(abi.encode(uint256(keccak256("vaipakam.storage")) - 1)) & ~bytes32(uint256(0xff))` — NOT the plain `keccak256("vaipakam.storage")` this note used to state (#1566 §7; `StorageSlotPinTest` pins the derivation).

**The `Storage` struct is NOT append-only in history, and an in-place facet refresh across a layout change mis-maps live state** — see #2092 and `packages/contracts/scripts/storage-layout-provenance.mjs`. Never insert, remove or retype a field in `LibVaipakam.Storage` (or in a struct it inlines) without a `--fresh` redeploy plan for every live chain; append only. The custody census reads storage at every layout era (`contracts/deployments/storage-slot-eras.json`, regenerated by `storage-layout-eras.mjs`), and `StorageSlotPinTest` fails when `deployments/storage-slots.json` no longer matches the compiler — regenerate it with `forge script script/StorageSlotProbe.s.sol`.
