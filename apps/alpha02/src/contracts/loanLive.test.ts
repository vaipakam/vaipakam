...
describe('refinancePayoffOf', () => {
  const base =
    live.principal +
    (live.principal * live.interestRateBps * live.durationDays) /
      (365n * 10_000n);

  it('is principal + full-term remaining interest within term', () => {
    expect(refinancePayoffOf(live, live.startTime + DAY)).toBe(base);
  });

  it('adds the grace-window late fee past maturity', () => {
    expect(refinancePayoffOf(live, endTime + DAY + 1n)).toBe(
      base + (live.principal * 150n) / 10_000n,
    );
  });
});
...
describe('refinanceApprovalOf', () => {
  ...
  const payoffNoFee = refinancePayoffOf(live, live.startTime);
  ...
  it('covers the fee at the grace end when the request outlives it', () => {
    // Last fillable moment = endTime + 3d → 1% + 3×0.5% = 2.5%.
    expect(
      refinanceApprovalOf(live, {
        expiresAt: endTime + 30n * DAY,
        graceSeconds,
      }),
    ).toBe(payoffNoFee + (live.principal * 250n) / 10_000n);
  });
  ...
  it('covers the fee at expiry - 1 when the offer clock binds first', () => {
    ...
    // expiresAt - 1 = endTime + 1d exactly → daysLate = 1 → 1.5%.
    expect(
      refinanceApprovalOf(live, { expiresAt: endTime + DAY + 1n, graceSeconds }),
    ).toBe(payoffNoFee + (live.principal * 150n) / 10_000n);
  });
