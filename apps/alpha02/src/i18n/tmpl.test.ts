...
describe('interpolate', () => {
  it('substitutes {{var}} and leaves unknown placeholders intact', () => {
    ...
    // A wrong/missing param fails loudly (visible placeholder), not silently.
...
describe('tmpl', () => {
  it('is a callable tagged with its template meta', () => {
    const t = tmpl('You’re on {{chainName}}');
    expect(isTmpl(t)).toBe(true);
    expect(t[TMPL].template).toBe('You’re on {{chainName}}');
    // Directly callable (pre-init / SSR / test path) → English.
    expect(t({ chainName: 'Base' })).toBe('You’re on Base');
  });

  it('selects the English plural variant by count', () => {
    const t = tmpl('You have {{count}} active positions.', {
      one: 'You have {{count}} active position.',
    });
    expect(t({ count: 1 })).toBe('You have 1 active position.');
    expect(t({ count: 3 })).toBe('You have 3 active positions.');
  });
});

describe('buildTemplate emits tmpl entries', () => {
  it('emits a simple template under its key', () => {
    const out = buildTemplate({ nudge: tmpl('On {{chain}}') }) as Record<string, string>;
    expect(out).toEqual({ nudge: 'On {{chain}}' });
  });

  it('emits _one / _other siblings for a plural', () => {
    const out = buildTemplate({
      active: tmpl('You have {{count}} positions.', {
...
describe('createTranslatedCopy translates tmpl leaves', () => {
  ...
  const source = {
    home: {
      ...
      activePositions: tmpl('You have {{count}} active positions.', {
        one: 'You have {{count}} active position.',
      }),
    },
  };
  ...
  it('serves the interpolated English default when no bundle carries the key', async () => {
    ...
    expect(copy.home.testnetNudge({ chainName: 'Base Sepolia' })).toBe(
      'You’re on Base Sepolia, a test network.',
    );
    // English plural via the JS-selected defaultValue.
    expect(copy.home.activePositions({ count: 1 })).toBe('You have 1 active position.');
    expect(copy.home.activePositions({ count: 5 })).toBe('You have 5 active positions.');
  });
  ...
  it('resolves a translated bundle and interpolates the value', async () => {
    ...
    const copy = createTranslatedCopy(source);
    expect(copy.home.testnetNudge({ chainName: 'Base' })).toBe(
      'Estás en Base, una red de prueba.',
    );
    // Locale-aware plural selection through i18next.
    expect(copy.home.activePositions({ count: 1 })).toBe('Tienes 1 posición activa.');
    expect(copy.home.activePositions({ count: 4 })).toBe('Tienes 4 posiciones activas.');
    await i18n.changeLanguage('en');
  });
