...
| `live-rate-desk.mjs` | #1129/#1134 — Rate Desk live half: loads the WETH/tLIQ market via the custom-pair branch (chain-read book), posts a 0.002 WETH GTC/Partial lend order at a distinctive rate, amends it in place (one `modifyOffer`, same offer id), waits out the REAL 300 s cancel cooldown, cancels (escrow refunded) — every offer-scoped step verified on-chain via viem, and the honest degraded states of the markets summary + tape (indexer routes down) observed and reported, never failed. NB: performs real testnet writes and blocks ~6 min on the genuine cooldown; self-cleans via a direct on-chain cancel fallback if the UI path dies mid-run. |
