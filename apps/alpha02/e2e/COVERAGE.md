...
| Support ticket capture — deployed Worker + D1 + ops-Telegram (#1040 phase 1) | Live-only | post-deploy drive: send a real ticket on alpha02.vaipakam.com, verify the ticket id renders and the D1 row + ops-bot message land | Needs the deployed agent Worker (real D1 migration applied, real ops-bot secrets) — the stub tier above deliberately fakes exactly these. |
