...
| Permit2 signature approvals (#1038) | CI-Anvil | `tests/12-permit2.spec.ts` (single-transaction property + classic fallback via the fixture's Permit2-reject flag) — plus specs 02/03 now exercise the permit path implicitly, against the REAL canonical Permit2 on the fork. The VPFI-deposit permit wiring shares the identical pattern but has no spec opening /vpfi (see the VPFI gap row). | — |
