...
| Telegram alert rails: link/unlink/opt-out with wallet signatures (#1055/#1056) | Live-only | `live/live-alerts-link.mjs` | Needs the deployed agent Worker (CORS origin gate, D1, signature verification) and the Telegram bot — no Anvil equivalent. |
...
| VPFI vault deposit/withdraw dry run | CI-Anvil (via 08) | `tests/08-dryrun-footer.spec.ts` | — |
