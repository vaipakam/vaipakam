...
The `DB` binding in `wrangler.jsonc` points at the **`vaipakam-archive`** D1 database (id `3cffebf5-b652-4da7-953c-9e1d143ad2fe`), **shared** with `apps/indexer` and `apps/keeper`. The agent writes `user_thresholds`, `notify_state`, `telegram_links`, `diag_errors`, `diag_legal_holds`, `diag_legal_hold_audit`, and reads `loans`.
