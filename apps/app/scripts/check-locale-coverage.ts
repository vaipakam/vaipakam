 ...
 *
 *     pnpm --filter @vaipakam/app i18n:coverage
 *
 ...
 * also runnable on its own — `pnpm --filter @vaipakam/app
 ...
 * follow-up. Generated — regenerate with `pnpm --filter @vaipakam/app i18n:coverage -- --prune`,
 ...
 * ignores the now-missing old leaf. Every app check green while the
...
if (stalePairs.length > 0 && !pruning) {
  problems.push(
    `${stalePairs.length} baseline entr(y/ies) already translated — run ` +
      `\`pnpm --filter @vaipakam/app i18n:coverage -- --prune\`: ` +
...
if (staleEnglishValued.length > 0 && !pruning) {
  problems.push(
    `${staleEnglishValued.length} english-valued baseline entr(y/ies) already ` +
      `translated — run \`pnpm --filter @vaipakam/app i18n:coverage -- --prune\`: ` +
...
if (problems.length > 0) {
  ...
  console.error(
    '\nFill the gaps rather than widening KNOWN_GAPS:\n' +
      ...
      '    --locales-dir apps/app/src/i18n/locales --missing-only\n' +
  ...
  if (problems.some((p) => p.includes('hold the English string'))) {
    console.error(
      '\nThose keys are PRESENT — they just still read in English, so\n' +
        ...
        '  pnpm --filter @vaipakam/i18n translate -- \\\n' +
        '    --locales-dir apps/app/src/i18n/locales <code>\n' +
