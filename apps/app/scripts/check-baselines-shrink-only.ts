...
const POLICY: Readonly<Record<string, AcceptedAsTranslatedEntry>> = (() => {
  const file = path.join(
    REPO_ROOT,
    'apps/app/src/i18n/translation-policy.json',
  );
...
const BASELINES = [
  'apps/app/src/i18n/untranslated-baseline.json',
  'apps/app/src/i18n/english-valued-baseline.json',
];
...
const currentValue = (pair: string): string | undefined => {
  ...
  const file = path.join(REPO_ROOT, 'apps/app/src/i18n/locales', `${code}.json`);
