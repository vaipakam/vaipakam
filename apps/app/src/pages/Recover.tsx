 ...
 * app's UI/UX) — returns ERC-20s that landed in the user's vault
 ...
 *
 * Flow (app in-page steps, no modal): form → review card with the
 ...
 */
const pendingRecoveryStore = makePendingMarkerStore('app.recoverPending');
...
async function claimPendingRecovery(
  ...
  const outcome = await lockManager.request(
    ...
    `app.recoverClaim.${chainId}.${account.toLowerCase()}`,
    { ifAvailable: true },
