...
- **`en.json`** — the translators' TEMPLATE, generated from
  `src/content/copy.ts` by `pnpm --filter @vaipakam/app
...
its own with `pnpm --filter @vaipakam/app i18n:coverage` (#1362): introducing a token the
...
ANTHROPIC_API_KEY=... pnpm --filter @vaipakam/i18n translate -- \
  --locales-dir apps/app/src/i18n/locales es zh hi ja
```
...
pnpm --filter @vaipakam/i18n merge-patch -- \
  --locales-dir apps/app/src/i18n/locales --patches path/to/patches
```
