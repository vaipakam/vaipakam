...
export function isHoldingAfterSubmit(input: ForcedCloseHoldInput): boolean {
  ...
  if (input.disposition === 'success') {
    return input.readsUpdatedAt <= input.submittedAt;
  }

  // `reverted` and `cancelled`: the chain says the close-out did not
  // execute. Nothing changed, a retry is legitimate, and applying the
