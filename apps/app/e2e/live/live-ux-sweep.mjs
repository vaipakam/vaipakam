 ...
 *  app.vaipakam.com): a real browser load produced exactly one
...
for (const pass of session.passes) {
  ...
  await page.evaluate((m) => localStorage.setItem('app.mode', m), pass.mode);
  ...
  for (const route of routes) {
    ...
    //
    // No false positive on `/definitely-not-a-page`: app's Worker
    ...
    const landmarks = navError !== null ? null : await page
      .evaluate(() => ({
        mode: localStorage.getItem('app.mode'),
...
await page
  .evaluate(() => localStorage.setItem('app.mode', 'basic'))
  .catch(() => {});
await done().catch(() => {});
} // end session loop
...
if (beaconPermitted.length > 0) {
  console.error(
    `CONFIG DEFECT (worse form): the injected Cloudflare Web Analytics beacon was ` +
      ...
      `what the live response headers actually contain, since apps/app/public/` +
