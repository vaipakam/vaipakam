// Shared Playwright driver for the app testnet review.
//
...
export async function launch({
  ...
  await setup('creating the browser profile directory', async () => {
    profileDir = freshProfile
      ? fs.mkdtempSync(path.join(os.tmpdir(), `app-fresh-${role}-`))
  ...
  // the live run refused every read app makes: the app talks to
