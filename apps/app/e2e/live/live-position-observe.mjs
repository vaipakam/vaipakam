...
/**
 * The page-head tracker — the state and predicates that used to sit here.
 *
 * #2120 — extracted to `pageHead.mjs` so that it runs under a fake page in
 * its own suite instead of being asserted against this file's source. What
 * this drive still owns is the WIRING: the deployment being observed, the
 * chain map its unknown-chain gate reads, the uncached fetch and probe
 * timeout the direct samples use, and the ordering clock above — handed in
 * ONCE, so the tracker and the ledger cannot end up on two clock origins.
 */
const pageHeadTracker = createPageHeadTracker({
  chainId: CHAIN_ID,
  diamondAddress: DIAMOND,
  observedPageChain,
  fetch: ufetch,
  probeTimeoutMs: CHAIN_PROBE_TIMEOUT_MS,
  now: orderingNow,
});
const {
  watchPageHead,
...
async function observeForcedClose(page, loan, headBeforeNav, pageHeadBeforeNav, pageSampledBeforeNav) {
  ...
  const diamondKeys = diamondKeysOf(page);
  const ceilingSound =
    ceiling.head > 0n &&
    [...diamondKeys].every(
      (k) => ceiling.sampled.has(k) || isForeign(k),
    );
...
async function readForcedCloseCard(page, timeoutMs = 30_000) {
  ...
  const mounted = await page
    .waitForFunction(
      // #2102 — THE SAME PREDICATE THE SCRAPE RUNS, by construction. This
      // wait used to rebuild `visible` from source strings sliced out of
      // this file by brace-matching and run them through `new Function`
      // INSIDE the page; when round 66 split the predicate into four
      // helpers the wait's fixed arity silently dropped one and threw
      // `ReferenceError` inside its own catch — a false absence. The
      // helpers now live in `visibility.mjs` and `withVisibility` composes
      // them with this body on the Node side, so there is no slice to
      // drift, no arity to keep in step, and no eval in the page.
      withVisibility((V) =>
        [...document.querySelectorAll('[data-testid="forced-close-card"]')].some(V.visible),
      ),
      undefined,
    ...
    .catch((err) => {
      ...
      // means the question was never put: the composed predicate failing
      // to evaluate, the execution context destroyed by a navigation
      // mid-poll, the page closing. Reading those as `false`
  ...
  const readCard = () =>
    page
      .evaluate(withVisibility((V) => {
        const { notClipped, paintsText, shownBox, visible, visibleTextOf } = V;
      ...
      if (rendered) {
        ...
        if (backAction.present) {
          ...
          backAction.painted = await back
            // #2102 — composed with the one predicate definition rather
            // than rebuilt in-page from sliced source.
            .evaluate(withVisibility((V, el) => {
              if (!V.visible(el)) return false;
              ...
              return leaves.length > 0 && leaves.some((n) => V.visible(n));
            }))
            .catch(() => null);
        }
