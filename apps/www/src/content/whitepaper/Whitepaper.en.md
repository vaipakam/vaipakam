...
The protocol maintains a per-user accumulator over the user's protocol-tracked vaulted VPFI balance. Every mutation of that balance closes the running accrual period at the tier that was in force and re-stamps at the **post-mutation** balance, recording each day's closing balance and each day's **lowest** balance. A user's effective tier is then derived from that history in three steps, all of which must pass:
