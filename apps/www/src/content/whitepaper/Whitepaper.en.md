...
1. **A minimum holding period.** Until the user's current balance has been held for a configured minimum number of days — a governance setting bounded between 2 and 14, currently 3 — the effective tier is zero and no discount applies at all. A balance that returns to zero restarts this clock.
2. **A time-weighted average** of the daily balance across a trailing window, which yields a raw tier. The window is a governance setting bounded between 14 and 30 days, currently 30, and its upper bound is structural: the protocol retains 30 days of daily history. The most recent days are weighted more heavily than the rest. The window never reaches back before the user's current balance began, so a recent depositor's average is not diluted by days they held nothing.
3. **A lowest-tier floor** over that same history, computed from each day's *minimum* balance rather than its closing balance, so a dip counts even when the balance is restored the same day. The effective tier is the lower of the raw tier and the lowest tier the user reached. In practice this floor spans the whole life of the current balance, capped at the retained 30 days.

Re-stamping at the post-mutation balance is the load-bearing anti-gaming choice. Stamping at the pre-mutation balance would let a user withdraw down to Tier 0 and keep accruing at their old high tier until the next balance change; stamping after the mutation means a withdrawal registers immediately. The lowest-tier floor is what makes a brief top-up worthless: an average can be pulled up by a late deposit, but a minimum cannot be, and a same-day dip is captured because the accumulator records the day's low as well as its close. A deposit made shortly before settlement therefore gains nothing — it fails the minimum holding period outright, and even once that period elapses the floor holds the tier at what was actually sustained.

**Historical note.** An earlier design averaged the discount across each loan's own lifetime, anchored per loan at origination. That averaging was removed: the per-loan anchors are retained in storage but are no longer read, and the anti-gaming property they existed to provide is now carried by the minimum holding period and the lowest-tier floor described above — which bind more tightly than an average did.
