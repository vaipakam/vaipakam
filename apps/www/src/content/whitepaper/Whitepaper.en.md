...
Two conditions narrow **this** path — the hold-tier discount on its own — and a loan that misses either pays the full fee: you must have enabled the platform-level fee-discount consent, and the lending asset must be liquid under the active chain's oracle and risk checks. Opting into the tariff described below is treated as that consent in itself, so it does not need the separate setting; and where a tariff opt-in cannot be completed the loan does not quietly fall back to the full fee — it is either refused or follows a downgrade you authorised in advance. The discount also **cannot take the fee to zero** — it is clamped at 50%, so half the gross fee is the floor however much VPFI you hold and whichever tariff you opt into.
