...
1. **A minimum holding period.** Until the user's current balance has been held for a configured minimum number of days, the effective tier is zero and no discount applies at all.
2. **A time-weighted average** of the daily balance across a trailing 30-day window, which yields a raw tier.
3. **A lowest-tier floor** over that same history, computed from each day's minimum balance. The effective tier is the lower of the raw tier and the lowest tier the user reached in the window.
