...
**Non-custodial at the user-action layer.** Assets move from a user's wallet into that user's own vault by explicit user action, and settlement flows transfer value directly to the counterparty's vault or the treasury rather than parking it in shared protocol custody. The Diamond itself holds user assets only for deliberately staged custody (for example liquidation swap-output routing, and — where the optional per-party VPFI tariff is enabled — the tariff paid at origination; borrower VPFI *fee* custody over a loan's life belongs to the retired fee path and applies only to loans opened while it was live), and claims are pull-based: funds are released only to the current holder of the relevant position NFT.
...
- **Claim Center** — pull-based claims for terminal loans (principal, interest, collateral, surpluses, and VPFI rebates where a loan still uses the retired custody fee path) plus platform-interaction reward claims.
...
- **Per-tier liquidation thresholds.** Liquidation thresholds are tier-specific rather than per-asset, with deeper tiers receiving higher thresholds (defaults `80%`, `85%`, `90%` for tiers 1–3 — the thinnest Tier 1 is liquidated earliest to leave the widest cushion for slippage, handling, and the liquidator bonus, while the deepest Tier 3 can safely tolerate the highest pre-liquidation LTV), governance tuning bounded, and cross-tier ordering preserved (`Tier 1 ≤ Tier 2 ≤ Tier 3`). A liquid asset too thin to clear even the Tier-1 depth probe is untierable and receives the most conservative Tier-1 threshold, never a deeper tier's. The threshold applied to a loan is snapshotted from the collateral's effective tier at initiation, so later tier degradation never retroactively changes an open loan's liquidation gate.
...
An offer may also be created without a transaction: the creator signs the binding offer terms once with an EIP-712 signature (ERC-1271 smart-contract wallets supported). The signed offer lives off-chain in the order book until a counterparty fills it; at fill it is materialized into an ordinary on-chain offer and accepted in the same transaction, so the resulting loan and every downstream rule — position NFTs, claims, liquidity classification, Health Factor gates — behave identically to an on-chain offer. The act of signing is the creator's risk-and-terms consent. Signed offers are currently **ERC-20-shaped only**: ERC-20 lender-principal offers and ERC-20-collateral borrower offers. NFT lender/rental offers, NFT-collateral shapes, and refinance-tagged offers are rejected at materialization (`SignedOfferUnsupportedShape`) — they must be created on-chain.
...
**Loan Initiation Fee.** At ERC-20 loan initiation, a Loan Initiation Fee of `{liveValue:loanInitiationFeeBps}`% of the lending-asset amount is charged to the borrower: the fee is deducted from the principal and routed to treasury before net proceeds are delivered, so a matched amount of 1,000 units delivers the net remainder to the borrower. The fee applies to loan initiation only — never to offer creation, modification, or cancellation — and the rate a loan pays is snapshotted at origination, so a later governance retune never changes an open loan's economics. A borrower holding VPFI pays a reduced fee directly, in the lending asset — applied at acceptance, with no VPFI leaving the vault to pay it — provided they have enabled the fee-discount consent and the lending asset is liquid; a loan missing either pays the full fee. Opting into the tariff does not substitute for that consent: the tariff authorises its own separate reduction, added on top, while the hold-tier reduction still requires the consent. An earlier model instead took the full fee equivalent in VPFI up front into protocol custody and returned part of it as a rebate at settlement; that path is retired and survives only for loans already open when it was retired. The tokenomics section specifies both. When the loan is initiated by the permissionless matcher, the matcher's configured share is paid from the treasury-directed fee flow.
...
- **Borrower side:** the collateral back after full repayment; the prepayment buffer after proper rental closure; any surplus remaining after a liquidation covers all obligations; and, for a loan still on the retired VPFI-custody fee path, the rebate paid out atomically with the normal borrower claim.
...
2. **Explicit consent.** The user must enable an on-chain consent flag before vaulted VPFI is used for fee discounts; with consent disabled the effective discount is zero regardless of balance. One flag covers both the borrower and lender legs, but it is **deployment-local**: the fee path reads the flag on the chain the loan settles on, so it must be enabled on each such chain. The canonical chain's flag additionally gates propagation — while it is off, the tier broadcast carries zero regardless of balance, so a user who settles only on a mirror must enable consent on the canonical chain as well as on that mirror.
...
The discount actually applied to the **lender yield fee** at settlement is **not a point-in-time tier lookup**, though it is resolved at the moment the fee is charged. The distinction is where the time-weighting lives: it is in the **tier**, not in a per-loan average. The borrower's initiation-fee discount resolves differently and earlier — it is fixed when the loan is accepted, so a later balance change does not move it; see Section 12.5.

The protocol maintains a per-user holding history over the user's protocol-tracked vaulted VPFI balance. Every mutation of that balance re-stamps the history at the **post-mutation** balance, recording each day's closing balance and each day's **lowest** balance. A user's effective tier is then derived from that history in three steps, all of which must pass:

1. **A minimum holding period.** Until the user has held a non-zero balance *continuously* for a configured minimum number of days — a governance setting bounded between 2 and 14, currently 3 — the effective tier is zero and no discount applies at all. The clock starts when the balance goes from zero to positive and restarts only when it returns to zero; adding to an existing holding does not restart it. This gate therefore constrains newcomers, not established holders who top up.
2. **A time-weighted average** of the daily balance across a trailing window, which yields a raw tier. The window is a governance setting bounded between 14 and 30 days, currently 30, and its upper bound is structural: the protocol retains 30 days of daily history. The most recent days are weighted more heavily than the rest. The window never reaches back before the user's current holding began, so a recent depositor's average is not diluted by days they held nothing.
3. **A lowest-tier floor**, computed from each day's *minimum* balance rather than its closing balance, so a dip counts even when the balance is restored the same day. The effective tier is the lower of the raw tier and the lowest tier the user reached. **This floor is measured over its own history, not over the averaging window of step 2** — it spans the whole life of the current holding, capped at the retained 30 days. The two therefore diverge whenever governance sets the averaging window below 30 days: with a 14-day window, a dip 20 days ago still binds the tier while falling outside the average entirely.

Re-stamping at the post-mutation balance is the load-bearing anti-gaming choice. Stamping at the pre-mutation balance would let a user withdraw down to Tier 0 and keep accruing at their old high tier until the next balance change; stamping after the mutation means a withdrawal registers immediately. The lowest-tier floor is what makes a brief top-up worthless: an average can be pulled up by a late deposit, but a minimum cannot be, and a same-day dip is captured because the accumulator records the day's low as well as its close. A deposit made shortly before settlement therefore gains nothing. Note that the floor, not the holding period, is what does this for an established holder: their clock was satisfied long ago and a top-up does not restart it.

**Historical note.** An earlier design averaged the discount across each loan's own lifetime, anchored per loan at origination. That averaging was removed: the per-loan anchors are retained in storage but are no longer read, and the anti-gaming property they existed to provide is now carried by the minimum holding period and the lowest-tier floor described above.

Against the specific pattern both designs targeted — a brief top-up before settlement — the replacement binds more tightly, since an average can be pulled upward by a late deposit and a minimum cannot. It is **not** uniformly stricter, and the difference is worth stating rather than glossing: because the lowest-tier floor reaches back at most 30 days, a long loan's early low-tier period eventually leaves the history, where a loan-lifetime average would have retained it for the whole term. On a 100-day loan whose lender held a low tier for the first 70 days and a high tier for the last 30, the current rule can resolve the high tier while the removed one could not. The change strengthened the brief-top-up defence and shortened the memory.

The three steps above are governance-bounded, and two details of their behaviour are worth stating explicitly: the time-weighting applies recency weighting within its window, and a balance that returns to zero resets the holding history, so the minimum holding period must be served again from scratch. Mirror-chain tier caches are nonce-ordered (a stale push can never overwrite a fresher entry), carry an expiry and a maximum age, and are invalidated wholesale when governance bumps the tier-table version.
...
Two conditions narrow **this** path — the hold-tier discount on its own — and a loan that misses either pays the full fee: you must have enabled the platform-level fee-discount consent, and the lending asset must be liquid under the active chain's oracle and risk checks. Opting into the tariff described below does **not** stand in for that consent — the tariff authorises only its own slice, which is added on top, so a tiered borrower who pays the tariff with the consent still disabled receives the tariff's slice alone and not their hold tier; and where a tariff opt-in cannot be completed the loan does not quietly fall back to the full fee — it is either refused or follows a downgrade you authorised in advance. The discount also **cannot take the fee to zero** — it is clamped at 50%, so half the gross fee is the floor however much VPFI you hold and whichever tariff you opt into.
...
**Loans opened under the earlier model are unaffected.** An earlier design took the full, non-discounted fee equivalent in VPFI from the borrower's vault into protocol custody for the life of the loan, then split it at settlement into a borrower rebate and a treasury share — with the rebate forfeited on default or Health-Factor liquidation. **That path is retired for new loans.** Loans that were already open when it was retired still settle exactly that way: a rebate on one of them is claimable alongside the ordinary borrower claim **when the loan closes properly** — repayment, early close, or refinance. If it defaults or is liquidated instead, the held VPFI is forfeited and there is no rebate: it is routed to treasury less the matcher's configured share on a matcher-created loan, the same share the loan-initiation fee flow pays. If you are not holding such a loan, there is no rebate pending for you, and none will accrue.
...
- **Reporters.** Every deployment — **including Base itself** — reports at day-close (triggered lazily by the first interaction of the next UTC day, or by a permissionless day-close poke). Mirror chains send their report over the cross-chain messenger; Base's own day-close writes its totals **directly into the aggregator's storage** under its own chain id, with no message hop. The admin-configured expected-source set therefore lists Base plus every mirror — omitting Base would silently exclude canonical-chain activity from the global denominators. The payload carries three fields: the day identifier, the chain's total lender interest **earned** for the day, and the chain's total borrower interest **paid** for the day. The lender-side and borrower-side figures travel **separately** — each side of the 50/50 split needs its own global denominator, and a single combined number cannot reconstruct the two.
...
- **Finalization rule.** A day finalizes once **all expected chains have reported**, or once a **grace window (default 4 hours, admin-tunable) measured from the first report received for that day** has elapsed — whichever comes first. If no report ever arrives, the grace clock never starts and only the admin force-finalize override can close the day. A report arriving after finalization is **rejected outright** — it is never stored and never reopens a finalized day, so claim determinism is preserved absolutely; missed contributions are traced through emitted events, not stored late reports.
...
| --- | --- | --- |
| Permissionless | `triggerLiquidation`, `triggerDefault`, `repayLoan` (full repayment of **ERC-20 loans** — NFT-rental close-out stays borrower-only because it settles from the borrower's vaulted prepayment), `autoDeductDaily`, `matchOffers` (when the matching switch is enabled) | Open to any caller because the action is a protocol-safety function or a deterministic action with no caller discretion; full repayment by a third party never transfers collateral or claim rights. |
...
17. Vaipakam repository documentation: `SECURITY.md`, `docs/FunctionalSpecs/`,
    `docs/FunctionalSpecs/TokenomicsTechSpec.md` — https://github.com/vaipakam/vaipakam (security reports:
