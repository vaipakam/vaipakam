...
系统会从 loaned amount 中收取一笔很小的 **Loan Initiation Fee (`{liveValue:loanInitiationFeeBps}`%)**，并转入 protocol treasury。因此 borrower 收到的是 `{liveValue:exampleBorrowerReceives}` USDC，而不是 1,000。（持有 **VPFI** 可以降低该 fee，让 borrower 保留更多的 1,000——折扣直接从 fee 中减免，而不是事后返还；VPFI 会在下文说明。）
