...
Vaipakamは、vault-to-vaultのlendingプラットフォームです。資産を
...
1,000 USDCではなく`{liveValue:exampleBorrowerReceives}` USDCです。（このfeeを**VPFI**で支払うことも
...
- あなたは**`{liveValue:exampleLenderNet}` USDC**を受け取ります（principal + interest
  から、interest部分にのみかかる`{liveValue:treasuryFeeBps}`% Yield Feeを差し引いた額）
- Treasuryは**`{liveValue:exampleTreasuryYieldFee}` USDC**をYield Feeとして受け取ります
- BorrowerのWETHはunlockされます

これらの数値はcent単位に丸めてあります。正確なinterestは6.575342 USDC、
正確なYield Feeは`{liveValue:exampleTreasuryYieldFeeExact}` USDCなので、丸めた数値どうしを引き算すると
1 centずれます。protocolは丸める前の金額でsettleします。

dashboardに**Claim** buttonが表示されます。clickすると、`{liveValue:exampleLenderNet}`
...
> **Blockchain network gas feeに関する注意。** Offerをcreateする時、
> loanをacceptする時、repayする時、claimする時、その他のon-chain
> actionを行う時には、あなたのtransactionをblockに含めるblockchain
> validatorsに対して、別途小さな**network gas fee**も支払います。
> そのgas feeはnetworkに行きます — **Vaipakamには行きません**。
> 同じchain上で他のtokenを送るときに支払うfeeと同じものです。金額は
> chainとその時点のnetwork混雑度に依存し、loanのsizeには依存しません。
> 上記のprotocol fees（Yield Fee `{liveValue:treasuryFeeBps}`%、
> Loan Initiation Fee `{liveValue:loanInitiationFeeBps}`%）は
> network gasとは完全に別物で、protocol自身が徴収するのはこの2つ
> だけです。
...
**VPFI**はVaipakamのprotocol tokenです。2つの役割があります。
...
- **Bridge it** — VPFIはChainlink CCIP CCT tokenなので、official bridge
  を使ってsupported chains間を移動できます。また、VPFIが取引されている
  open marketで入手することもできます。
...
**protocol feesを下げる**だけが目的の場合は、**Dashboard** pageで
VPFIをvaultへdepositします。それだけです — fee-discount tierはその
瞬間からautomaticに適用されます。
