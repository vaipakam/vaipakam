...
- Tú recibes **`{liveValue:exampleLenderNet}` USDC** (principal + intereses menos una
  Comisión sobre Rendimiento del `{liveValue:treasuryFeeBps}`% aplicada únicamente a los intereses)
- El tesoro recibe **`{liveValue:exampleTreasuryYieldFee}` USDC** como Comisión sobre Rendimiento
- El WETH del prestatario se desbloquea

Estas cifras están redondeadas al céntimo. Los intereses exactos son
6,575342 USDC y la Comisión sobre Rendimiento exacta `{liveValue:exampleTreasuryYieldFeeExact}` USDC, así
que restar una cifra redondeada de otra te deja a un céntimo — el
protocolo liquida con los importes sin redondear.

En tu panel ves un botón **Reclamar**. Al hacer clic, los `{liveValue:exampleLenderNet}`
...
un préstamo no manipula el cálculo: el periodo mínimo de tenencia lo
rechaza de entrada, y la regla del nivel más bajo mantiene tu tasa
en lo que realmente sostuviste.
