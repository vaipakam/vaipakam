...
Kreditnehmer erhält also `{liveValue:exampleBorrowerReceives}` USDC, nicht 1.000. (Sie können die
...
- Sie erhalten **`{liveValue:exampleLenderNet}` USDC** (Hauptbetrag + Zinsen abzüglich
  einer Renditegebühr von `{liveValue:treasuryFeeBps}` % nur auf den Zinsanteil)
- Die Treasury erhält **`{liveValue:exampleTreasuryYieldFee}` USDC** als Renditegebühr
- Das WETH des Kreditnehmers wird entsperrt

Diese Beträge sind auf den Cent gerundet. Die exakten Zinsen betragen
6,575342 USDC und die exakte Renditegebühr `{liveValue:exampleTreasuryYieldFeeExact}` USDC. Wer eine
...
klicken, und die `{liveValue:exampleLenderNet}` USDC wandern aus der Abwicklung in Ihr
