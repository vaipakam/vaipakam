...
作为过滤器芯片公开；其他是 indexer 端终端，将在后续工作中获得
专门的 UI 处理：
...
- **Sold** — 报价已选择加入 borrow-OR-sell 并行销售流程
  ...
  地址不是。借款人的 Activity 提要钱包匹配由 indexer 在
  摄取时添加（它连接报价行以查找创建者），因此 per-wallet
  过滤器找到借款人，无需事件本身对其索引。
- **Fully Filled（indexer 状态，尚无芯片）** — 仅限 Range
  订单。当部分填充匹配消耗报价的剩余预算时（最后一次匹配
  完全填满范围，或部分匹配留下 sub-dust 余额），
  `OfferMatchFacet` 发出 `OfferClosed(FullyFilled | Dust)`，
  indexer 在报价行上盖上 `status = 'fullyFilled'`。合约的
  `accepted` 状态和上面的链上 Filled 标签保留用于
  direct-accept 终端，因此 `fullyFilled` 在 indexer 端是
  区分的。dapp 的 `MyOfferStatus` 尚未将此终端作为其自己的
  过滤器芯片公开 — `useMyOffers` 当前忽略具有 `fullyFilled`
  indexer 状态的行 — 因此完全填充的范围报价在专用芯片登陆
  之前实际上完全从 My Offers 视图中消失。芯片表面作为单独的
  UI 后续跟进排队。

从未达到终端事件的过去 GTT（Good-Til-Time）报价尚未在 dapp 中
作为不同的状态芯片公开；目前归类在 Active 下，直到 indexer
...
GTT 报价，此到期是报价的原始 Good-Til-Time；出借人接受不会
...
在 OpenSea 上获得实际可购买的列表是 dapp 今天不会自动化的
单独两部分步骤：
...
2. **在 OpenSea 上发布。** 重建 facet 构建的相同 OrderComponents。
   ...
   `Seaport.getCounter(ctx.borrowerVault)`，而不是 bidder 的
...
- *任何出借人接受之前*（场景 A）：填充 OpenSea 列表的买家支付
  ...
  executor 仅将 **净收益**（listed price 减去这些 marketplace
...
- *出借人接受之后*（场景 B）：列表通过贷款初始化继续 — 借款人
  ...
  **净收益**（销售价格减去 marketplace / creator fees）传递
...
- *填充时间块（开放 PrecloseFacet offset）。* 如果贷款有
  ...
  链接被清除。dapp 目前不会在 Loan Details 页面上为此组合
