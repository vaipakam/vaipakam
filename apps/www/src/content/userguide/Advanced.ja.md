...
- **HF-liquidation または default** — それでも確認してください。surplus が
  ...
  close-out が取引所に出されず内部で反対側の position とマッチした場合、そして swap がそもそも成
  立しなかった場合です — 売却に失敗すると collateral の引き渡しに戻ります。通常の liquidation
  も時間ベースの default も、取引所に向かう前にその内部マッチを試みますが、割引での引き取りは試みません。つまりこの二つの経路は結果が一定ではないということで、だからこそ経路ではなく claim を読むべきなのです。割引での引き取りはむし
  ろ確定していますが、向きが逆です: 売りもマッチもしないので、残るのは常に collateral です。部分 liquidation はそもそも close-out
