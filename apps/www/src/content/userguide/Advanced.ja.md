...
公開されています。他は indexer 側のターミナルで、後続作業で
専用 UI 処理を受けます:
...
- **Sold** — オファーが borrow-OR-sell の並行販売フローに
  オプトインされ(Create Offer → Allow optional sale を参照)、
  ...
  dapp は Activity フィードにも行を `Offer sold via OpenSea`
...
**ボロワーオファー** をポストする場合、dapp は担保セクションの
...
有効期限はオファーの元の Good-Til-Time です;レンダー受諾は
...
リスティングを取得することは、dapp が今日自動化していない
別の二部ステップです:
...
- *レンダーが受諾する前*(Scenario A): OpenSea リスティングを
  ...
  marketplace / creator fee レッグを引いたもの)のみを diamond に
...
- *レンダーが受諾した後*(Scenario B): リスティングはローン
  ...
  コレクションでは、Seaport は OpenSea protocol-fee と
  creator-fee レッグを最初に設定された受信者に直接ルーティング
  し、executor は **net proceeds**(販売価格から marketplace /
...
- *fill 時ブロック(open PrecloseFacet オフセット)。* ローンに
  ...
  がクリアされるまで fill 試行は revert します。dapp は現在、
