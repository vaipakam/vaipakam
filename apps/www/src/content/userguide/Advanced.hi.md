...
- **Sold** — ऑफ़र को borrow-OR-sell parallel-sale flow में opt-in
  किया गया (देखें Create Offer → Allow optional sale) और किसी
  ...
  के लिए surface करता है। on-chain event स्वयं है
...
Past-GTT (Good-Til-Time) ऑफ़र जो कभी terminal event पर नहीं पहुँचे
...
offer का मूल Good-Til-Time है; lender acceptance listing को
...
1. **Diamond पर record + wire करें।**
   ...
   `apps/agent/src/openseaFees.ts` reference है) और call करने
...
2. **OpenSea पर publish करें।** वही OrderComponents reconstruct
   ...
   bidder का counter) भी चाहिए। यह वही context है जिसे
...
place करेंगे — आपके specific collateral से tied bids, collection
के किसी token से नहीं। Vaipakam इन item offers को Loan Details
page पर real-time में surface करता है — "List collateral on
...
Collection-wide / criteria offers (bids जिन्हें collection में
...
demand collection-wide है, आज practical path item-specific bid
की प्रतीक्षा करना या listing को अपने fixed ask पर छोड़ना और
किसी भी buyer को इसे directly fulfill करने देना है। आप
collection-wide bid को manually स्वयं settle नहीं कर सकते —
...
जब आप acceptable offer पाते हैं और **Match offer** click करते
हैं, dapp **Confirm Match** modal खोलती है, जो matched value
...
> **No race window — atomic by construction.** यह v1 two-step
...
> जब तक bidder का `fulfillOrder` बाद के block में land नहीं
> होता — mempool देखने वाला कोई भी bidder को उस price से snipe
> कर सकता था जो उसने bid किया था। Atomic path उस hole को
> दोनों orders को एक Seaport match call में bind करके बंद करता
> है: या तो bidder agreed price पर fill करता है या पूरी
> transaction revert होती है।

**Match click करने से पहले आप जो verify करना चाहते हैं:**

- **Modal में matched value confirm करें।** Modal gross OpenSea
  offer amount surface करता है। Fee-enforced collections पर,
  diamond bidder-side marketplace / creator fee legs के बाद
  ...
  (row value, payment token, offer kind, truncated bidder, और
