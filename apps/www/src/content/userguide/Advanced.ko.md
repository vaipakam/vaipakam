...
- **HF-liquidation 또는 default** — 그래도 확인해 보세요. surplus가 남아
  ...
  기반 default도 거래소로 가기 전에 그 내부 매칭을 먼저 시도하지만, 할인된 값의 직접 인수는 그러지 않습니다. 그러니 이 두 경로는 결과가 늘 같지는 않습니다 — 그래서 경로가 아니라 claim을 읽어야 합니다. 할인 경로는 오히려
  예측 가능합니다. 다만 반대 방향으로: 팔지도 매칭하지도 않으므로 언제나 collateral이 남습니다. 부분
