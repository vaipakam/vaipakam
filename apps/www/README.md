...
- Public-read tools that live on the connected-app domain by industry convention (analytics, NFT verifier, protocol console) are linked OUT rather than duplicated here. All of that routing goes through ONE helper in `src/lib/appUrl.ts`:
  - `appUrl(destination)` — takes a named destination, never a raw path, and one `APP_TARGET` constant selects the host AND the route table together (the two surfaces disagree on paths: `/nft-verifier` vs `/nft`, `/vpfi-vault#step-2` vs `/vpfi#deposit`).
  - There used to be a second helper, `legacyToolUrl(path)`, for Analytics and the Protocol Console while those two were unported. #1959 ported both to `apps/app`, so the helper is **deleted** and every destination now goes through `appUrl`. Do not add another raw-path escape hatch — a named destination is what keeps the host and the route table from being chosen separately.
  `APP_TARGET` now resolves to the connected app, so the human-facing links point at `app.vaipakam.com`. `VITE_APP_TARGET=legacy` opts a build back out; the sense is inverted on purpose, so an unset or mistyped value lands on the surface users are meant to be on. The one deliberate exception is the ten `/recover` guide links, which still point at the legacy host — that flow reads a pending-recovery marker held in same-origin browser storage, and a reader mid-recovery who lands on the new origin cannot see their own marker and may broadcast a second recovery. Those move when the legacy attempts have drained, not when a route exists.
