...
### D1 — owns the canonical schema for `vaipakam-archive`

The `DB` binding in `wrangler.jsonc` points at the **`vaipakam-archive`** D1 database (id `3cffebf5-b652-4da7-953c-9e1d143ad2fe`). This Worker is the **schema owner**: `apps/indexer/migrations/` is the single source of truth for every table the live db holds, even ones only the sibling Workers write to (`apps/keeper` and `apps/agent` both bind to the same database id; neither has its own `migrations/` directory).

Apply migrations from inside this directory:

```bash
wrangler d1 migrations apply vaipakam-archive --local    # local dev
wrangler d1 migrations apply vaipakam-archive --remote   # production
```
