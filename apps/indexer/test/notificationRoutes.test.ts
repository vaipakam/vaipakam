 ...
 * — recipient-scoped, ordered newest-first by CHAIN ORDER
 * (block_number, log_index) with a keyset cursor, served no-store.
 * Read/unread state is client-side, so there is no server mutation
 * route to test. Runs against the REAL migrated schema.
 */
...
function makeHarness() {
  ...
  let auto = 0;
  /** Seed a row at (block, logIndex). `createdAt` is intentionally
   *  decoupled from block order so the tests prove the feed sorts by
   *  CHAIN order, not the (wall-clock-fallible) timestamp. */
  const seed = (
    recipient: string,
    kind: string,
    loanId: number,
    ...
    h.db
      ...
      .run(recipient, kind, loanId, block, logIndex, createdAt, `k${auto}`);
  };
...
describe('GET /notifications/:addr', () => {
  it('returns the wallet rows newest-first by CHAIN ORDER, excluding other wallets', async () => {
    const h = makeHarness();
    // createdAt is deliberately INVERTED vs block order — a wall-clock
    // fallback must not decide the feed order.
    h.seed(ME, 'loan_matched', 1, 100, 0, 9000);
    h.seed(ME, 'loan_repaid', 2, 300, 0, 1000); // newest by block, oldest ts
    h.seed(ME, 'partial_repay', 3, 200, 0, 5000);
    h.seed(OTHER, 'loan_matched', 9, 999, 0, 0); // another wallet — excluded
