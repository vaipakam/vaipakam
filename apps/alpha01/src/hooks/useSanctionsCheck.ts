...
export function useSanctionsCheck(who: string | null | undefined) {
  ...
  const [state, setState] = useState({ loading: false, isSanctioned: false, error: null as string | null });

  useEffect(() => {
    if (!who || !chain.diamondAddress) {
      setState({ loading: false, isSanctioned: false, error: null });
      return;
    }
    let cancelled = false;
    ...
    void checkSanctioned(publicClient, chain.diamondAddress as Address, who as Address)
      .then((flagged) => {
        if (!cancelled) setState({ loading: false, isSanctioned: flagged, error: null });
      })
      .catch((e) => {
        if (!cancelled) {
          setState({
