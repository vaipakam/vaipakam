...
function main(): void {
  ...
  // root English shell for `/es/dashboard` etc.
  //
  // Static-asset preserve rules MUST come first
  // -------------------------------------------
  // Cloudflare Pages with `200` status rewrites the response body
  // unconditionally — it does NOT skip rules when a real file exists
  // at the source path. So a naive catch-all like `/* / 200` matches
  // `/assets/index-abc.js` and rewrites the response to the body of
  // index.html, breaking every JS / JSON / image fetch (manifest.json
  // syntax errors, "MIME type text/html for module script", PWA
  // service-worker registration failure). Symptom in production:
  // deploy succeeds but the page is blank because none of the bundle
  // chunks load.
  //
  // The fix is explicit no-op rewrites for every static-asset prefix
  // BEFORE the locale and catch-all rules. `/:splat` echoes whatever
  // matched `*` back into the destination, so `/assets/x.js` rewrites
  // to `/assets/x.js` (a no-op) and Cloudflare's first-match wins,
  // skipping the SPA fallback below. This preserves Vite's hashed
  // bundle paths, the public/ folder static files (manifest.json,
  // sw.js, icons), and the build-emitted sitemap / robots.
  //
  // IMPORTANT: destinations on the locale rules use `/<locale>` (no
  // `/index.html` suffix) because Cloudflare's `_redirects`
  // loop-detector strips `.html` and `/index` during static analysis
  // and then rechecks the result against the source pattern. With
  // `/es/*` → `/es/index.html`, the analyser strips `index.html` to
  // get `/es/`, which matches `/es/*` again → false-positive
  // infinite-loop error and the deploy refuses to ship. Using just
  // `/es` side-steps the strip pass; Cloudflare still resolves the
  // directory's `index.html` at serve time.
  const redirects: string[] = [
    '# Auto-generated by scripts/generate-locale-shells.ts',
    '# Per-locale shell rewrite — Tier-A SSG.',
    '',
    '# Static asset pass-through. These no-op rewrites take precedence',
    '# over the SPA fallback below so JS/CSS/images/PWA files keep their',
    '# correct content-type instead of being served as index.html.',
    ...
    '/icon-dark.png  /icon-dark.png       200',
    '/icon-light.png /icon-light.png      200',
    '/logo-dark.png  /logo-dark.png       200',
    '/logo-light.png /logo-light.png      200',
    '/logo-stacked-dark.png  /logo-stacked-dark.png   200',
    '/logo-stacked-light.png /logo-stacked-light.png  200',
  ...
  redirects.push('');
  redirects.push('# Default English shell for SPA routes (anything not matched above).');
  redirects.push('/*    /    200');
  redirects.push('');
