...
function main(): void {
  ...
  // Then emit one shell per non-default locale. Two files per locale:
  //
  //   - `dist/<locale>/index.html` — for direct URL hits at `/<locale>/`
  //     (sitemap / hreflang / crawler URLs use the trailing-slash form).
  //   - `dist/<locale>.html` — top-level file used as the rewrite
  //     destination from `_redirects`. Using a `.html` file (not a
  //     directory index) is what avoids Cloudflare's
  //     `auto-trailing-slash` auto-redirect — when the rewrite
  //     destination is a directory path like `/ta`, Cloudflare sees
  //     `/ta/index.html` exists and 301-redirects the browser to
  //     `/ta/`, collapsing the original sub-route (e.g.
  //     `/ta/help/advanced`) down to `/ta/`. With a `.html` file, the
  //     destination is a literal file path so no directory redirect
  //     fires and the original URL is preserved in the address bar.
  for (const locale of SUPPORTED_LOCALES) {
    ...
    writeFileSync(join(DIST_DIR, `${locale}.html`), shell, 'utf-8');
    console.log(`[locale-shells] wrote dist/${locale}/index.html + dist/${locale}.html`);
  }
  ...
  // Cloudflare Workers Static Assets `_redirects`: route every URL
  // under `/<locale>/...` to that locale's shell so the crawler /
  // first paint gets the locale-correct `<html lang>` + meta tags.
  //
  // **Only the locale-rewrite rules go here.** Earlier iterations
  // also wrote a catch-all `/* / 200` rule meant as a SPA fallback
  // for the English root, but that rule fires unconditionally on
  // every request — including `/assets/<hash>.js` and
  // `/manifest.json` — and rewrites the response body to
  // `index.html`. Symptom in production: every JS module fetch fails
  // with "Expected JavaScript-or-Wasm module script but the server
  // responded with a MIME type of text/html", the manifest fails to
  // parse, and the page renders blank. Service workers also fail to
  // register.
  //
  // The English SPA fallback comes from
  // `wrangler.jsonc.assets.not_found_handling: "single-page-application"`
  // instead — that's the documented native mechanism in Workers
  // Static Assets and it correctly *only* fires when no static file
  // matches. We don't need to (and must not) duplicate it as a
  // catch-all here.
  //
  // Per-locale rewrites are still needed because `not_found_handling`
  // serves the root `index.html`, not the locale-prefixed shell —
  // `/es/dashboard` would otherwise drop the user onto the English
  // shell. The `/<locale>/* /<locale> 200` rewrite points the request
  // at the matching `dist/<locale>/index.html` instead.
  //
  // IMPORTANT: destinations are written as `/<locale>.html` (a top-
  // level file path), not `/<locale>` (a directory) and not
  // `/<locale>/index.html` (a directory index). The reasons:
  //
  // (1) `/<locale>/index.html` trips Cloudflare's `_redirects`
  //     loop-detector, which strips `.html` and `/index` from the
  //     destination during static analysis and then rechecks against
  //     the source pattern. The strip yields `/<locale>/`, which
  //     matches the source `/<locale>/*` (the `*` matches an empty
  //     string in the loop check) → false-positive infinite-loop
  //     error and the deploy refuses to ship.
  //
  // (2) `/<locale>` (no extension, no trailing slash) deploys cleanly
  //     but Cloudflare's default `assets.html_handling:
  //     "auto-trailing-slash"` then 301-redirects the browser from
  //     `/<locale>` to `/<locale>/` because `dist/<locale>/index.html`
  //     exists and the auto-trailing-slash logic kicks in *during the
  //     rewrite resolution*. The browser preserves the URL fragment
  //     across same-origin redirects but the path collapses, so a
  //     "Learn more →" link to `/ta/help/advanced#dashboard.your-escrow`
  //     ends up as `/ta/#dashboard.your-escrow` — the help/advanced
  //     segment vanishes.
  //
  // (3) `/<locale>.html` is a literal file (we now emit `dist/ta.html`
  //     etc. as a sibling of `dist/index.html`), so Cloudflare serves
  //     it directly without any directory normalisation. The strip
  //     pass yields `/<locale>` which doesn't match `/<locale>/*`
  //     (the source requires a `/`-then-something prefix), so the
  //     loop-detector is happy. URL bar preserves the original sub-
  //     route and the React app routes to the correct page.
  const redirects: string[] = [
    '# Auto-generated by scripts/generate-locale-shells.ts',
    '# Per-locale shell rewrite — Tier-A SSG.',
    '#',
    '# Only locale rewrites live here. The English SPA fallback comes',
    '# from wrangler.jsonc.assets.not_found_handling = "single-page-',
    '# application", which *only* fires on a 404 and therefore does',
    '# not intercept /assets/* or /manifest.json the way a catch-all',
    '# `/* / 200` rule would.',
    '#',
    '# Destinations are `/<locale>.html` (top-level files emitted by',
    '# this script) — NOT directory paths — so Cloudflare\'s default',
    '# `auto-trailing-slash` html_handling does not 301-redirect',
    '# /<locale>/sub/path to /<locale>/ and collapse the URL.',
    '',
  ];
  for (const locale of SUPPORTED_LOCALES) {
    if (locale === DEFAULT_LOCALE) continue;
    redirects.push(`/${locale}/*    /${locale}.html    200`);
  }
  redirects.push('');
  writeFileSync(join(DIST_DIR, '_redirects'), redirects.join('\n'), 'utf-8');
  console.log(`[locale-shells] wrote dist/_redirects (per-locale shell rewrites only; SPA fallback via wrangler.jsonc not_found_handling)`);
