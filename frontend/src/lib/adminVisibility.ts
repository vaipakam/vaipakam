/**
 * T-042 Phase 1d — public-visibility flag for the admin dashboard.
 *
 ...
 *
 * Operators who'd rather not surface the cockpit publicly (e.g. on a
 ...
 * the flag off:
 *   - Direct navigation to `/admin` redirects to home unless an admin
 *     / governance wallet is connected (Phase 4 wires the wallet
 *     check; Phase 1 hard-redirects unconditionally when the flag is
 *     off, since wallet-aware admin detection isn't live yet).
...
export function isAdminDashboardPublic(): boolean {
  try {
    const raw =
      (import.meta.env.VITE_ADMIN_DASHBOARD_PUBLIC as string | undefined) ?? '';
    // Default-true on missing / empty: the public dashboard is the
