...
export const ADMIN_KNOBS: KnobMeta[] = [
  ...
  {
    id: 'predominantDenominator',
    label: 'PAD — Predominantly Available Denominator',
    short:
      'Chainlink Feed Registry denominator the protocol pivots through when the active numeraire is non-USD. `Denominations.USD` (0x...0348) by post-deploy default. Routing through PAD biases toward Chainlink\'s top-rated USD feed set without needing on-chain rating metadata.',
    category: 'periodicInterest',
    unit: 'address',
    hardMin: '0',
    hardMax: '0',
    ...
    midMin: '0',
    midMax: '0',
    getter: { facet: 'ConfigFacet', fn: 'getPredominantDenominator', returns: 'address' },
    setter: {
      facet: 'ConfigFacet',
      fn: 'setPredominantDenominator',
      args: [
        { name: 'newDenominator', type: 'address' },
        { name: 'newSymbol', type: 'bytes32' },
        { name: 'newEthPadFeed', type: 'address' },
        { name: 'newPadNumeraireRateFeed', type: 'address' },
      ],
    },
    infoAnchor: 'predominantly-available-denominator-pad',
    hasNumericRange: false,
  },
  {
    id: 'predominantDenominatorSymbol',
    label: 'PAD — symbol (lowercase ASCII)',
    short:
      'bytes32 lowercase symbol used by Tellor/API3/DIA when querying asset/PAD pairs. Empty bytes32 reads as "usd". Rotated atomically alongside the PAD denominator via setPredominantDenominator.',
    category: 'periodicInterest',
    unit: 'bytes32',
    hardMin: '0',
    hardMax: '0',
    ...
    midMin: '0',
    midMax: '0',
    getter: { facet: 'ConfigFacet', fn: 'getPredominantDenominatorSymbol', returns: 'bytes32' },
    setter: {
      facet: 'ConfigFacet',
      fn: 'setPredominantDenominator',
      args: [
  ...
  {
    id: 'ethPadFeed',
    label: 'PAD — ETH/<PAD> Chainlink feed',
    short:
      'Chainlink AggregatorV3 returning ETH priced in PAD-units (ETH/USD on the post-deploy default). Load-bearing for WETH pricing AND the derived PAD/<numeraire> FX rate when no direct feed is set. REQUIRED on every chain post-T-048.',
    category: 'periodicInterest',
    unit: 'address',
    hardMin: '0',
    hardMax: '0',
    ...
    midMin: '0',
    midMax: '0',
    getter: { facet: 'ConfigFacet', fn: 'getEthPadFeed', returns: 'address' },
    setter: {
      facet: 'ConfigFacet',
      fn: 'setPredominantDenominator',
      args: [
  ...
  {
    id: 'padNumeraireRateFeed',
    label: 'PAD — direct PAD/<numeraire> FX feed (optional)',
    short:
      'Optional Chainlink AggregatorV3 returning the PAD/<numeraire> rate (e.g. USD/EUR on Ethereum mainnet). When unset (zero), the protocol derives the rate from ETH/<numeraire> ÷ ETH/PAD using existing infrastructure.',
    category: 'periodicInterest',
    unit: 'address',
    hardMin: '0',
    hardMax: '0',
    ...
    midMin: '0',
    midMax: '0',
    getter: { facet: 'ConfigFacet', fn: 'getPadNumeraireRateFeed', returns: 'address' },
    setter: {
      facet: 'ConfigFacet',
      fn: 'setPredominantDenominator',
      args: [
