...
protocol 的 view methods 针对您的 address 直接计算。每一行都会
deep-link 到完整 position 页面，里面包含 HF、LTV、accrued interest、
由您的 role 和 loan status 控制的 action surface，以及可粘贴到
...
window。这会关闭一种 gaming pattern：在 loan 即将结束时临时 top up
...
HF 计算由 protocol 自己执行 — gate 无法绕过。interest 上的 1%
...
提交前的 acknowledgement gate。同样的 risk surface 适用于双方；
...
- **Smart-contract risk** — contract 代码 runtime 不可变；bug 可能
  影响锁定的 collateral。
- **Oracle risk** — staleness 或操纵可能在真实市场价格本来安全
...
与 Buy VPFI 页面上的 "Step 3 — Unstake" 相同的 surface — 把 VPFI
...
由您的 role 和 loan status 控制的 action surface，以及 inline
keeper status。
...
时，常见的 buffer target 是 1.5。提高 HF 的 levers：
...
Action surface 由 protocol 按 role gated。下面的 role-specific tabs
...
- **Claim as lender** — Terminal-only。返回 principal 和 interest
...
- **Refinance** — 为新 terms 发布 borrower offer；一旦 lender accept，
  ...
  您的 escrow。
- **Claim as borrower** — Terminal-only。full repayment 时返还
...
每个 wallet 最多 5 个 keepers 的 keeper whitelist。每个 keeper 都有
...
2. 在 Offer Book 或 Loan Details surface 上设置的 per-loan
   opt-in toggle。
...
- 数据 freshness (跨 chain 的最大 staleness)。
...
State：offset 已 initiate，swap 仍在 mid-execution (或 quote 已被
