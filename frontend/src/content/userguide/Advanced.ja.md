...
各セクションは、カード名の横にある `(i)` info icon に対応して
います。
...
user ごとの upgradable contract — この chain 上にあるあなたの
private vault — です。あなたが最初に loan に参加したタイミング
で deploy されます。address ごと、chain ごとに 1 つの escrow。
あなたの loan positions に紐づく ERC-20、ERC-721、ERC-1155
balances を保持します。pooling はありません: 他の user の assets
がこの contract に入ることはありません。

escrow は、collateral、貸し出された assets、locked VPFI が置か
れる場所です。protocol は deposit / withdrawal のたびに、この
escrow に対して authenticate します。implementation は protocol
owner が update できますが、timelock 経由のみです — 即時には
変えられません。
...
対して live に calculate されます。各 row は full position page
へ deep-link し、HF、LTV、accrued interest、role と loan status
で gated された action surface、block explorer に貼れる on-chain
loan id を表示します。
...
Vaipakam は LayerZero V2 上で VPFI を cross-chain に move します。
...
April 2026 の業界全体の事故後、cross-chain message-verification
...
です — refinance は既存 loan を mutate せず、新しい loan を作成
します。
...
- **APR** — basis points (1% の 1/100) で表す fixed annual rate。
  acceptance 時に snapshot され、その後 react しません。
...
fee と 5% buffer です。NFT 自体は delegated state に移ります
...
submission 前の acknowledgement gate です。同じ risk surface が
...
backend cache はありません — page load ごとに re-fetch します。
...
Buy VPFI ページの "Step 3 — Unstake" と同じ surface です —
...
parties、role と loan status で gated された action surface、inline
...
Refinance はこれらの values を mutate せず、新しい loan を作成
します。
...
は "n/a" に collapse し、唯一の terminal path は default 時の full
...
ください — volatility を吸収する common buffer target は 1.5 です。
HF を引き上げる levers:
...
role ごとに protocol で gated された action surface です。下の
...
- **Lender として claim** — terminal のみ。Principal plus interest
...
- **Borrower として claim** — terminal のみ。full repayment では
...
2. Per-loan opt-in toggle。Offer Book または Loan Details surface
   で設定します。
...
- data freshness (chains 間の最大 staleness)。
...
status: offset initiated、swap が mid-execution (または quote は
consumed されたが final settle が pending) です。2 つの exits:
...
fair price を shape します: buyer の payoff は terminal 時の
