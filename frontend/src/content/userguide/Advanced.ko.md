...
처음 loan에 참여할 때 나를 위해 deploy됩니다. address당, chain당
...
escrow는 collateral, 빌려준 assets, locked VPFI가 보관되는 장소입니다.
protocol은 모든 deposit / withdrawal에서 이 escrow를 기준으로
authenticate합니다. implementation은 protocol owner가 update할 수 있지만,
...
role과 loan status로 gated된 action surface, block explorer에 붙여 넣을
수 있는 on-chain loan id를 함께 보여 줍니다.
...
Vaipakam은 LayerZero V2 위에서 VPFI를 cross-chain으로 move합니다.
...
April 2026 산업 사고 이후 hardened된 cross-chain 메시지 검증
...
VPFI를 top up해 full-tier discount를 받고 몇 초 뒤 withdraw하는 gaming
pattern을 막습니다.
...
만들어야 합니다. HF math는 protocol 자체 규칙이며 gate는 bypass할 수
...
immutable입니다 — refinance는 기존 loan을 mutate하지 않고 새 loan을
만듭니다.
...
- **Rental** 하위 유형 — ERC-4907 (rentable ERC-721) 및
  ...
  renter는 전체 rental cost(duration × daily fee)에 5% buffer를 더해
  upfront로 지불합니다.
...
- **APR** — basis points(1%의 1/100) 단위의 고정 연이율.
  acceptance 시 snapshot되며 이후에는 react하지 않습니다.
...
— duration × daily fee에 5% buffer를 더한 금액입니다. NFT 자체는
...
submission 전 acknowledgement gate입니다. 같은 risk surface가 양쪽에
...
- **Oracle 위험** — staleness 또는 manipulation은 real-market price로는
...
— page load마다 re-fetch합니다. events는 transaction hash별로 group되어
...
Buy VPFI 페이지의 "Step 3 — Unstake"와 같은 surface입니다 — escrow에서
...
gated된 action surface, inline keeper status를 render합니다.
...
Refinance는 이 values를 mutate하지 않고 새 loan을 만듭니다.
...
Illiquid collateral의 on-chain USD value는 zero입니다. HF와 LTV는 "n/a"로
collapse되고, 유일한 terminal path는 default 시 full collateral transfer입니다
...
volatility를 견디기 위한 common buffer target은 1.5입니다. HF를 끌어올리는
levers:
...
protocol이 role별로 gated한 action surface입니다. 아래 role-specific tabs가
...
- **Lender로 claim** — terminal 전용. Principal plus interest minus 1%
...
- **Early withdrawal 시작** — 선택한 asking price로 lender position NFT를
  ...
  proceeds를 받습니다. sale fill 전에는 cancel할 수 있습니다.
...
- **Refinance** — 새 terms로 borrower offer를 게시합니다. lender가 accept하면
  ...
  atomically swap합니다.
- **Borrower로 claim** — terminal 전용. full repayment 시 collateral을
...
allowance를 bypass합니다. 따라서 여기 list가 clean해도 향후 deposits를
막지 않습니다.
...
channel creation까지 stubbed 상태입니다.
...
settled되었고 mid-claim인 position을 식별할 수 있습니다.
...
wallet당 최대 5 keepers까지 둘 수 있는 keeper whitelist입니다. 각 keeper는
...
2. per-loan opt-in toggle. Offer Book 또는 Loan Details surface에서 설정.

keeper는 네 conditions가 모두 true일 때만 act할 수 있습니다: approved,
master switch on, per-loan toggle on, 그리고 그 keeper에 specific action
permission이 set되어 있음.
...
- data freshness(chains 간 최대 staleness).
...
상태: offset initiated, swap이 mid-execution 상태입니다(또는 quote는
...
price를 shape합니다: buyer의 payoff는 terminal에서 principal plus interest이며,
