...
हर सेक्शन कार्ड के title के पास दिखने वाले `(i)` info icon से
जुड़ा है।
...
तिजोरी — जो आपके पहली बार किसी loan में भाग लेने पर deploy
होता है। प्रति address, प्रति chain एक escrow। इसमें आपकी loan
...
और locked VPFI custody में रहते हैं। protocol हर deposit और
withdrawal को इसी escrow के against authenticate करता है।
implementation को protocol owner update कर सकता है, लेकिन
...
इस chain पर connected wallet से जुड़ा हर loan —
चाहे आप lender side पर हों, borrower side पर हों, या अलग-अलग
positions में दोनों। protocol के view methods से आपके address
के लिए live calculate होता है। हर row पूरी position page पर
deep-link करती है और साथ में HF, LTV, accrued interest, आपकी
role, loan status से gated action surface, और on-chain loan id
दिखाती है, जिसे आप block explorer में paste कर सकते हैं।
...
lock-on-send / release-on-receive semantics चलाता है। हर दूसरी
सपोर्टेड chain mirror चलाती है, जो आने वाले bridge packets पर
mint करती है और बाहर जाने वाले packets पर burn। design के
हिसाब से, bridging के दौरान सभी chains पर कुल supply invariant
रहती है।

April 2026 की उद्योग-व्यापी घटना के बाद hardened cross-chain
...
tier जारी रहे। इससे वह gaming pattern बंद होता है जहाँ कोई
...
आकांक्षात्मक सारांश कार्ड। एक ही दृश्य में दोनों धाराओं में
जुड़े वॉलेट की संयुक्त VPFI पुरस्कार छवि को सतह पर लाता है,
मुख्य आंकड़ा
`stakingPending + stakingLifetimeClaimed + interactionPending
+ interactionLifetimeClaimed` है।

प्रति-धारा विभाजन पंक्तियाँ लंबित + दावा किया गया दिखाती हैं
और इसके मूल पृष्ठ पर पूर्ण दावा कार्ड के लिए शेवरॉन डीप-लिंक:

- **स्टेकिंग प्रतिफल** — `previewStakingRewards()` (लाइव
  पढ़ें) + स्थानीय log-index से इस वॉलेट के लिए हर
  `StakingRewardsClaimed` इवेंट का योग। डीप-लिंक
  `/buy-vpfi#staking-rewards`।
- **प्लेटफ़ॉर्म-इंटरैक्शन पुरस्कार** —
  `previewInteractionRewards()` वॉलेट के सभी ऋणों में + हर
  `InteractionRewardsClaimed` इवेंट का योग। डीप-लिंक
  `/app/claims#interaction-rewards`।

जीवनकाल दावा संख्याएं log-index इवेंट स्कैन से क्लाइंट-साइड
से प्राप्त की जाती हैं — चालू कुल के लिए कोई ऑन-चेन getter
नहीं है। एक नया ब्राउज़र कैश 0 (या आंशिक) जीवनकाल दिखाता है
जब तक कि (chain, diamond) स्कैन ऐतिहासिक ब्लॉकों को नहीं
भरता; एक बार स्कैन पूरा होने पर संख्या सच्चाई पर पहुंच जाती
है। अंतर्निहित दावा कार्डों के समान विश्वास मॉडल।

कार्ड हमेशा कनेक्ट किए गए वॉलेट के लिए रेंडर होता है, यहां
तक कि सभी-शून्य स्थिति में भी। खाली-स्थिति संकेत जानबूझकर है
— शून्य पर कार्ड छिपाने से पुरस्कार कार्यक्रम नए उपयोगकर्ताओं
के लिए अदृश्य हो जाएंगे जब तक कि वे Buy VPFI या Claim Center
में नहीं जाते।
...
को mutate नहीं करता, बल्कि नया loan बनाता है।
...
- **Rental** sub-type — ERC-4907 (rentable ERC-721) और
  ...
  चुकाता है (अवधि × दैनिक शुल्क) साथ ही 5% buffer।
...
साथ में 5% buffer। NFT खुद delegated state में चला जाता है
...
- **Illiquid** — जो ऊपर वाली liquid criteria pass न करे।
  ...
  तौर पर acknowledge करना होता है, तभी offer submit हो सकता है।
...
submission से पहले acknowledgement gate. वही risk surface दोनों
पक्षों पर लागू होता है; नीचे की role-specific tabs बताती हैं
कि offer के जिस side पर आप sign कर रहे हैं, उसके हिसाब से हर
risk आपको कैसे प्रभावित करता है। Vaipakam non-custodial है:
...
- **Oracle जोखिम** — staleness या manipulation आपके ख़िलाफ़
...
cache नहीं — हर page load re-fetch करता है। Events transaction
...
VPFI और rolling 24 घंटों में 500,000 VPFI। governance इन्हें
...
यदि उस chain पर Permit2 configured नहीं है, तो यह gracefully
fall back हो जाता है।
...
Buy VPFI page के "Step 3 — Unstake" जैसा ही surface — escrow
...
role और loan status से gated action surface, और inline keeper
status render करता है।
...
Refinance इन values को mutate करने के बजाय एक ताज़ा loan
बनाता है।
...
"n/a" पर collapse हो जाते हैं और एकमात्र terminal path default
...
volatility absorb करने के लिए 1.5 एक common buffer target है।
...
protocol द्वारा role के हिसाब से gated action surface। नीचे की
...
- **Claim as lender** — सिर्फ़ terminal. Principal plus
...
- **Claim as borrower** — सिर्फ़ terminal. पूरे repayment पर
...
- Data freshness (chains के बीच अधिकतम staleness)।
...
यदि नई terms पर HF 1.5 से नीचे होगा तो revert करता है।
...
State: offset initiated, swap mid-execution में है (या quote
consumed लेकिन final settle pending)। दो exits:
...
expected fair price shape करते हैं: buyer का payoff terminal
