...
vault 中的同一笔 VPFI 也会自动视为 "staked"，并赚取 5% APR。
...
### Step 2 — 把 VPFI 存入您的 vault

购买 VPFI 后，它会进入您的 wallet，而不是 vault。要获得 fee
discount 和 5% staking yield，您需要亲自把它移入 vault。这始终
是一个明确的一键操作 — App 不会在未经您同意的情况下移动您的 VPFI。
一笔 transaction (在支持的 chains 上可以是一笔 signature) 即可完成。

<a id="buy-vpfi.unstake"></a>

### Step 3 — 从 vault 中 unstake VPFI
...
1. **Staking** — 您保留在 vault 中的 VPFI 自动赚取 5% APR。
...
把 VPFI 从 vault 移回您的 wallet。一旦回到 wallet，它就不再赚取
