...
bash .grok/scripts/pr-poll-watch.sh <PR> --interval 120 --initial-wait 60
```

Run in background. Full loop (fix → push → trigger → seed state → poller) is in
[`.grok/PR_PROCEDURES.md`](.grok/PR_PROCEDURES.md). Use `--no-initial-wait` only
when re-arming mid-session after the trigger already has a 👀 reaction.
