...
const PINNED = new Map([
  ...
  ["apps/agent/src/env.ts", [5, "RETRACTION — the RPC-breadth note explaining #687-A removed the watchdog that justified it", "3ad9215c9233"]],
  ...
  ["docs/ops/DeploymentRunbook.md", [46, "LIVE-TEXT — known debt; §\"VPFIBuyAdapter — payment-token mode\" still carries an actionable pre-flight checklist under a Historical banner. #884 LOWERED 47→46: the configure-phase prose no longer lists ConfigureVPFIBuy as a launch child", "31529ef50d8f"]],
  ...
  ["packages/contracts/src/abis/PerkFacet.json", [1, "UNTRIAGED (#1728) — admitted by a widened scope; classify on first movement", "815e05cffa32"]],
  ...
  ["packages/contracts/src/abis/RewardBroadcastFacet.json", [1, "UNTRIAGED (#1728) — admitted by a widened scope; classify on first movement", "815e05cffa32"]],
  ...
  ["packages/contracts/src/abis/RewardIngressFacet.json", [1, "UNTRIAGED (#1728) — admitted by a widened scope; classify on first movement", "815e05cffa32"]],
  ["packages/contracts/src/abis/RewardEpochFacet.json", [1, "TRIAGED (#1566 transport epochs 3b) — the single mention is `VPFIInvalidOriginChainId`, a #687-A error still DECLARED in IVaipakamErrors with zero references anywhere in src/script/test. Every facet inheriting that interface carries it into its ABI, which is why 46 exported ABIs match and why each new facet adds a pin. The fix is deleting the declaration, not pinning its consumers; that is one line plus the pin-count movement it causes across those 46 files, so it belongs in its own change rather than inside a ledger PR. Raise on movement here, and prefer the deletion to a 47th pin.", "815e05cffa32"]],
  ["packages/contracts/src/abis/RewardReconciliationFacet.json", [1, "UNTRIAGED (#1728) — admitted by a widened scope; classify on first movement", "815e05cffa32"]],
