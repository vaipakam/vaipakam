...
- Spec context: [`apps/www/src/content/whitepaper/Whitepaper.en.md`](../../apps/www/src/content/whitepaper/Whitepaper.en.md) §2.1 (Chain Model) describes per-network protocol instances; §3.1 (Diamond Pattern) covers Diamond implementation context, with source-level details anchored by the contracts above.
