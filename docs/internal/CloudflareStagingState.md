...
- https://vaipakam-app.dawn-fire-139e.workers.dev — **also the standing post-deploy review target**, not just a fallback: a deploy reaches this URL before `app.vaipakam.com` does, so the domain can lag or answer for an older version. Review here unless you have confirmed both serve the same `/assets/index-*.js` hash. This exact origin is in the agent's `FRONTEND_ORIGIN` for that reason — do not remove it
...
| `app.vaipakam.com` | *(not recorded — read from the dashboard)* | *(not recorded — read from the dashboard)* | **live** (200), verified 2026-09-07 by serving the exact `/assets/index-*.js` hash a fresh `pnpm run deploy` produced. The ids are deliberately left blank rather than guessed: this binding was made out of band, and no id was captured at the time. Fill them from *Workers → vaipakam-app → Settings → Domains & Routes* at the next audit. `apps/app/wrangler.jsonc` declares no `routes` / `custom_domain`, so — unlike `indexer.vaipakam.com` — a `wrangler deploy` will neither create nor restore this binding |
...
- [x] Apply migrations to `vaipakam-warm`
