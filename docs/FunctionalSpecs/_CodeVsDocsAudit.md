...
| 2026-06-29 | `DefaultedFacet.triggerDefault` / HF-liquidation (`attemptInternalMatchAutoDispatch`) | A sanctioned matcher now SKIPS the auto-dispatch (returns false, non-fatal) so the close-out completes via the external path with no 1% bonus to the flagged wallet; liveness preserved. Verified: `forge test --match-test test_attemptAutoDispatch_sanctionedMatcher_returnsFalse`. | #817 (#815 group A) |
