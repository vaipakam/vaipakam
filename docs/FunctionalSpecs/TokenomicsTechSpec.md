...
- **strictly records-only**: the day's live stamped recycled budget, both outstanding-commitment ledgers, and every claim/remittance consumer keep the previous single-pool figures unchanged — publishing the summed per-chain figure to consumers that still distribute it pro-rata would move armed-day rewards and bucket consumption to the wrong chains, so the resolution rides as records until the next mesh stage flips the consumers and arms the per-chain reservation ledger together
