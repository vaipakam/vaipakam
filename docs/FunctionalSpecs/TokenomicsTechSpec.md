...
- day attribution is **integrity-clamped**: the credit accepted for a chain's day never exceeds the increase in that chain's lifetime total over the attribution baseline, so a reporting bug or replay can never feed the absorption accounting credit that the availability ledger does not back. A report for an earlier day that arrives **after** a later day was already accepted (a re-ordered cross-chain delivery) still receives its exact attribution when the immediately preceding day's report was recorded; when no sound baseline exists for such a late day, its day attribution is conservatively dropped while the availability total still self-heals. Each chain's day is attributed at most once, and a day already finalized rejects late reports entirely (unchanged)
- **backward decodability**: reports produced before this widening remain accepted — their recycled figures simply read as absent and advance nothing. Rollout is **receiver-first**: the canonical chain's transport must accept both report shapes before any mirror begins sending the widened one, so a not-yet-upgraded mirror (or a delayed in-flight report) can never have its day zeroed by a shape mismatch
