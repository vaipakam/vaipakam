...
- **the canonical chain is the exception to that last relation, deliberately.** Releasing a remittance whose message can verifiably never execute restores that day's funding commitments while leaving every value counter reserved — the sent tokens sit in the transport's custody, genuinely outside the platform's, so the balance is NOT re-credited (the delivered-backing section above states this directly). The canonical chain's balance can therefore sit below its own reservations by the full released amount, for as long as those tokens remain stranded. That is the intended conservative state, not a fault: treating it as corruption would amount to re-crediting custody the platform does not hold. An observer of this relation must therefore scope the strict form to receiving chains, and treat a canonical shortfall as something to reconcile against the released remittances before suspecting anything else
