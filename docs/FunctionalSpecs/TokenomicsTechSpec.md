...
- a batch sweep draws the expiring entries' fresh shares against remaining pool capacity **and** the recycle bucket's backing room **per entry**: at most one boundary entry is partially credited and later entries defer, so a batch can never terminalise several entries against one remaining sliver, and it can never revert on the bucket-backing invariant (which would poison the whole permissionless batch); an entry whose fresh share could not be credited at all (fully exhausted, or no backing room) is deferred, never processed with its value silently burned
