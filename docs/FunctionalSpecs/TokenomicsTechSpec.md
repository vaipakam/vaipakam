...
- ~~strictly records-only~~ **superseded by stage B2-b below**: the resolution's records-only posture was transitional; the per-destination broadcast stage makes the stamps live, flips the consumers, and arms the reservation split in one change

Per-destination day broadcast and consumer coupling (recycling completion plan §M3, stage B2-b — live, armed days):

- the day broadcast becomes **per-destination**: on post-cutover days each configured mirror receives its OWN funded figures — its per-side fresh floors, its side-specific recycled global-equivalent numerators, the slice it is instructed to consume from its own bucket, and a reserved keeper-allocation figure (zero until a per-chain keeper resolution exists) — never one shared payload that every mirror would price identically even when a chain's slice was trimmed. The canonical chain is an explicit stamping destination too: it stamps its own per-side funded figures directly at finalization (it receives no broadcast), and never prices its claims from the summed aggregate
...
- **claim pricing flips to the per-chain stamps**: on every chain, a post-cutover day's accumulators price from that chain's own per-side fresh and recycled figures — the day-level aggregate remains a published metric only. A post-cutover day whose per-chain stamp has not arrived halts claims for that day (fail-closed wait, as before)
- **reservation and consumption split by funding source, with two-sided ledger symmetry**: at finalization the canonical chain reserves only the canonical-funded shares (its own slice plus every top-up) on the global outstanding ledger, and books each mirror's locally-funded share as **consumed on that mirror's availability ledger immediately** — the instruction is irrevocable once stamped, and the canonical chain can never observe mirror claim-level consumption, so a deferred conversion could only leak availability. The mirror surrenders the SAME figure from its own bucket exactly once, at broadcast arrival. Consequently a mirror's recycled claim legs never debit its local bucket again (they are funded by the arrival-time surrender plus remitted value), and the skipped amounts remain publicly visible for reconciliation. Under-claim residuals on both chains resolve through the later reconciliation stage — conservative in the meantime: value can idle, but can never be double-funded
- the arrival application is **whole-day idempotent** across every bucket-touching field: a re-delivered packet must match everything the first application wrote (divergence is rejected) and is otherwise a no-op — the bucket surrender can never run twice
- **remittances ship only the canonical-funded share** of a mirror's recycled budget: the mirror-locally-funded share already sits on the mirror, so sending it again would double-fund it; the canonical bucket is debited for exactly what leaves it. The fuller source-scoped netted-remittance machinery remains the next stage
