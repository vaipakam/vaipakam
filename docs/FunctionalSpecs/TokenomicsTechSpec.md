...
Every chain prices its own reward days (recycling completion plan §M3; stage B2-d4; design record §2g):

- **a chain other than the canonical one prices reward claims for post-cutover days from its own funding figures**, exactly as the canonical chain always has. The earlier blanket refusal existed only because such a chain's arriving reward funding was not recorded against its own recycled balance; the relocated-custody behaviour below fixed that, so claims there now have real backing
- **the one genuine wait remains**: a chain cannot price a day whose funding figures have not yet reached it
- **an unbacked payout is prevented by value, not by waiting on an event.** A receiving chain's funding figures arrive before the tokens: the instruction describes the day as funded partly from that chain's own balance and partly by a top-up, and the top-up lands later with the remittance, so in that gap the instruction promises more than the chain holds. What protects the gap is that a claim budgets the recycled portion against the chain's **live** balance and **defers** any day that balance cannot cover — the shortfall never reaches the point where the balance is drawn down. This is long-standing behaviour, not new here
- **deferral is not a stall**: a deferred day becomes payable the moment its funding arrives, and the claim can be retried immediately. Deferral must never be expressed as a *pricing* halt, because a halt stops the day cursor and would strand every later day behind it — including days the canonical chain deliberately never remits (one funded entirely from the receiving chain's own balance, or one whose liability rounds to nothing)
