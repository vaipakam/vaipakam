...
- **the two bullets above describe the pre-mesh funding shape.** An unarmed day is NOT fresh-only: it already carries a recycled portion, sized from the canonical chain's own bucket and remitted from there (Phase A′). **⚠ This statement is currently CONTRADICTED by the code, which returns a zero recycled half for an unarmed day — recorded as an open divergence `D-#1457-01` in `_CodeVsDocsAudit.md`, pending an intent decision. It is deliberately left as written here rather than flipped to match the code.** What ARMING changes is *who funds the recycled portion* — it moves from canonical-only to per-chain self-funding with a canonical top-up. It does not introduce the recycled portion. With that said, a day's budget has **two portions with different funding rules**:
