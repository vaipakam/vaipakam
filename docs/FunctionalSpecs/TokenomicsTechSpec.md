...
- **the two bullets above describe the pre-mesh funding shape.** An unarmed day is NOT fresh-only: it already carries a recycled portion **sized against** the canonical chain's own recycle bucket (Phase A′) — **sized only.** While unarmed the bucket is neither debited nor reserved against, and the day's remittance carries no recycled component at all: the stamp is a record. An earlier revision of this sentence said the portion was "remitted from there", which asserted a funding transfer that no unarmed-day path performs — verified across the day-finalize stamp, `_planDay`, the per-side split and both bucket-debiting call sites. **Read that at the BUDGET layer, which is the layer this bullet is about: the day's finalized record carries a recycled portion sized from the canonical bucket. It does NOT mean an unarmed day PAYS recycled value at claim time — while unarmed, the day's stamp is a record only and the claim math pays the scheduled portion, because reserving commitments with no consume side would collapse the fresh availability the cap depends on. Arming wires reservation and consumption together, which is when the recorded recycled portion becomes payable.** That two-layer split is easy to misread as a contradiction — it was filed as one and refuted (`D-#1457-01`). What ARMING changes about FUNDING: it **introduces recycled funding at all**, and simultaneously sets *who* provides it — per-chain self-funding with a canonical top-up. An earlier revision of this clause said arming changes only who funds, "not whether", which cannot stand beside the paragraph above (#1457 r17): if an unarmed day never debits, reserves or remits recycled value, then there is no unarmed recycled FUNDING to re-source — only a recorded figure. The *record* predates arming; the funding does not. With that said, a day's budget has **two portions with different funding rules**:
  ...
  - the **scheduled portion** — the day's share of the fixed allocation — is funded by the canonical chain for every chain, exactly as before. Local recycled value must never substitute for it, because it is a different pot — recycled custody backs recycled payouts and nothing else. **Intended behaviour that is NOT yet enforced at claim time (#1460):** a claim reduces the recycled ledger by its recycled part and then pays the whole amount out of one fungible balance, so on a chain holding recycled custody whose scheduled portion has not arrived, a scheduled-only claim can be paid from the tokens backing the recycled pool. The consequence is a corrupted book rather than a wrong recipient — the recycled pool then claims more than it holds, and a LATER recycled claim fails instead of the scheduled claim having failed for want of scheduled funding. Activating the recycled programme requires this enforced and asserted across a PAYING claim, not only at the credit chokepoint;
...
- mirror-side `claimInteractionRewards()` draws from the mirror Diamond's local VPFI balance, credited by the remittance above, after the relevant loan has closed; no synthetic IOUs, no cross-chain claim hops. A mirror whose claim gate is open but whose budget has not yet been remitted has claims revert (empty balance) until the operator/keeper funds it — recoverable back-pressure, never lost value. **That "empty balance" reasoning holds only where the balance is genuinely empty (#1460):** a chain that has absorbed recycled value holds a NON-empty balance earmarked for recycled payouts, and an unremitted scheduled claim there is currently paid out of it rather than reverting. The intended behaviour is the revert; until #1460 enforces the earmark at claim time, the observable behaviour on such a chain is a successful claim that leaves the recycled pool over-claiming its own backing.
...
- separation invariant: the protocol's VPFI balance always covers LIF custody plus the unclaimed reward budget plus the recycle bucket — the bucket can never claim value that belongs to users. **Enforced on INFLOW only, and not yet at claim time (#1460):** every credit into the bucket checks the balance behind it, but a payout debits the bucket by its recycled component and then transfers the whole amount from the same pooled balance without checking that the non-recycled part fits outside the bucket. So the invariant is the intent and holds wherever recycled payouts alone draw on the bucket; it is not currently enforced against a scheduled payout on a chain whose scheduled funding has not arrived. Activating the recycled programme requires it enforced and asserted across a PAYING claim, not only at the credit chokepoint
...
- consumption is source-split: the fresh component draws against the allocation's hard cap and retires that day's fresh commitment; the recycled component **debits the recycle bucket at claim/remit time** (never at finalization) and retires the recycled commitment. At fresh-headroom exhaustion the recycled term keeps paying — claims never fail on exhausted fresh headroom while a recycled budget stands. **That guarantee is conditional on the separation this specification records as unenforced (#1460):** the two components debit two ledgers but pay out of one fungible balance, so a scheduled payout on a chain whose scheduled funding has not arrived can spend the custody backing the recycled pool without reducing it. A recycled claim can therefore fail *later* over a recycled budget that still reads as standing — not because the recycled term stopped paying, but because the balance behind it was already spent by a claim that should itself have failed
