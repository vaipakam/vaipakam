...
- the canonical chain keeps a **per-chain recycled ledger** built from those reports: the highest lifetime total accepted from each chain (its recycled **availability**, once netted against whatever the canonical chain later instructs that chain to consume — the consumption side arrives with the later mesh stages and is zero until then), plus a per-day **attribution** series feeding the absorption average's per-day accounting
