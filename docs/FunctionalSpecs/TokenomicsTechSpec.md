...
- the VPFI-LIF payment path is doubly gated: (a) it engages only when governance has set the VPFI fee-discount price peg (`weiPerVpfi`) AND the borrower has consent enabled + sufficient tracked VPFI, and (b) it requires the borrower's effective discount tier to be `> 0` — a tier-0 borrower (for example within the minimum-history window) earns a zero rebate and is routed to the lending-asset fee path, since paying the LIF in VPFI would gain them nothing. Consistent with the near-zero-legal-surface posture (§7 and `docs/DesignsAndPlans/VPFISecuritiesFeatureExcision.md`), the price peg is intended to remain **unset at Phase-1 launch until an organic VPFI secondary market exists** to anchor VPFI's value; while the peg is unset the VPFI-LIF path is inert and every borrower pays the LIF in the lending asset. Treasury buyback (which would create demand pressure on VPFI) likewise remains **dormant** in Phase 1 for the same reason — see §C of the excision plan; it is unfunded and unwired, and turning it on is a deliberate, legally-reviewed operator decision
