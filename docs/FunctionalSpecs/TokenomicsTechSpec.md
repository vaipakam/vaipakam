...
- the bump is honored on the **primary** lender-yield settlement paths (repay, preclose direct close, refinance, auto-lifecycle). The **secondary** paths (swap-to-repay, preclose obligation-transfer / offset, rental-prepay) do not yet honor it and are a **hard blocker on the `feeEntitlementEnabled` cut-over** until they do
