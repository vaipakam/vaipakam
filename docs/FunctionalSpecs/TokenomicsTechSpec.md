...
- **mirror-side rules**: a day has a clock only when an authenticated broadcast delivered one. The mirror rejects a clock-bearing broadcast that names a different Base deployment than the day's recorded one (era binding — a delayed broadcast from a retired deployment must never install its clock into the new era); rejects a re-delivery whose clock facts diverge from the installed ones; rejects a broadcast claiming to carry a clock of zero; and accepts a clock-bearing broadcast for a day whose figures were already applied by the older wire as a **clock backfill** — verifying only the day identity, the destination binding, the deployment era and the immutable global pair, and writing only the clock facts, never re-applying or re-comparing the per-destination figures (which the supported migration sequence may legitimately have changed)
