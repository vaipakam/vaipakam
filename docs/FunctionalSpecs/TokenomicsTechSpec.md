...
Mirror→Base commitment REPORT and the remit gate it lights (recycling completion plan §M3, stages B2-c + B2-d1; the delivered-backing design record §2b documents the retiming):
...
- **remit-ineligible-pending-reconciliation** marks the one case automatic remittance must never touch: an armed day finalized with a chain's interest contribution **zeroed** out of the global denominator (grace-window close or operator force-close over a missing report) — that chain's slice was sized without its real demand. The chain's late commitment report is still accepted and stored for bookkeeping — but it prices at that chain's deliberately-zero funding composition, so the operator sizes the compensation from the mirror's locally readable state (its day totals and entry set). Clearing the flag records the reconciliation; the funding vehicle itself is the delivered-backing stage's evidenced manual-budget path (a zeroed chain has no slice in the finalized denominator, so the ordinary remittance call cannot fund it, and a manual send must reserve and acknowledge like any remittance or it would defeat the delivered-backing ledger) — until that stage lands, zeroed-chain compensation stays the pre-mesh out-of-band governance posture. Historical reports also survive later edits to the expected-chain topology: membership is checked against the day's own finalized evidence, not only the current list. Chains whose interest reported normally are never marked — their reports simply arrive after finalization in the ordinary sequence. The canonical chain is exempt (it is never remitted to), which keeps the whole surface inert on a single-chain deployment
