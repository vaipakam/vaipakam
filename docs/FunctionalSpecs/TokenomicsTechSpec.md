...
- the whole series must be reconstructible by any independent reader from published on-chain state, without privileged access and without trusting a platform-run indexer. Specifically it must be reconstructible from the platform's published NOTICES alone — not only from querying current state — so that a reader who followed the announcements from the beginning arrives at the same history as one who queries today. That is a stronger requirement than it appears: it means every figure in the series has to be announced when it is determined, rather than merely being derivable later by asking, because a reader replaying announcements cannot ask
