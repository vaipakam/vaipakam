...
- the day's stamped recycled budget is the **sum of the funded slices** — a metric; each chain's claimable figure is its own funded stamp, never the aggregate. On a single-chain deployment this equals the previous single-pool sizing exactly
- each chain's per-day funded stamp records: the per-side funded budgets (the binding caps at claim/remittance), the side-specific global-equivalent numerators that make the existing per-side claim arithmetic pay exactly the funded amount on that chain (side weights differ per chain, so one scaled value cannot serve both sides), and the slice the chain will be instructed to consume from its own bucket (the remainder arrives by remittance)
- reservations split by funding source: mirror-locally-funded slices reserve against that chain's availability in a per-chain outstanding ledger; canonical-funded shares (the canonical chain's own slice plus every top-up) reserve in the global outstanding ledger — both at the capped committable amounts, with rounding dust trimmed against the availability actually backing it, so reservations can never exceed what exists and no slice is ever reserved in both ledgers
- the absorption average folds in every mirror's **accepted** day credits alongside the canonical chain's own local series, with the canonical chain counted exactly once
- a chain not folded into the day's finalized denominator receives no slice; days with no emission schedule still stamp zero; pre-cutover days keep the single-pool sizing untouched
- records-only: nothing broadcasts or consumes these stamps until the next mesh stage ships each chain its own figures and consumption instructions
