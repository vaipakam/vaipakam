...
- **a chain's remittance for a day is gated on that chain's own commitment report for that day**, and clamped by the liability that report carries (§9, mirror→Base commitment report). A day is never funded from a partial or absent report — a late report delays funding, it never zeroes it. The one deliberate exception is a day finalized with a chain's contribution zeroed out of the global denominator: that chain's slice was sized without its real demand, so automatic remittance must never touch it and the operator funds it through the evidenced manual path instead
