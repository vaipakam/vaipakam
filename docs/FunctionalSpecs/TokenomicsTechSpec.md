...
- the clock starts only when the sweep first observes the entry claimable, which cannot happen before feature activation — so every entry that existed before activation gets at least one full horizon of runway afterward
- the ratified **90-day activation-notice floor** is additionally explicit: every non-zero configuration of the horizon — first activation, re-activation after a governance dark reset, AND any retune including a shortening — re-grants every entry, however stale its clock stamp, at least 90 days of fresh runway before it can expire. Neither a dark interval nor a horizon shortening can ever be silently counted against dormant claimants
- the protected floor is the **funded 90-day final notice** (see the two-phase rule below), not the main horizon: an entry can be removed only after a contiguous 90-day window, counted solely from moments the reward is provably claim-executable, during which the claimant could actually claim. "Claim-executable" means the amount a claim would really pay (the fresh component capped to remaining pool capacity, plus the recycled component) is non-zero, covered by the chain's local reward funding, and the owner is not sanctioned
- the horizon clock **starts** only from the first moment the reward is claim-executable — a cross-chain remittance outage (or a sanctioned owner) at first-claimability does not start it, so the clock is never anchored before the reward could actually be claimed. Once started, the H-day main horizon (≥180d) is a coarse **wall-clock grace** period before the final-notice clock can even begin; because the funded 90-day final notice is the real protection, an entry blocked for part or all of the main horizon still receives its full funded final notice from the first moment it is both due and claim-executable
- "a claim would actually succeed" also means the owner is **not sanctioned**: a sanctioned owner cannot claim (the claim path screens them), so the horizon clock does not run for their entries and they can never be swept while flagged — a later delisting re-opens the clock, consistent with the freeze-not-seize sanctions posture
- expiry is **two-phase**: once an entry is past its horizon, a first sweep touch that finds it genuinely payable only **arms** a final-notice window (a public last-call signal), and the entry can be removed no earlier than 90 days after that funded arming. Every removal therefore follows a funded final notice — even when a funding outage straddled the horizon instant and nobody observed the outage itself
- the funded final-notice arming is **invalidated** whenever a later touch finds the entry no longer executable (funding lost, or the owner sanctioned) or whenever the horizon is reconfigured (a dark reset or a retune): the next payable touch re-arms and the 90-day final notice re-counts from that point, so a stale arm can never spring an expiry the instant the entry becomes payable again, and a reconfigured horizon re-grants armed entries the same fresh final notice an unarmed entry would get
- a batch sweep draws the expired entries' fresh shares against remaining pool capacity **per entry**: at most one boundary entry is partially credited and later entries defer, so a batch can never terminalise several entries against one remaining sliver
