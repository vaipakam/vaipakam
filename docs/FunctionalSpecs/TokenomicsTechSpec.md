...
- **two operator valves, both evidenced and last-resort**: a reservation whose delivery verifiably landed but whose ack cannot be recovered may be operator-finalized; a reservation whose message can verifiably never execute may be operator-RELEASED — its days re-open for funding, the emission counters and the day's funding commitments are restored (so a re-remittance's retirement pairs off exactly), but the recycled ledger's bucket is deliberately NOT re-credited: the sent tokens sit in the transport's custody, genuinely outside the platform's, and re-crediting would un-back the bucket (physical recovery re-enters through the recycled economy's custody-relocation class once the tokens actually return, never as a blind ledger write). The normal recovery for a stuck message remains re-execution — the reservation just stays in flight meanwhile; if a released message executes after all, the late ack is loudly surfaced (the mirror was double-funded) rather than silently absorbed. A released day whose funding included recycled tokens does NOT become fundable again until the recycled ledger actually has the backing: every remittance-planning surface applies a running recycled-backing check, and an under-backed day simply waits — open, not closed — so a re-remittance can never label fresh or user custody as recycled funding. Receipts are likewise bound to the canonical DEPLOYMENT that sent them — not merely its chain: the mirror records the authenticated sender at delivery, the acknowledgement echoes it, and the canonical side accepts only acknowledgements that name itself (identifiers are per-deployment and restart across a rotation, so a stale-era receipt could otherwise finalize an unrelated same-numbered reservation — even after a same-chain redeployment). A delivery from a rotated deployment supersedes a stale same-numbered receipt, so the new reservation's acknowledgement is never blocked by pre-rotation state
