...
Commitment-gate plumbing for ShareOfPool finalization (recycling completion plan §M3, stage B2-c — the Base-side gate; the mirror→Base commitment report that fills it lands in the delivered-backing stage):

- the finalization-readiness gate knows about mirror reward-headroom **commitments**: on a post-cutover (armed) day the fast full-coverage close additionally waits for every expected MIRROR chain's commitments to be **complete**, so a mirror can be funded only once its day-`D` commitments are known — never from a partial or absent set. The canonical chain is EXEMPT (it is never remitted to), which also keeps the gate inert on a single-chain deployment. The grace window stays the backstop that still closes a stuck day
- **any** finalize of an armed day without a MIRROR's complete commitments — the ordinary **grace-window** close as well as an operator **force-close** — marks that mirror **remit-ineligible-pending-reconciliation**: its ShareOfPool remittance is blocked (never sized from a partial set), so after such a close its quotes return zero and the remittance path skips that chain-day until an operator reconciles the true headroom off-chain (evidenced) and clears the flag. Only the fast full-coverage close (which requires completeness) leaves a mirror eligible
- **scope boundary — this stage lands the GATE plumbing only; the report moves to the delivered-backing stage.** The mirror→Base commitment report that marks a mirror *complete* is designed once, correctly, in that next stage, alongside the mirror consumption and the delivery-acknowledged remitted clamp its inputs are coupled to. The report captures a mirror's **day-`D` claimable liabilities**, which are bounded by the per-day share cap (the reward cap being a canonical-chain feature) — not a live snapshot of the mirror's active-loan list, which is the wrong set (it omits closed-but-still-claimable loans and includes loans opened after the day) and drifts across message boundaries. Until that report lands, the gate here is DORMANT: nothing sets a mirror complete, so it is inert on the current single-chain testnet and fail-safe on any armed multi-chain day (the fast close waits; a force-finalize marks every included mirror remit-ineligible until reconciled)
