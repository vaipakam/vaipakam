...
- on the canonical chain the quote lands as **evidence, never funding**: manual compensation for a quoted day is bounded **per side** by the standing quote (an aggregate bound would admit overfunding one side while shorting the other); an unquoted day cannot be manually funded at all. The standing quote is **bound to the sending deployment's identity**, stamped into the wire by the messaging layer itself: a re-delivery from the same deployment refreshes it while the day is unfunded, a divergent deployment is refused, and an operator clear releases a stale binding after a mirror redeployment — a delayed wire from a retired deployment can never overwrite newer evidence or spuriously clear a day's funding anchor. Once funded, the quote that stood at dispatch is the receipt-bound obligation — re-quotes and clears are both refused
