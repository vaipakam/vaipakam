...
- the reserve and that balance are published **together or not at all**. Where the live balance cannot be read, the counter-derived reserve is withheld too, and the reason is stated. Publishing the half that is always computable would leave a reader with a confident-looking figure and no way to check it — which is the failure the pairing exists to prevent, so falling back to it is worse than publishing nothing. Reading live state also gives that one figure a way to fail that the rest of the surface does not have; the failure is contained to it, and the counter-derived series stays readable
