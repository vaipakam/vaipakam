...
- horizon time only counts — and expiry only arms or processes — while a claim would actually succeed at that moment: the bar is the amount a claim would really pay (the fresh component capped to remaining pool capacity, plus the recycled component), and it must be non-zero and covered by the chain's local reward funding. During a cross-chain remittance outage the clock does not start and a due expiry cannot advance, so a funding delay never consumes a claimant's window
