...
- **a chain must be able to SETTLE before the canonical chain starts instructing it.** Everything above only restores capacity if the receiving chain's settlement paths — paying a claim, forfeiting one, letting one lapse past the horizon — can actually run there. A chain that cannot yet price its rewards for a coupled day still records what it was instructed to fund, but can settle none of it: its settlement totals stay at zero, and the canonical chain's picture of its spare capacity is left **permanently lower, by the accumulating stock of unsettled instructions, than it would otherwise be** — while that chain's balance sits untouched. Note the defect is a SHORTFALL, not necessarily a falling number: only coupled days that actually instruct that chain to fund something locally move the figure at all, and a chain that keeps absorbing can raise its reported total faster than the instructions subtract from it. The observable that always holds is unsettled instructions accumulating while settlement stays at zero. The behaviour is recoverable — the totals are cumulative, so settlements after the block lifts close the backlog — but for the whole window the platform funds from the canonical chain what the receiving chain could have funded itself, which is the waste this whole mechanism exists to remove. **Switching the coupling on is therefore gated on receiving-chain settlement being reachable, not only on the mesh being complete** — and that switch is one-way, so the gate is checked before it is thrown, never after
