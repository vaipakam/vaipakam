...
- "claim-executable" means: the amount a claim would really pay (the fresh component capped to remaining pool capacity, plus the recycled component) is non-zero, is covered by the chain's local reward funding, and the owner is not sanctioned. Any touch that finds the entry non-executable — unfunded, zero-payable, or sanctioned — credits nothing and **records the block**, so the interval spanning it is dropped on recovery even when the outage was shorter than the gap bound; unobserved outages are dropped too (they exceed the bound). Either way, blocked time can never be counted against the claimant
- the countdown the claim surface exposes accounts for the interval a sweep at the current block would itself credit, so it never reports a removal later than the contract would actually enforce; a horizon reconfiguration's re-notice and the final-notice signal are both timed from the true crossing, not the sweep timestamp, so the free notice pipeline can never schedule a last-call that lands after the entry is already reapable
- this is a genuine **soundness guarantee**, not best-effort: because credit requires observation at both ends with a bounded gap, no *unobserved* funding outage or sanction can let the clock run past time the claimant could not actually claim. The cost is a keeper **heartbeat**: an entry only ever expires if keepers observed it claim-executable throughout, with no gap over the bound; if they don't, the entry simply never reaps (a safe failure mode that errs toward not-reaping)
