...
- the **compensation quote originates on the zeroed mirror** as authenticated evidence: a permissionless, batched accumulation walks the day's own reward entries and prices each at the **counterfactual fair share** — the per-side delta the chain would have priced at had its report been included in the day's finalization, computed entirely from finalization-frozen data (the day's funded pool half, the finalized global that excluded this chain, and the mirror's own folded side total) with the day's per-entry ceilings applied unchanged. Accumulation completeness is proven by a conservation identity against the mirror's folded side demand before the quote may dispatch; the dispatched quote is deterministic from frozen inputs, so a re-send carries identical figures (the lost-message retry), and accumulation closes at dispatch
