...
- separation invariant: the protocol's VPFI balance always covers LIF custody plus the unclaimed reward budget plus the recycle bucket — the bucket can never claim value that belongs to users. **Enforced on both INFLOW and at claim time (#1460):** every credit into the bucket checks the balance behind it, and a payout is refused unless its non-recycled component fits in the balance outside the bucket — so the recycled ledger can never be left claiming tokens that have already left. One known imprecision remains and is stated rather than hidden: per-loan borrower-LIF custody shares the same balance and has no running total to subtract, so the claim-time headroom is an upper bound on genuinely free tokens rather than an exact one
