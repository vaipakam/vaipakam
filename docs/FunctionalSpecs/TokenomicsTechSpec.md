...
- **the keeper drives the loop but can neither forge nor distort it**: an automation pass scans the canonical chain's dense reservation sequence from a persisted frontier, and for each in-flight reservation whose delivery has landed sends the mirror's ack (rate-limited per reservation — acknowledgements take minutes to deliver and the finalize is idempotent, so retries are safe but bounded); a second rediscovery pass re-surfaces operator-reconciled old days to the mirror reporting flow even when they fall outside its bounded scan window
