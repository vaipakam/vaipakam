...
- "claim-executable" means: the amount a claim would really pay (the fresh component capped to remaining pool capacity, plus the recycled component) is non-zero, is covered by the chain's local reward funding, and the owner is not sanctioned. Any touch that finds the entry non-executable — unfunded, zero-payable, or sanctioned — credits nothing, and the interval spanning that outage is dropped (it exceeds the gap bound), so blocked time can never be counted against the claimant
