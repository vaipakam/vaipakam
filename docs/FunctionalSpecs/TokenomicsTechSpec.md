...
- the VPFI-LIF payment path is doubly gated: (a) it engages only when governance has set the VPFI fee-discount price peg (`vpfiDiscountWeiPerVpfi`, historically `1 VPFI = 0.001 ETH`) AND the borrower has consent enabled + sufficient tracked VPFI, and (b) it requires the borrower's effective discount tier to be `> 0` — a tier-0 borrower (for example within the minimum-history window) earns a zero rebate and is routed to the lending-asset fee path, since paying the LIF in VPFI would gain them nothing. Consistent with the near-zero-legal-surface posture (§7 and `docs/DesignsAndPlans/VPFISecuritiesFeatureExcision.md`), the conservative retail posture is that this peg **should remain unset at Phase-1 launch until an organic VPFI secondary market exists** to anchor VPFI's value (while the peg is unset the VPFI-LIF path is inert and every borrower pays the LIF in the lending asset). NOTE: the current deploy configure phase (`ConfigureVPFIBuy` / `setVPFIDiscountRate`, invoked by `DiamondConfigSpell`) DOES set the peg, so realizing this posture requires skipping or adjusting that step at Phase-1 launch — reconciling the runbook with this posture is a tracked follow-up. Treasury buyback (which would create demand pressure on VPFI) likewise remains **dormant** in Phase 1 for the same reason — see §C of the excision plan: the cross-chain pipe may be wired by `ConfigureCcip` (messenger + `setBuybackRemittanceReceiver`), but the valve stays closed because no buyback allowed-token, funded budget, or committed intent is ever configured. Turning it on is a deliberate, legally-reviewed operator decision
