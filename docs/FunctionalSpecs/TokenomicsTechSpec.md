...
- it is measured against payouts made **since that chain switched into the coordinated mode**, not against its lifetime payouts. Payouts made before the switch were drawn from the ordinary schedule and were funded by no delivery; charging them would defer every subsequent day until new funding covered history, which halts a chain with any prior activity. Fixing the baseline at the switch keeps the figure correct without assuming the switch happened at launch.
- **nothing enforces it yet.** Secondary chains do not price coordinated-mode days at all while the pricing block described in §M7 stands, so there is nothing for the bound to limit. When it is enforced, a shortfall must **postpone** the day, never pay a reduced amount: the lifetime cap only ever falls, so trimming against it is safe, whereas a delivered budget grows with each arrival and trimming would permanently underpay a day whose funding was still in transit.
