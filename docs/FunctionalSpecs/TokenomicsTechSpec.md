...
- each MIRROR chain computes, for every armed day `D`, its per-side **day-`D` claimable liability** — the sum over its participating users of that user's day-`D` reward demand, individually clamped by the per-day share cap net of anything already paid — and reports the two per-side totals to the canonical chain. The figures are recomputed by the mirror from its own records for every unit a keeper feeds it, so the keeper can delay a report but never distort one; the report is a compact per-side aggregate, never a paged snapshot of a live loan list (the wrong set — it omits closed-but-still-claimable loans and drifts across message boundaries)
- **completeness is proven by demand conservation**: the reported units must exactly exhaust the day's per-side interest totals. A missing user keeps the two sums unequal and the day incomplete — a partial set can therefore only DELAY the report, never understate it. Double-reporting a user is structurally impossible (each user is accepted at most once per day and side, in ascending order), submissions are restricted to the operator's keeper so a third party cannot deliberately wedge a day with a partial submission, and an operator valve can wipe a day-side's accumulation for full resubmission while the report has not yet been dispatched
- **report timing**: a day's liability prices from the per-side caps and the day's funding composition, which the canonical chain computes at that day's finalization and then broadcasts — so the report always ARRIVES after the day is finalized. Finalization readiness therefore has NO commitment input (a readiness wait on the report would deadlock every armed day into the grace backstop); what waits for the report is the chain-day's **ShareOfPool remittance** (the delivered-backing stage's gate + clamp): remittance for `(D, chain)` proceeds only once that chain's day-`D` report is complete — never from a partial or absent set, a late report delays but never zeroes. A mirror may not dispatch a report before its own day's totals are final (the day's funding broadcast is the precondition), so a quiet-looking not-yet-closed day can never ship an irreversible zero report, and each day's report is dispatched exactly once
- **remit-ineligible-pending-reconciliation** marks the one case automatic remittance must never touch: an armed day finalized with a chain's interest contribution **zeroed** out of the global denominator (grace-window close or operator force-close over a missing report) — that chain's slice was sized without its real demand. The chain's late commitment report is still accepted and stored, giving the operator the exact liability figure to size a manual, evidenced remittance; the operator then clears the flag. Chains whose interest reported normally are never marked — their reports simply arrive after finalization in the ordinary sequence. The canonical chain is exempt (it is never remitted to), which keeps the whole surface inert on a single-chain deployment
