...
- **the two bullets above describe an UNARMED day, where every chain's budget is funded from the canonical chain's fresh pool.** Once recycling is armed (completion plan §M3), a day's budget has **two portions with different funding rules**, and only one of them changes:

  - the **scheduled portion** — the day's share of the fixed allocation — is funded by the canonical chain for every chain, exactly as before. Local recycled value never substitutes for it, because it is a different pot;
