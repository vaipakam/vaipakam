...
Every chain prices its own reward days, once its funding has ARRIVED (recycling completion plan §M3; stage B2-d4; design record §2g):

- **a chain other than the canonical one now prices reward claims for post-cutover days from its own funding figures.** The earlier blanket refusal existed only because such a chain's arriving reward funding was not recorded against its own recycled balance; the relocated-custody stage below fixed that, so claims there now have genuine backing
- **but a receiving chain waits for a day's FUNDING to have arrived, not merely its funding INSTRUCTION.** Those are two separate messages. The instruction lands first and describes the day as funded by a mix of that chain's own balance plus a top-up from the canonical chain — yet at that moment only its own share is present; the top-up and the entire freshly-issued portion travel later, with the tokens. Paying in that gap would not fail loudly: the claim path bounds the freshly-issued side against its lifetime ceiling but applies no equivalent bound to the recycled side (on the canonical chain that side is guaranteed by construction, its figures being sized against its own balance — a guarantee that does not transfer), and drawing down a recycled balance floors at zero rather than refusing. The result would be a silent shortfall: the balance bottoming out, the lifetime paid-out figure over-counting, and the payout drawn from custody the platform holds for unrelated purposes
- **the wait cannot strand a payable day.** The canonical chain includes a day in a remittance whenever it funds anything at all, and every payable day carries a freshly-issued component that it funds — so a day whose funding never arrives is a day with nothing to pay on that chain
- **the canonical chain never waits**, since nothing is remitted to it; gating it would block its own days entirely
