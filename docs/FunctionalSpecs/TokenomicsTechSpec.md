...
- **re-dispatch from the position is uncharged and position-bounded**: a replacement manual or supplemental compensation may name the recovery position as its funding source — the lifetime cap is not charged, the draw is bounded by the position balance, a lifetime re-dispatched cumulative records it, and the reservation is marked as recovery-funded so no later path (release included) can restore headroom that was never consumed. Every other bound — quote, era, clock, gate — applies identically to both funding sources
