...
- a batch sweep draws the expiring entries' fresh shares against remaining pool capacity **per entry**: at most one boundary entry is partially credited and later entries defer, so a batch can never terminalise several entries against one remaining sliver; an entry whose fresh share could not be credited at all (fresh budget fully exhausted) is deferred, never processed with its value silently burned
