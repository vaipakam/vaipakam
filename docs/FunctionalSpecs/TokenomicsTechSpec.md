...
- **a chain other than the canonical one does not price reward claims for post-cutover days.** The pause exists because such a chain's reward funding arrives by remittance. Its original stated cause — arriving reward funding not being recorded against the chain's own recycled balance — has since been fixed (see the relocated-custody behaviour below), and the recycled side has a second protection besides: a claim budgets the recycled portion against the chain's **live** balance and defers any day it cannot cover. Two further causes remain, and BOTH must be resolved before the pause can lift:
  - **the freshly-issued side has no delivered-funding limit on a receiving chain.** That side is funded entirely by the canonical chain and travels with the remittance, but a claim limits it only against the programme's global lifetime ceiling less that chain's own past payouts — not against what has been received. Resuming would let a chain pay before its funding arrived, drawing on tokens the platform holds for unrelated obligations
