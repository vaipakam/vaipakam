...
- horizon time only counts — and expiry only processes — while the chain could actually pay the entry's claim: on a chain whose local reward funding is interrupted (a cross-chain remittance outage), the clock does not start and a due expiry is deferred until funding is back, so a funding delay never consumes a claimant's window
