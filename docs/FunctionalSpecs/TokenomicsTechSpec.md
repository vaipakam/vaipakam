...
- the countdown the claim surface exposes accounts for the interval a sweep at the current block would itself credit, so it never reports a removal later than the contract would actually enforce; but it folds that pending interval in only while the entry is currently claim-executable — mirroring the sweep's own gate and its zero-credit-fresh defer exactly (owner not sanctioned, a non-zero post-cap payable, local balance covers it, and the fresh share is genuinely creditable against the pool cap and the recycle-bucket backing room) — so it also never shows a false-imminent removal a sweep would in fact defer, and an already-claimed or already-expired (processed) entry carries no countdown at all. A horizon reconfiguration's re-notice and the final-notice signal are both timed from the true crossing, not the sweep timestamp, so the free notice pipeline can never schedule a last-call that lands after the entry is already reapable
