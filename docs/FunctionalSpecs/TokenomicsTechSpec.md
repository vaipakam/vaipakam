...
- ~~strictly records-only~~ **superseded by stage B2-b below**: the per-destination broadcast stage makes the CANONICAL stamps live (the canonical chain prices its own claims + remittances from them); the MIRROR-side consumption of those stamps is deferred to the delivered-backing stage (see the scope boundary below)
...
- **scope boundary — mirror consumption is DEFERRED to the delivered-backing stage.** Turning on a mirror consuming its own recycled bucket to fund its slice — and the two-sided netting that books that consumption against the canonical ledger — is only safe once each mirror's ACTUALLY DELIVERED recycled backing is tracked (its own surrendered slice plus received cross-chain remittances). Enabling it earlier would let a mirror pay recycled rewards from value still in transit (cannibalising other reward ledgers) and report availability it does not physically hold (letting the canonical chain fund future days from phantom backing). So in this stage the canonical chain funds the **whole** mesh budget: every mirror's local availability counts as zero, the whole commit is canonical-funded and reserves into the single global ledger (numerically identical to the pre-mesh single-pool sizing), mirrors are funded entirely by remittance, and no mirror is instructed to consume its bucket (the consume-instruction field rides the wire as zero, ready for the next stage). The mirror broadcast ingress is **store-only**: it records the stamp + cap family + arming day and never debits the local bucket
