...
- the bump is honored on the **primary** lender-yield settlement paths (terminal repay, preclose direct close, refinance, auto-lifecycle held by the recorded lender). The **secondary** paths — swap-to-repay, preclose obligation-transfer / offset, rental-prepay, **partial repay**, **periodic-interest**, and the **auto-lifecycle transferred-position** case (current holder ≠ recorded lender) — do not yet honor it, and are a **hard blocker on the `feeEntitlementEnabled` cut-over** until resolved
- **a loan extended in place is repriced for the new term.** The `+10%` bump is delivered **per term** — the current term's interest is settled (with the Full stamp intact) at the extension boundary before the loan rolls onto its new term. Because an extension pays **no fresh `C*` tariff** for the added term, the lender Full stamp is **downgraded** on extension so no unpriced `+10%` survives on the added term, and the loan-side reward-cap base is reset for the new term (its reward budget is `0` absent a fresh tariff). The lender's ordinary consent-gated hold discount is unaffected. The **borrower's** Full custody is left intact — a borrower's `C*` is rebated once at terminal over the whole loan lifetime, a whole-loan mechanism that legitimately spans an extension
