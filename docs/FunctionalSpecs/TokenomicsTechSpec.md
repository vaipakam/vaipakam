...
- **the two bullets above describe an UNARMED day, where every chain's budget is funded from the canonical chain's fresh pool.** Once recycling is armed (completion plan §M3), a day's funding is resolved in **two passes**: each chain first funds its own slice from the recycled value it has itself absorbed and not yet spent, and only the shortfall that remains draws on the canonical chain. A mesh where every chain absorbs roughly what it claims therefore settles almost entirely without a cross-chain send — the canonical chain becomes the top-up of last resort rather than the funder of first resort. The per-chain availability this resolves against, and the ledger that tracks it, are specified in §9; §4a fixes only the observable consequence, which is that a chain's remittance is its slice *net of what it could fund itself*
...
- **the `69,000,000` cap bounds FRESH issuance only.** Value recycled back into the reward bucket is re-spent, not newly issued, so it is outside the cap by construction — a day whose fresh headroom is exhausted can still fund from the recycle bucket, and must, or recycling would end the moment the allocation was fully committed rather than fully paid. The fresh headroom is measured against issuance already **committed** as well as issuance already paid out: a finalized day that has committed fresh value has spent that headroom even though no tokens have moved, so no later day can size against the same room twice
