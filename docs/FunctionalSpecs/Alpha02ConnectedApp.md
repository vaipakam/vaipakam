...
- The translation covers the whole displayed interface, not only the
  ...
  per-item labels, and every review-receipt line and hint switch with
  the chosen language once a locale's bundle is filled in. Text that
  embeds a live value mid-sentence — a day count, an amount, a position
  number — is translatable too: the count picks the language's correct
  plural form and numbers format for the locale. Only signing-critical
...
- **An acceptance is honoured in every open tab, not only the one that paid.**
  ...
  every other tab holding the same wallet on the same network applies the same
  correction: its prompt closes and its writes proceed without asking again.
  The correction is bounded by the same short window in every tab, expiring
  simultaneously, so an acceptance undone by a chain reorganisation is not
  papered over anywhere for longer than in the tab that made it. A tab that
  has meanwhile seen a newer terms version keeps prompting for the newer
  version — an acceptance of older text never opens the gate on newer text.
