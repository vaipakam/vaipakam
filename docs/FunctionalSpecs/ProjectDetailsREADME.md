...
- **Liquid Asset Criteria:** For Phase 1, an ERC-20 token is considered "Liquid" on a given network only if:
  ...
  2.  It has sufficient on-chain DEX liquidity on the active network through at least one configured v3-style concentrated-liquidity AMM factory for `asset/WETH`. The deploy-time factory set may include Uniswap V3, V3-fork DEX V3, and V3-fork DEX V3, and the check treats those venues with OR logic: one sufficiently deep venue is enough.
...
- **Oracle Usage:**
  ...
  - **ETH as Reference Asset:** ETH is the protocol's quote and reference asset for liquidity classification. In practice the DEX sees WETH, so the protocol checks `asset/WETH` pools while using the configured Chainlink `ETH/<numeraire>` feed to convert that depth into the active numeraire.
...
- **Worker D1 Migrations:** Worker deployments should run idempotent D1 migrations as part of testnet and mainnet deploy flows so offer / loan / activity endpoints cannot fail because tables were not created. Already-applied migrations should be skipped by the migration tool rather than treated as errors.
