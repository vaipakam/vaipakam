...
- **Worker Environment Preservation:** Each Worker that relies on operator-tuned environment values managed outside its checked-in configuration should declare, in that configuration, that such values survive a deployment.
  ...
  - Automated checks should verify that every Worker holding operator-managed values declares this preservation. That verification should read configurations **structurally**, discover them rather than trust a list, and cover **every configuration that names such a Worker** — not only the one in that Worker's own directory — since a deployment can be pointed at a different configuration file, in which case the canonical one's declaration is not what is loaded.
  ...
  - What the declaration does not cover should be **stated where the checking lives** rather than left to be inferred: a deployment that explicitly overrides the declaration, a configuration that does not exist in the tree at checking time, and a Worker whose configuration declares no operator-managed values while values exist for it outside the configuration.
