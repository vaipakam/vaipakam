...
- **Worker Environment Preservation:** Each Worker that relies on operator-tuned environment values managed outside its checked-in configuration should declare, in that configuration, that such values survive a deployment.
  ...
  - What the declaration does not cover should be **stated where the checking lives** rather than left to be inferred: a deployment that explicitly overrides the declaration on the command line, a configuration that does not exist in the tree at checking time because it is generated — which a command-level check could refuse by declining to trust an unreadable selection, so removing one is a real reduction in coverage and not merely an unchanged limitation — and a configuration checked in under a name that does not follow the deployment tool's own convention. Recognising that last one would mean classifying arbitrary files by their contents, which is the failure mode that produces false reports on correct work.
