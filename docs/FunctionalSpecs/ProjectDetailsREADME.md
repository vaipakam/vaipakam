...
- **Cloudflare Surface Separation:** Public hosted infrastructure should keep read-only, read/index, write/act, and ingestion duties separated. A representative staging split is `labs.vaipakam.com` for marketing/docs with no chain credentials, `defi.vaipakam.com` for the app surface, `agent.vaipakam.com` for read/index APIs and diagnostics endpoints, `keeper.vaipakam.com` for autonomous write actions with signing and aggregator credentials, and `indexer.vaipakam.com` for chain-to-archive ingestion with database write credentials. Shared archive storage such as D1 should be accessed with least-privilege credentials per Worker.
