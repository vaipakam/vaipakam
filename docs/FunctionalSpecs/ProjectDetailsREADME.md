...
- **LayerZero Owner Separation:** `ConfigureLZConfig` and OApp ownership transfers should run under the documented OApp owner key (`PRIVATE_KEY`, `VPFI_OWNER_PRIVATE_KEY`, or `REWARD_OWNER_PRIVATE_KEY` as applicable), not the Diamond admin key by assumption. Handover scripts should read the current on-chain `owner()` first and skip with operator guidance when the signer does not own that OApp.
