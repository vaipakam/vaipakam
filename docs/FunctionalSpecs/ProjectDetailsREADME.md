...
- **Security Considerations:**
  ...
  - **Stored Configuration Is Data, Not Instructions:** A configuration file must be read as a set of name-and-value settings and must never be executed by the tool that reads it. Executing it makes every guarantee above unenforceable, because a file that runs code can rewrite the command line itself, disable the checks meant to catch it, or replace the machinery used to apply the operator's choices — and it can hand execution onward by setting a name some later program treats as a startup hook. Reading order therefore also matters: settings are read before the operator's choices, so the choices are applied last and win. A setting the reader does not recognise stops the run and names it, rather than being skipped, because a skipped setting is one the operator believes is in effect. The recognised set is the platform's documented settings; an unrecognised name failing loudly is preferred to an unknown name being honoured silently.
