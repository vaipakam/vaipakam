...
- **Worker Environment Preservation:** Each Worker that relies on operator-tuned environment values managed outside its checked-in configuration should declare, in that configuration, that such values survive a deployment.
  ...
  - Automated checks should verify that every Worker holding operator-managed values declares this preservation. That verification should read each Worker's configuration **structurally**, and should discover the configurations rather than trust a list of them, so a Worker added later is examined rather than silently exempt.
  - A **secondary check that judges individual deployment commands is deliberately NOT required**, and an implementation should not carry one. Deciding whether a piece of text will run a deployment, and against which configuration, requires the execution model of every system that might run it — a continuous-integration system's, a document format's grammar, a build tool's variable language, a shell's quoting rules, a command interpreter's name resolution. Where a check would need that, the correct answer is a **declaration from the deployment itself** rather than a better approximation in the check. This was stated here as the conclusion of three withdrawn transformations before the check was removed; the record of what each attempt cost, and of the wrong verdicts the check produced, is in [`DeployGuardRewriteScanRecord.md`](../DesignsAndPlans/DeployGuardRewriteScanRecord.md), which is a record and binds nothing.
  - Because the declaration makes every spelling of a deployment safe at once, the platform should not be understood as trading coverage for simplicity here. What it does give up should nonetheless be **stated where the surviving check lives** rather than left to be inferred: a deployment that explicitly overrides the declaration, a configuration outside the places the check looks, and a Worker whose configuration declares no operator-managed values at all while values exist for it outside the configuration.
