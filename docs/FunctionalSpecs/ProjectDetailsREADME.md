...
- **Peer Rotation Requires a Drain:** Because a message carries the originator that sent it, a message already in flight from a previous peer will be rejected once a new peer is installed, and cannot be recovered afterwards. Rotating a channel's peer should therefore be an operational procedure with a drain step: stop the old peer sending, let in-flight messages reach finality or be abandoned deliberately, then clear and reassign. Introducing or changing the message format should likewise be treated as a lockstep upgrade across the chains in a lane, since messengers on different formats cannot interpret each other's messages.
