...
- **Process:**
  ...
  3.  A new Lender (Lender B) accepts Alice's tagged Borrower Offer. When the contract path supports atomic accept-and-refinance, the acceptance creates the replacement loan and immediately chains into the old-loan payoff in the same transaction; otherwise the borrower or authorized keeper must complete the standalone refinance transaction before treating the rollover as done.
