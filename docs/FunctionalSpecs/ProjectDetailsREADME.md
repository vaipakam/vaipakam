...
- **VPFI Received From Protocol-Fee Flows:** VPFI received through protocol-fee utility paths is recycled through a **governance-configurable** treasury-conversion path — an ordered target list carrying per-target allocation basis points, plus protocol-wide (not per-target) conversion-eligibility thresholds (a minimum USD value and a single shared max-interval timer) — not a hard-coded split. It also requires the Diamond to be the treasury; in external-treasury deployments the on-chain conversion path is unavailable. The **recommended launch configuration** is:
