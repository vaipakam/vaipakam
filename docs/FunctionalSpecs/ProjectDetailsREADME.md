...
- **Holding A Service Off Its Data Is Done By Removing The Capability:** Where the platform must stop every writer to an off-chain database — moving one, for example — it does so by depriving the service of the ability to reach any database at all, not by listing the places in the service that write. A list of writers is never provably complete: it cannot cover scheduled work that traverses no route, self-rescheduling background work, work already in flight, or an entry point added later. Removing the capability covers all of them without naming any, and is therefore the only form of this guarantee the platform will claim.
