...
- **Partial Liquidation Sizing ("as much as needed"):** A routine partial must not over-liquidate the borrower. After the slice, the resulting Health Factor must not exceed a governance-set target ceiling (default health factor 1.20); a partial that leaves the borrower comfortably above the ceiling — meaning it sold more than needed — is rejected so the keeper picks a smaller fraction. The ceiling is waived in exactly two cases so solvency and dust-cleanup are never blocked: (1) **deep underwater** — the position was already at or below a governance-set health threshold (default 0.95) before the partial, so aggressive delevering is permitted; and (2) **pre-existing dust** — the position was already tiny at entry (its debt or collateral valued below a governance-set dust floor, default ~$1,000), so it isn't blocked from clearing; this is judged on the position's size *before* the partial, never the leftover after it, so a keeper cannot manufacture a tiny leftover by over-selling and bypass the guard (a larger position that cannot be partialled cleanly falls back to full liquidation instead). This governs only how much collateral is sold; it never changes how the loan is priced, and full liquidation remains available unchanged as the alternative.
