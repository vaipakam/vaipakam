...
- **Partial Liquidation:** For active, in-term loans that are only mildly underwater, a keeper may liquidate a bounded fraction of collateral instead of closing the whole position. The proceeds are applied interest-first and then principal, the loan remains `Active`, and the interest clock restarts on the reduced principal from the partial-liquidation timestamp. A partial **must not accelerate the borrower's exposure to the default or late-fee windows, nor clip the lender's coupon window** — both the loan's maturity and its grace period are anchored to the originally agreed term and are preserved exactly across one or many partials. Concretely: the loan's maturity date is held to the second (the interest-clock restart is arranged so it lands back on the original end date, never earlier and without drifting later across repeated partials), and the grace window that follows maturity is sized from the loan's original committed duration, so a partial that reduces the live remaining term can never collapse the grace bucket and bring the default deadline forward. The call must strictly improve Health Factor and restore it to at least `1.0`; otherwise it reverts and the keeper must choose a larger fraction or fall back to full liquidation.
