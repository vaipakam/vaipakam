...
- **Partial Liquidation:** For active, in-term loans that are only mildly underwater, a keeper may liquidate a bounded fraction of collateral instead of closing the whole position. The proceeds are applied interest-first and then principal, the loan remains `Active`, and the interest clock restarts on the reduced principal from the partial-liquidation timestamp. The remaining term must be preserved such that the loan's maturity is **never pulled earlier** than the originally agreed end date — a partial may not accelerate the borrower's exposure to the default or late-fee windows, nor clip the lender's coupon window. Because the remaining term is tracked in whole days, a sub-day remainder at the moment of a partial is rounded **up**: the re-stamped maturity equals the original maturity rounded up to the next whole day, so it holds or extends by at most one day but never shortens, and repeated partials stay monotonic (the maturity only ever holds or grows). The call must strictly improve Health Factor and restore it to at least `1.0`; otherwise it reverts and the keeper must choose a larger fraction or fall back to full liquidation.
