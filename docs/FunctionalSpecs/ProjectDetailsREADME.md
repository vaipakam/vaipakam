...
The sale vehicle posts **no fresh collateral**: the exiting lender is transferring an existing position, and the collateral already securing the underlying live loan continues to back it after the sale — so the exiting lender is not required to lock new capital to list. The protocol must therefore accept a sale-vehicle offer that carries zero collateral, and must not reject it under the ordinary borrower-offer rule that caps borrowing against posted collateral (that ceiling does not apply to a protocol-authored position sale, whose real collateral lives on the linked loan, not on the offer). The exiting lender is the offer's creator, so the sale proceeds and the right to cancel the listing bind to that seller — even when the listing is submitted on their behalf by an authorised keeper. Because a buyer's acceptance auto-completes the sale in the same transaction, that completion runs as an internal step that does not re-enter the acceptance's reentrancy guard. To keep the settlement identity unambiguous, the exiting lender's position is consolidated to its current holder at listing time — the stored lender-of-record and any held-for-lender balance are re-anchored to whoever actually owns the position NFT — so the party who lists, the party the buyer pays, the party accrued-interest / shortfall is charged against, and the vault physically holding the proceeds are all the same address, even for a position transferred on the secondary market before listing. A position carrying unresolved held VPFI cannot be unified this way and is refused at listing until it is cleared. Once listed, the sale offer is **immutable** (its amount, rate, and collateral are pinned to the live loan until it is accepted or cancelled) and can be taken only through the direct acceptance path, never the range/partial matcher — a position sale is an all-or-nothing transfer. Because the listed price is pinned to the loan's principal at listing time and the offer cannot be re-priced, a partial repayment of the underlying loan while the listing is live would leave the listed price larger than the position it now represents; accepting a listing whose pinned price no longer equals the live principal is therefore **refused**, so a buyer can never overpay for a shrunk position — the seller re-lists at the current principal. While a listing is live the same position cannot also be sold through the **direct instant-exit** path (the lender-swap-in sale): that path is refused for a loan that already carries a live listing, so a position can never be handed to two buyers at once; the seller cancels the listing first. The position's **collateral is likewise protected while a listing is live**: a borrower-initiated collateral withdrawal on the underlying loan is refused for as long as the listing stands (the seller cancels the listing to change collateral), because the buyer's acceptance binds only the loan and its principal, not the loan's live collateral. Because some collateral reductions are not borrower-initiated — a periodic-interest auto-liquidation may sell collateral to cover a shortfall — the collateral in force at listing is additionally **snapshotted**, and accepting a listing whose live collateral has fallen below that snapshot is **refused**; between the withdrawal block and this snapshot check, a buyer can never pay the listed price for a position whose collateral was reduced after listing. A listing's **own borrower cannot buy it**: if the linked loan's borrower accepted the vehicle the lender position would migrate onto them, leaving an active loan whose lender and borrower are the same party (a party owing itself), so that acceptance is refused — the borrower exits by repaying or preclosing. The incoming buyer is also **re-screened against the loan's continuing borrower** at acceptance, the same counterparty compliance check the direct instant-exit sale already applies (a no-op on the permissionless retail deploy; consistent behaviour for the gated industrial variant). Accepting a listing is a **secondary-market transfer, not a new origination**, so it is **not** charged a fresh loan-initiation fee — that fee was already paid when the underlying loan was first taken — and the seller receives the full sale price; the read-only accept preview mirrors this and quotes no origination fee for a sale acceptance. To keep off-chain matching honest, the read-only match preview reports a sale-vehicle offer as **non-matchable**, mirroring the on-chain matcher's refusal, so a matching bot never quotes (or wastes a transaction submitting) a pair that would always revert. In Phase 1 this flow is available for loans with **ERC-20 collateral** only; a loan whose collateral is an NFT cannot be listed for lender-sale yet, because the sale vehicle escrows no collateral of its own and the accept / complete / cancel paths would otherwise attempt to move an NFT that was never held.
...
- **Surplus Rule:** If the Insurance / Bug Bounty pool grows above `2%` of total VPFI supply, the excess VPFI should be recycled through the same configured conversion path. This is a treasury-strengthening conversion, not a token burn.
...
- **Proposed Allocation:**
  ...
  - **Reserve (pending reallocation):** includes the former `1%` fixed-rate-sale allocation and the former `24%` staking-yield allocation; final disposition is a governance decision
