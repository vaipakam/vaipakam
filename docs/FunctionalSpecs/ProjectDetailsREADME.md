...
- **Reconciling Is A Three-Way Question, And Only One Answer Is Automatic:** With a record of what the copy carried, each record admits three facts — whether the copy carried it, whether the original still holds it, and whether the destination holds it. Exactly one combination may be acted on without a person: a record that appeared on the original after the copy and has never existed on the destination. The platform treats every other divergence as a decision and reports it rather than resolving it, because each of the others can be made worse by guessing — two databases running independently can issue the same identifier to different records, a record the destination has since deleted is indistinguishable from one that never arrived unless the copy's record says otherwise (and a deletion may be an obligation rather than housekeeping), and a record deleted on the original after the copy appears in none of its records at all, so anything working through them never encounters it while the destination quietly keeps something that should be gone.
