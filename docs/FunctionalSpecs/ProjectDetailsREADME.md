...
- **Auto-Lifecycle Consent and Keeper Actions:**
  ...
  - Auto-lend is a per-user surface for passive lending, expressed through the standing **lender-intent** layer rather than fixed-duration offer posting. A lender configures an intent on a `(lending asset, collateral asset)` pair with their own bounds — maximum exposure, minimum fill size, minimum rate floor, maximum initial LTV, and maximum loan term — funds working capital, and records the auto-lend consent marker. The protocol keeper then fills matching borrower demand within those bounds and, when the lender delegates the dedicated auto-roll authority, redeploys fully-repaid intent loans straight back into the lender's capital. Enabling is an ordered, resumable sequence — register the intent, delegate the keeper (auto-roll, plus signed-fill when the intent is kept keeper-gated), record consent, then fund capital last — so capital is never committed to custody before a fillable, properly delegated intent exists. The dapp must reflect both governing kill switches honestly: the auto-lend consent switch gates the consent step, and the intent-fill switch governs whether keeper fills may execute yet (an intent may be registered and funded while filling is paused, and begins filling automatically once governance re-enables it). Auto-lend remains opt-in, default-off, and is not guaranteed lending execution; lenders can pause (clearing consent while leaving the intent and capital in place) or withdraw their un-lent capital and cancel the intent at any time.
