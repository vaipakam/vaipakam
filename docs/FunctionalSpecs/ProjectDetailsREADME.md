...
- **Partial Liquidation:** For active, in-term loans that are only mildly underwater, a keeper may liquidate a bounded fraction of collateral instead of closing the whole position. The proceeds are applied interest-first and then principal, the loan remains `Active`, and the interest clock restarts on the reduced principal from the partial-liquidation timestamp. A partial **must not accelerate the borrower's exposure to the default or late-fee windows, nor clip the lender's coupon window** — both the loan's maturity and its grace period are anchored to the originally agreed term. Concretely: the loan's maturity is **never moved earlier** than the originally agreed end date (when a partial lands part-way through a day, the whole-day remaining term is rounded up, so the maturity holds or moves at most about a day later — borrower-favourable — never earlier, and never compounding earlier across repeated partials), and the grace window that follows maturity is sized from the loan's **original committed duration**, so a partial that reduces the live remaining term can never collapse the grace bucket and bring the default deadline forward. The same original-duration grace anchor governs every recovery path (direct repay, swap-to-repay, collateral listing), so a partially-liquidated borrower keeps the full grace window on all of them. The call must strictly improve Health Factor and restore it to at least `1.0`; otherwise it reverts and the keeper must choose a larger fraction or fall back to full liquidation.
