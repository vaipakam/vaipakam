...
- **Worker Environment Preservation:** Each Worker that relies on operator-tuned environment values managed outside its checked-in configuration should declare, in that configuration, that such values survive a deployment.
  ...
  - Where a runner's resolution behaviour cannot be determined, an implementation should **establish it rather than assume it**, and should record which direction it chose and why; assuming the platform the rules were written for is the present behaviour and is the defect.
