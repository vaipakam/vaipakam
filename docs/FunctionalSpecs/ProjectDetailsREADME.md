...
- **Auto-Lifecycle Consent and Keeper Actions:**
  ...
  - **Collateral carry-over (refinance-tagged offers).** A refinance is "same debt, same collateral, better lender/rate," so a refinance-tagged offer must carry the existing collateral over **in place** rather than make the borrower pledge a second batch. The collateral never leaves the borrower's vault; the protocol's locked-balance ledger simply **retags** the lien from the old loan to the new loan (same owner, asset, and amount — the locked-balance total is unchanged across the refinance). The borrower no longer momentarily locks 2× collateral. This holds even when the borrower position was transferred after the original loan: the collateral stays in the original borrower's custody vault while the new borrower position goes to the refinancer, and every close path returns it from the right vault to the rightful holder. The post-refinance health-factor and LTV checks run against the carried collateral. Changing the collateral as part of a refinance is out of scope (the collateral identity is pinned to the old loan's); adjusting collateral uses the dedicated add/remove-collateral flow. (Principal-side netting stays wallet-pull — see the note above — only the collateral side carries over.)
...
- **Internal-Match Incentive and Surplus Handling:** The internal matcher may receive a bounded per-leg incentive from each matched notional amount. The incentive must be lower than the typical external liquidation cost surface and must be range-bounded by governance. Any residual borrower collateral after a partial internal match remains claimable through the ordinary borrower claim path. Likewise, a **full** internal-match close of an over-collateralized loan (where the match consumes less collateral than the borrower pledged) must leave the over-collateralization residual **protected** — its collateral protection retained (not torn down and freed) — and recorded as claimable through the ordinary borrower claim path, owed to the current holder of the borrower position. The residual must not be freely withdrawable by the stored loan record's original borrower once the position has been transferred away (the same transferred-position drain protection that governs proper closes — see §2207). This applies to both the active-loan full-close and the fallback-rescue full-close. Retrieving this residual is a **borrower-side** action: it closes the borrower position but does not by itself settle the loan, which remains in the internally-matched terminal state with the lender side pending. The lender side of an internal match — routing the lender's matched proceeds (and any amount held back from a pre-empted offset) to the **current** holder of the lender position, which matters once that position has been transferred, and closing the lender position record — is a distinct concern that must not be short-circuited by the borrower's residual claim; settling the loan as part of that claim would strand the lender's held funds. As a staging constraint, a fallback-pending loan that has received an extra collateral top-up while awaiting cure is **ineligible** for internal matching altogether. Such a loan's collateral is split — the original sits in protocol custody (moved there at fallback) while the top-up sits in the borrower's own vault — and internal-match settlement always draws the moved collateral from protocol custody, so any match shape (full, partial, or exactly-collateralized) would mis-account the vault-held top-up. The protocol must therefore exclude these loans **up front, before any funds move**, from every mechanism that would draw their whole collateral from protocol custody — both internal matching and the speculative collateral re-swap that a fallback claim would otherwise attempt (which would likewise consume the vault-held top-up the protocol doesn't hold). A directly requested match is declined; the automatic keeper/claim-time matcher skips the loan; it is filtered out of candidate scans so it is never offered to other loans as a match target; and its fallback claim resolves straight through the safe in-kind payout, which only touches the protocol-held portion and leaves the vault top-up untouched. Recovery never stalls and the loan stays fully recoverable through every path. This holds until the accounting that reconciles a vault-held top-up against the custody draw is in place (with the lender-side lifecycle work); excluding up front is strictly safer than the prior behaviour, which freed the residual to the original borrower outright.
