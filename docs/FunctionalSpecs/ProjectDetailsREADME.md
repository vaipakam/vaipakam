...
- **Deployment Artifact Sync:** Per-chain `contracts/deployments/<chain>/addresses.json` files are the source of truth for deployed contract addresses. After a redeploy, operators should run `contracts/script/exportFrontendDeployments.sh` to merge those artifacts into committed typed deployment JSON for both the frontend (`frontend/src/contracts/deployments.json`) and the watcher (`ops/hf-watcher/src/deployments.json`), with provenance stamps alongside each consumer. Frontend `.env.local` should keep only operator-specific values such as RPC URLs, WalletConnect IDs, feature flags, and tuning knobs; deployed Diamond / facet / adapter addresses should not be hand-copied into local env files or Worker vars.
- **Cloudflare Surface Separation:** Public hosted infrastructure should keep read-only, read/index, write/act, and ingestion duties separated. The current split is `vaipakam.com` for the marketing / docs surface with no chain credentials, `defi.vaipakam.com` for the connected app surface, `agent.vaipakam.com` for read/index APIs and diagnostics endpoints, `keeper.vaipakam.com` for autonomous write actions with signing and aggregator credentials, and `indexer.vaipakam.com` for chain-to-archive ingestion with database write credentials. Shared archive storage such as the `vaipakam-archive` D1 database should be accessed with least-privilege credentials per Worker, and Worker bindings should grant only the read or write capability each surface needs.
...
- **Worker ABI Sync:** Watcher / worker ABIs must be generated from the same compiled contract artifacts as the frontend and keeper-bot ABIs. Hand-typed positional tuples for structs such as `Offer` or `Loan` are not acceptable because a single inserted field can silently corrupt decoded worker rows. Deploy scripts should run the watcher ABI export alongside other ABI exports, and the worker bundle should carry a provenance stamp.
...
- **Fresh-Deploy Indexer Seed:** Fresh deploys should seed the worker indexer cursor at the active chain's safe head after deployment metadata is exported, so the first cron tick starts from the intended baseline rather than backfilling retired history.
