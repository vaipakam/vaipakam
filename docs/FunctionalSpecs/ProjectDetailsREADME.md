...
- **Reward-Budget Lane Readiness:** A mirror chain is not interaction-reward ready until its reward-budget receiver is deployed, registered, guardian-pausable on the receive path, rate-limited under the configured lane capacity, funded for cross-chain fees, and authorized for the production keeper or documented manual operator path. The largest expected single-day mirror slice should fit inside the provisioned lane capacity before automation is enabled.
