...
`getProtocolConfigBundle()` is the source for governance-mutable values such as fees, slippage settings, rental buffer, staking APR, VPFI discount tiers, Range Orders master flags, and matcher-fee BPS. `getProtocolConstants()` is the source for compile-time constants such as `MIN_HEALTH_FACTOR`, VPFI pool caps, and the maximum interaction-reward claim window.
