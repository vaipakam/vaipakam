...
Ending a listing without a sale — whether it expired on its own or the seller cancelled it — starts a **one-day quiet period** before the same loan can be listed again. This is the borrower's action window: while a listing stands, the borrower's partial-repayment and collateral-withdrawal options are held, so a seller must not be able to chain listings back-to-back and keep the borrower's options frozen indefinitely. During the quiet period the borrower can freely exercise every held option; after it lapses the lender may list again.
