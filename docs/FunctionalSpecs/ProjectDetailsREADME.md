...
- **Security Considerations:**
  ...
  - **A Setting Must Not Be Able To Start A Program:** Some names are treated by other programs as instructions to run something at start-up. A configuration reader must refuse those rather than pass them on, because the processes a deployment starts carry its credentials. The set of such names is open-ended, so this is stated as an obligation to refuse the known ones and to keep the list current — not as a claim that any implementation of it is complete.
