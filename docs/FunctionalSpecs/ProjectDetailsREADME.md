...
- **CCIP Lane Hardening:** Messenger configuration should allow only approved source chains, remote messengers, channel peers, and token pools. Chain-selector and channel-handler mappings must remain one-to-one, and conflicting operator assignments must be rejected rather than overwritten. The reject-rather-than-overwrite rule applies to the channel-peer assignment as much as to the routing ones, and it matters most there: a mis-set chain selector or handler prevents delivery, which announces itself, whereas a mis-set peer delivers normally and only mis-states the originator to the receiving contract. Changing a peer that is already configured should therefore require clearing it first, so the change is two deliberate operator actions and is distinguishable in the event history from a first-time assignment; re-stating a peer's existing value should remain permitted and inert, so that reasserting a known configuration is safe.
