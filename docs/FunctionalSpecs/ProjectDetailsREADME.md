...
- **Worker Environment Preservation:** Each Worker that relies on operator-tuned environment values managed outside its checked-in configuration should declare, in that configuration, that such values survive a deployment. Preservation is a property of the Worker, not of the command used to deploy it: any ordinary deployment of that Worker should leave operator-managed values intact, whatever route or wording is used to trigger it, and whether the deployment publishes immediately or stages a version. This matters because those values govern live behaviour — liquidation thresholds and confidence windows for the keeper, recipient-token validation and marketplace pagination for the agent — so losing them silently reverts risk behaviour to defaults at the moment it starts mattering. Deleting such a value should therefore be a deliberate operator action rather than a side effect of shipping code. Automated checks should verify that every Worker holding operator-managed values declares this preservation, and a secondary check may additionally reject individual deployment commands that would not preserve them, for Workers where the declaration is absent. Where such a check reads a configuration file to decide either question, it should not trust the checked-in copy of a configuration that the deploying file rewrites before the deployment runs, since that copy is then not the file the deployment loads; a configuration whose contents cannot be relied upon should leave the command to be judged on its own terms rather than be treated as preserving. Deciding that a file rewrites a configuration should rest on the file actually performing a write: text that merely matches or shares a name with one — a pattern being matched, a comparison between two values, a locally defined function named after a copying operation, a quoted example in a language where a quoted value is inert — is not a rewrite. A shell is the exception and should be read as one: there a quoted string handed to a command may well be executed by it, so such a check should treat quoted shell text as executable unless it is plainly being stored, and accept that a script quoting an example of a write to its own configuration will be reported. Whether text is shell should be decided by whether something runs it rather than by the kind of file holding it: a command handed to an interpreter from inside a program written in another language is still a command, so shell spellings of a write should be looked for there too, while the same characters standing as that program's own syntax should be read in that program's language. That rule runs both ways: a spelling of a write excluded in one language because it means something else there should still be recognised inside text that another language executes. Where what the file runs differs from what it contains, the rewrite question should be asked of a TRANSFORMED copy of the file rather than of a selected part of it: a build file's recipe variables expanded, because they are expanded before the shell sees them and a variable holding a redirection is a write. The distinction between transforming and selecting is load-bearing. A transformation cannot omit anything, so a construct the check fails to understand still reaches it; a selection of "the parts that can run" turns every construct it fails to recognise into a silent pass, which is the failure this check exists to prevent. Only transformations that ADD text belong here. Adding text wrongly can invent a write and cost a report; removing it wrongly deletes a real write and passes silently, so a subtraction — blanking a document's prose so that a sentence describing a write stops reporting the deployment below it — should not be attempted at all, however conservatively its rule is stated. The reason is not that such a rule is hard to get right but that it cannot exist: this check treats a bare, unindented line in a document as an actionable command, which is why a runbook's copy command is reported, and a prose sentence naming a write has that same shape, so separating them is the very judgement whose answer produced the unwanted report. A document should therefore be read exactly as written, and the resulting report on prose accepted until it is addressed by a means that cannot go silent. Transformations should also preserve line count, so a position means the same thing before and after and no coordinate translation is needed. Where a build file's variables are modelled, ONE model should serve both the question of where deployments are and the question of where writes are — including which lines count as recipes at all — since the two describe the same text the build tool hands the shell, and a correction applied to one copy alone leaves the halves disagreeing. That model should NOT attempt to reproduce the build tool's semantics. Asking what value the tool gives a name requires interpreting its language, and pursuing that produced a long run of corrections in which each was more faithful than the last and each introduced a fresh defect, until every finding was an edge of the previous fix — the signature of a rule with no end. The model should instead ask the one question about a name it can decide by reading: DOES THE FILE GIVE IT EXACTLY ONE ANSWER? A name assigned more than once, assigned anywhere inside a conditional, or named by a removal directive is AMBIGUOUS; nothing else needs deciding, and in particular which value would win never does. Two narrow exceptions are lexical rather than semantic and belong: an escaped currency symbol does not begin a variable reference, and the tool's canonical alternative default filenames are build files. Following a settable recipe marker does NOT belong, and was withdrawn after being tried: resolved over the whole file it applies a late declaration retroactively and drops an earlier recipe, converting a stated miss into a silent one, and resolving it correctly means tracking that state per line, which is the interpretation this rule exists to avoid. Where a name is ambiguous the two consumers should resolve it by what each would otherwise lose, which is why one shared model must expose the ambiguity rather than resolve it: the rewrite question substitutes only unambiguous names, since it can then never invent a rewrite and what it declines was already missed, while the deployment scan substitutes regardless, since declining there would lose deployments it finds today. Different policies over one shared model is the correct resolution; two models is not. A miss the check knowingly keeps should be recorded and pinned by a test that asserts the miss, so that a later correct fix announces itself rather than passing unnoticed. An executable helper carrying no file extension should be classified by the interpreter its opening line names, for every interpreter such a check knows, since a file read in the wrong language misreads what is data and what is code in both directions. Because the check runs inside routine validation and a report blocks work that is correct, it may otherwise miss an exotic spelling of a write, which the preservation declaration already covers, rather than report text that performs no write at all. That restraint is about what the text IS, not about what it would do when run: a command that genuinely copies or replaces a file should be treated as writing even where its options or the state of the target might mean it does not, because deciding otherwise means modelling how each command's options override one another, and such tables do not stay correct. A command told not to overwrite is therefore reported, and the consequence is only that the configuration's contents go unread.
