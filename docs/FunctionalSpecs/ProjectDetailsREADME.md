...
The **floor** is the least the seller will receive out of the buyer's payment once the settlement forfeiture is deducted. It is not the figure shown on their screen: the forfeited interest grows for as long as the listing stands, so a floor set at the displayed value would make the seller's own listing unfillable within minutes. The enforceable floor is the worst case they are accepting — the same settlement arithmetic evaluated at the moment the listing expires — which is a true statement to show them ("fill any time before this runs out and you receive at least this much") and a bound the platform can check. It is computable only because a listing must carry a finite expiry.
