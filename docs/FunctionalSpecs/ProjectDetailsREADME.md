...
The **direct sale** — the one-transaction route that spends a standing buy offer rather than resting a listing — carries the same optional bound, because the two sale routes must let a seller bind their economics identically or the same position could be sold on different terms depending on which route it left by. Here there is no listing window; the drift the bound guards is the interval between the seller reading a quote and their own transaction mining, in which a settlement can move the live state the sale recomputes from. The bound submission carries the seller's reviewed minimum net receipt, maximum cost, and an optional deadline, and the sale is refused when execution would be worse for the seller — a net below the floor, a cost above the ceiling, or a fill past the deadline — and passes when it is at least as good, each refusal naming its condition. The bound is checked against the very figures the settlement uses, so it cannot diverge from what the seller actually receives. The original unbound direct sale is unchanged and remains available.
