...
- **Worker Environment Preservation:** Each Worker that relies on operator-tuned environment values managed outside its checked-in configuration should declare, in that configuration, that such values survive a deployment.
  ...
  - Automated checks should verify that every Worker holding operator-managed values declares this preservation. A secondary check may additionally reject individual deployment commands that would not preserve them, for Workers where the declaration is absent.
  - Where such a check reads a configuration file to decide either question, it should not trust the checked-in copy of a configuration that the deploying file **rewrites** before the deployment runs, since that copy is then not the file the deployment loads. A configuration whose contents cannot be relied upon should leave the command to be judged on its own terms rather than be treated as preserving.
  - Deciding that a file rewrites a configuration should rest on the file **actually performing a write**. Text that merely matches or shares a name with one — a pattern being matched, a comparison between two values, a locally defined function named after a copying operation, a quoted example in a language where a quoted value is inert — is not a rewrite. A shell is the exception: there a quoted string handed to a command may well be executed by it, so quoted shell text should be treated as executable unless it is plainly being stored, accepting that a script quoting an example of a write to its own configuration will be reported.
  - Whether text is shell should be decided by **whether something runs it**, not by the kind of file holding it. A command handed to an interpreter from inside a program written in another language is still a command; the same characters standing as that program's own syntax should be read in that program's language. The rule runs both ways: a spelling of a write excluded in one language because it means something else there should still be recognised inside text that another language executes.
  - An executable helper carrying no file extension should be classified by the interpreter its opening line names, for every interpreter the check knows, since a file read in the wrong language misreads what is data and what is code in both directions.
  - A check of this kind should admit a transformation of the text only where it is a **semantics-preserving normalisation of a known notation** — a total, deterministic rewriting of one spelling of a command into another — and never where it requires inferring which text is executable or what a name stands for. Whether characters are added or removed is not the test.
  - A rule keyed on the **interpreter** should not be applied as though it established something about the **runner** or the **language**, and these are three different things. Whether two spellings of a command name denote one program depends on how the runner resolves command names, not on its platform family. Whether two spellings of a variable name denote one variable depends on the shell language, and holds on every platform that language runs on. A correction that gates either on the platform would be wrong, and in at least one direction would convert a false report into a silent pass.
  - Where a runner's resolution behaviour is determinable, an implementation should **establish it rather than assume it**; assuming the platform the rules were written for is the present behaviour and is the defect. Where it is **not** determinable — an unresolvable or matrix-selected runner — the check should not guess in either direction, because reporting risks naming a command the runner does not have and staying silent risks the unsafe deployment passing. It should instead stop relying on the normalisation for that deployment and **judge the command on its own terms**, exactly as it does for a configuration whose contents cannot be relied upon: the deployment is then required to be safe whatever it targets. That is the same fallback the rest of this specification already takes when a fact it needs is unavailable, and it resolves the conflict by removing the dependence on the unknown rather than by choosing which way to be wrong.
  - A miss the check knowingly keeps should be **recorded and pinned by a test that asserts it**, so that a later correct fix announces itself rather than passing unnoticed.
  - Because the check runs inside routine validation and a report blocks work that is correct, it should prefer missing an exotic spelling of a write — which the preservation declaration already covers — over reporting text that performs no write at all. That restraint is about what the text **is**, not about what it would do when run: a command that genuinely copies or replaces a file should be treated as writing even where its options or the state of the target might mean it does not, because deciding otherwise means modelling how each command's options override one another, and such tables do not stay correct. A command told not to overwrite is therefore reported, and the consequence is only that the configuration's contents go unread.
  - Where the check would need another system's execution model to answer correctly — a continuous-integration system's execution model, a document format's grammar, a build tool's variable language — the correct answer is a **declaration from the deployment itself** rather than a better approximation in the check. Three transformations that tried the approximation route were built and withdrawn; the record of what each cost is in [`DeployGuardRewriteScanRecord.md`](../DesignsAndPlans/DeployGuardRewriteScanRecord.md), which also carries the catalogue of wrong verdicts the check currently produces and POINTS AT where each one's scope and bounds are established — it does not restate them, deliberately, since a restatement that drifts is worse than a pointer; the bounds live in the fixtures and on the issues, fixtures first because only they run. That document is a record, not a requirement: nothing in it binds, and where it and a fixture disagree about what the check does, the fixture is right.
