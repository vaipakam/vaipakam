...
- For a resting sale listing that check is binding **at the moment the buyer's
  ...
  settled.
- Whether a position is price-discoverable enough to be sold is judged **as of
  the sale**, not as of origination. A loan records each leg's liquidity when it
  is opened, and that record is never refreshed; a market that has since
  degraded would otherwise let a position be sold on the strength of prices the
  protocol no longer accepts, without ever being recognised as unpriceable. Sale
  admission therefore also consults the live liquidity classification — the same
  determination made when a loan is originated, and the same one that governs
  whether fresh collateral may be added to a live loan. A leg counts as
  measurable only when the live determination and the loan's own record agree,
  because the recorded value is what decides whether risk arithmetic is
  available for that loan at all: a position recorded as illiquid has no health
  factor to compare no matter what its market has since done.
- Where a leg is **not** measurable, the position cannot be assessed and the
  sale is **refused**. In Phase 1 this holds unconditionally — it does not
  depend on whether the progressive risk-access regime is switched on.
  - Admitting such a sale would hand an unpriceable — in the worst case
    worthless — position to whoever authored a standing offer, with no
    loan-specific agreement anywhere in the flow.
  - The buyer-consent regime is deliberately **not** treated as a substitute
    here, even when it is enabled. That regime grades an asset by what it *is* —
    its identity and the depth class of its market — whereas measurability is a
    property of the pricing oracle's state at this moment. The two come apart
    precisely where it matters: the platform's reference and quote assets count
    as lowest-risk by identity and require no opt-up or per-pair agreement at
    all, so one of them with a stale price feed is simultaneously unpriceable
    and exempt from every consent step. A regime that never consults liquidity
    cannot express agreement to the risk of an unpriceable position, so relying
    on it would admit exactly the case this rule exists to refuse.
  The refusal is not silent: it states that a specific leg is not priceable and
  names which one. The surface must never show a health figure for a position
  that has none — "this position has no price-based safety check" is the honest
  line, not a blank or a zero — and the accept preview reports this case as a
  plain block rather than as a measured health shortfall.
- Both directions of that rule matter, and neither is redundant. A recorded
  value that has gone stale in the permissive direction is caught by the live
  determination; a position whose record blocks risk arithmetic is refused with
  the unpriceable explanation rather than with an opaque internal failure from
...
- **Permissionless Liquidation / Authorized Keepers / Third-Party Execution Allowed:**
  ...
  - The project is not planning to move Vaipakam position NFTs into protocol vault during borrower preclose, borrower refinance, or lender early-withdrawal flows, and is not planning to natively lock transfers for those flows through `_beforeTokenTransfer` — with an exception for the sale / offset sub-flows that bind a position to a live listing or link. A native transfer lock DOES apply while such a binding is live: a live prepay collateral-sale listing locks the borrower-position NFT; a lender early-withdrawal sale offer (`EarlyWithdrawalFacet.createLoanSaleOffer`) locks the lender-position NFT; and a preclose offset link (`PrecloseFacet`) locks the borrower-position NFT. Each such lock keeps the listing/link bound to the holder's vault and releases on cancel or settlement.
...
- In full-close mode the borrower names an **upper bound** on the collateral
  ...
  for that amount before the borrower submits. The preview answers how much
  would sell, not whether this caller may close right now: it does not
  apply the authority, sanctions or live-intent checks, so it can quote a
  loan the close-out would then refuse. The sized amount is the most the
  protocol ever authorises a route to sell. Because the figure moves with
  accrued interest and price updates, a route quoted for a fixed amount can
  drift from it: a route that sells LESS than the sized amount is accepted
  only if its proceeds still clear the floor set for the sized amount and
  cover the full repayment, and the collateral it did not sell stays pledged
  and claimable; a route that would sell MORE than the sized amount is
  refused, and the next route in the list is tried. An integration should
  therefore quote for the preview figure or slightly under it. An
...
- **Batch Claiming ("Claim All"):** A user with several eligible pull-payouts — resolved-loan proceeds across multiple loans, interaction rewards, vaulted platform-token balances, un-lent lending capital, payroll — should be able to execute them in a **single transaction** instead of one transaction per claim. The batch preserves the caller's identity: each batched action is authorized exactly as if the user had called it directly, so a keeper or any third party cannot claim on another party's behalf through the batch. Batching is **best-effort per item** for the "Claim All" flow: an item that is not currently eligible — a loan not yet claimable, or one finalized by another party between the interface preview and the transaction — is skipped and reported as unsuccessful while the remaining items still execute; the batch never lets one ineligible item forfeit the rest. (A caller may alternatively mark an item must-succeed, so its failure aborts the whole batch.) The batch grants no capability a user does not already have alone, and because the interaction-rewards claim is bounded to a fixed number of finalized days per call, a single batch may not fully drain a long-dormant user's rewards — the interface should surface any residual and offer a re-claim.
...
- **Swap Failover:** The liquidation caller supplies a ranked try-list of swap adapter calls. Production routing may include 0x Settler, 1inch v6, Uniswap V3, and Balancer V2. The Diamond tries routes in the submitted order and falls back to the next route if a venue reverts, returns insufficient output, or becomes stale. A forced close requires at least one **registered** swap adapter AND a non-empty try-list of **enabled** routes: if the deployment has no swap adapter registered at all (an operator misconfiguration), or the caller's try-list would attempt no enabled route, the forced close **reverts** (`NoSwapAdaptersConfigured` / `NoEnabledSwapRoute`) rather than silently routing into the in-kind collateral fallback with zero DEX attempts. The Fallback Claim Model below therefore engages per-loan only after the configured routes are actually attempted and fail — never because routes are simply absent. (Owner decision 2026-07-11, spec-review item L-i: a zero-adapter deployment must fail loud, not fall through; complements the S9 / #1005 empty-try-list hardening.)
...
- **Fail-closed freeze of mid-loan servicing payouts.** The same fail-closed posture must also govern the servicing payments a loan makes to its lender-position holder *inline, during the loan's life* — the daily NFT-rental fee, the periodic-interest auto-liquidation proceeds, and an ERC-20 partial repayment's principal-plus-interest — because these pay the holder immediately rather than through a later claim, so a fail-open screen alone would let a holder confirmed sanctioned at close-out be paid during an oracle outage with no claim gate left to recover the funds. Each such payout must make its pay-or-freeze decision with the same registry-aware, outage-hardened logic as the release gate: a clean or never-confirmed holder is paid inline exactly as before, but a holder who is affirmatively flagged (or previously confirmed flagged, during an outage) has their share diverted into the loan's **stored lender** vault — never a vault minted for the flagged holder — credited to that loan's lender accumulator so the eventual claim folds it in, reserved so the stored lender cannot spend it ahead of the frozen holder's claim, and recorded as the fail-closed frozen claimant; the flagged holder's own wallet receives nothing and the funds release only once that holder is proven de-listed. Discretionary, holder-initiated value actions (for example an early-withdrawal option) keep the hard-block posture and are not parked; only the must-complete Tier-2 servicing payouts are frozen this way.
...
The sale vehicle posts **no fresh collateral**: the exiting lender is transferring an existing position, and the collateral already securing the underlying live loan continues to back it after the sale — so the exiting lender is not required to lock new capital to list. The protocol must therefore accept a sale-vehicle offer that carries zero collateral, and must not reject it under the ordinary borrower-offer rule that caps borrowing against posted collateral (that ceiling does not apply to a protocol-authored position sale, whose real collateral lives on the linked loan, not on the offer). The exiting lender is the offer's creator, so the sale proceeds and the right to cancel the listing bind to that seller — even when the listing is submitted on their behalf by an authorised keeper. Because a buyer's acceptance auto-completes the sale in the same transaction, that completion runs as an internal step that does not re-enter the acceptance's reentrancy guard. To keep the settlement identity unambiguous, the exiting lender's position is consolidated to its current holder at listing time — the stored lender-of-record and any held-for-lender balance are re-anchored to whoever actually owns the position NFT — so the party who lists, the party the buyer pays, the party accrued-interest / shortfall is charged against, and the vault physically holding the proceeds are all the same address, even for a position transferred on the secondary market before listing. A position carrying unresolved held VPFI cannot be unified this way and is refused at listing until it is cleared. Once listed, the seller cannot edit the sale offer's terms (they cannot re-price or re-collateralise it out from under a pending buyer) and it can be taken only through the direct acceptance path, never the range/partial matcher — a position sale is an all-or-nothing transfer. The buyer's signed acceptance binds against the **live loan's immutable or discrete facts**, never a listing-time snapshot: the buyer signs a principal that must equal the loan's *current* principal, a term that must equal the loan's *original fixed* duration (the maturity is fixed at origination; the shrinking "days remaining" is shown live but never signed), and a collateral **floor** that the live collateral must be at least. The position's **behavioural terms** are handled differently, and deliberately so: whether the borrower may repay early in parts, whether they may post a prepayment listing, whether interest settles on a periodic cadence, and which interest model the loan runs under are all **copied onto the listing when the seller lists**, and the buyer's signature binds against the listing's copy. Binding the listing is the right answer here — rather than reading the live position, as the numbers do — because all four are settled when the loan is first taken and never change for as long as it runs, so a listing-time copy is a permanent truth rather than a snapshot that can drift. It also means every screen and every client that shows a buyer the listing's terms is showing the position's real terms, with nothing further to reconcile. What the listing must never do is leave them blank: an absent value in these fields reads as a denial, so a listing that omitted them would tell a buyer the position permits none of these, have that agreement stand, and hand over a position that permits all of them — and these are precisely the terms that decide what the borrower may do to the buyer after the sale. A listing whose behavioural terms **disagree with its position does not describe what it sells**, and a purchase against it ought to be refused rather than completed — the buyer cannot cure such a listing by re-signing, because they signed it faithfully and it is the listing that is wrong; the seller relists instead, and a fresh listing is copied from the position and agrees by construction. That refusal is enforced **at the moment of purchase**, by comparing the listing's behavioural terms against the position's own — the one comparison that can see the fault, since the checks that compare the buyer's agreement against the listing are satisfied by a wrong listing the buyer signed faithfully. The refusal happens before any of the buyer's funds move, and it is reported as a **stale listing** rather than as a disagreement over the buyer's terms: telling a buyer their terms do not match would send them to re-sign the same wrong listing, when what is required is that the seller relist. All four behavioural terms are in scope — whether the borrower may repay early in parts, whether they may post a prepayment listing, which interest model the loan runs under, and whether interest settles on a periodic cadence — and each is compared in its own right, so a listing wrong about only one of them is refused exactly as one wrong about all four. The stale-listing refusal is **the lowest-priority refusal a purchase can meet**, and must be reported only when no other refusal applies: it instructs the seller to relist, and every other ground for turning a purchase down also forbids that relist — a terminal position cannot be relisted at all, a matured one cannot be relisted, a listing whose position fails the sale-solvency floor would fail the same floor on relisting, and a sanctioned seller or a paused asset is refused at listing creation. Answering "the listing is out of date" in any of those states is advice that cannot be taken, and it conceals the reason that can be acted on, so the comparison happens after every refusal that moves no value — after expiry, after the position's status, maturity and solvency, and after the compliance, pause and self-trade gates. The single exception is the rule applied rather than broken: the refusal aimed at a buyer who is the position's **own current borrower** ranks *below* staleness, because that is the one refusal relisting genuinely does cure for everyone else — the seller relists correctly and the new listing is simply still unbuyable by that one party. A listing created since the copying rule agrees with its position by construction and is unaffected, so an ordinary purchase pays nothing for this. Recording the position's payment cadence onto the listing must **not** subject the sale to the eligibility rules that govern *originating* a periodic loan — that a payment interval be shorter than the term, that finer cadences require a minimum principal, that the feature be enabled and both assets liquid. Those decide whether such a loan may be **created**; a sale creates nothing, and the position's cadence was already admitted when the loan was taken. Re-asking the origination question against a listing built from the position's *current* state — days remaining rather than the original term, principal after any partial repayment — would strip the exit from positions that are running perfectly normally: an annually-settling loan would become unlistable the day after it starts, and a partial repayment could strand its lender. Only terms that genuinely describe the **offer** stay offer-scoped in their own right: the seller's asking rate (it is the seller's fixed ask), and whether the offer permits a parallel sale — that one is never recorded onto a position at all, so there is no position-level counterpart for it to mirror. A partial repayment while the listing is live shrinks the live principal, so a buyer who signed the old (larger) principal is **refused** and simply re-signs for the smaller position they are now buying — never overpaying for a shrunk position. While a listing is live the same position cannot also be sold through the **direct instant-exit** path (the lender-swap-in sale): that path is refused for a loan that already carries a live listing, so a position can never be handed to two buyers at once; the seller cancels the listing first. The position's **collateral is protected by the signed floor**: a borrower-initiated collateral withdrawal on the underlying loan is refused for as long as the listing stands (the seller cancels the listing to change collateral), and for the reductions that are *not* borrower-initiated — a periodic-interest auto-liquidation selling collateral to cover a shortfall — the buyer's signed floor catches it: if the live collateral has fallen below what the buyer signed, the acceptance is **refused**. A collateral *top-up* only improves the position, so it clears the floor. There is no listing-time collateral snapshot to store or drift; the floor is enforced structurally at the signature. A listing's **own current borrower cannot buy it**: if the loan's borrower (resolved as whoever currently holds the borrower position NFT, not a stored origination address that may have gone stale on the secondary market) accepted the vehicle, the lender position would migrate onto them, leaving an active loan whose lender and borrower are the same party (a party owing itself), so that acceptance is refused — the borrower exits by repaying or preclosing. The incoming buyer is also **re-screened against the loan's current borrower** at acceptance, the same counterparty compliance check the direct instant-exit sale already applies (a no-op on the permissionless retail deploy; consistent behaviour for the gated industrial variant). If a listed loan reaches a terminal state without a completed sale — the borrower repays it, it defaults, or it is liquidated — the dangling listing can be **torn down permissionlessly**: anyone (the seller, a keeper, the frontend) may trigger a cleanup that unlocks the seller's lender position NFT and marks the stale sale offer cancelled so it drops out of the open book. Until that cleanup runs, no buyer can be harmed, because an acceptance against a non-active loan is already refused. Accepting a listing is a **secondary-market transfer, not a new origination**, so it is **not** charged a fresh loan-initiation fee — that fee was already paid when the underlying loan was first taken — and the seller receives the full sale price. The read-only accept preview mirrors the live-bound reality: it quotes the *live* loan's principal and collateral (not the listing snapshot), quotes no origination fee for a sale acceptance, and surfaces the structural blockers — the linked loan is no longer active, it has passed maturity, it fails the sale-solvency floor, the listing's terms are out of date, or the viewer is the loan's own current borrower — so the UI can disable "Accept" without a wasted transaction. The preview must report **the same first blocker the purchase itself would report**, in the same order, including the ordering rule above: a preview that named a different reason than the transaction would have the card either offer a purchase that then fails, or refuse one for a reason the buyer cannot act on. One blocker sits ahead of every other and is easy to omit because it is not part of the purchase's own sequence of checks at all: while the **whole platform is paused**, no purchase can be attempted, so that is the only thing the preview may report — a listing-, offer- or buyer-specific reason would be doubly misleading there, since relisting is equally unavailable while paused. This platform-wide pause is a different thing from an individual asset being paused, which is checked in its own place within the sequence. To keep off-chain matching honest, both the read-only match preview and the lender-intent fill preview report a sale-vehicle offer as **non-matchable**, mirroring the on-chain matcher's refusal, so neither a matching bot nor an intent solver ever quotes (or wastes a transaction submitting) a pair that would always revert. In Phase 1 this flow is available for loans with **ERC-20 collateral** only; a loan whose collateral is an NFT cannot be listed for lender-sale yet, because the sale vehicle escrows no collateral of its own and the accept / complete / cancel paths would otherwise attempt to move an NFT that was never held.

A lender holding an active position must be able to **see what their options are without going looking for them**, and in whichever view they are using — the ways out of a position are not an advanced topic, and a lender who never switches views must not be left unaware that an early exit exists at all. What that surface promotes is the inverse of the borrower's equivalent: a borrower carrying a debt benefits from being shown ways to act, whereas a lender's position is already the thing that pays, so the option that forfeits nothing — leaving it alone — is stated **first**, before any exit. That statement is conditional and never a promise of repayment (it says what follows if the borrower repays and what follows if they do not), and it describes **when** the lender is paid from the loan's own terms rather than assuming: on a periodically-settling loan the lender is paid during the term, and on a loan that permits partial repayment each part the borrower repays reaches the lender when it is paid, so a single end-of-term description would misstate the one fact the statement exists to convey. While those terms are not yet known the surface says so rather than choosing a shape. Each exit is shown with its cost **before** the lender opens the tool behind it, and the listing exit additionally discloses what it does to the *counterparty* — while a listing stands it also holds the borrower's discretionary paths on that loan — because that consequence is cross-party and a seller has no reason to expect it. An exit that is currently unavailable **explains why and names what remains open**, rather than being hidden: a missing option reads as a nonexistent one, and a lender told nothing will go looking for a tool that would only refuse them. Two qualifications follow from that. A cost stated for an exit the lender has *already committed to* — a listing of theirs that is standing, which a buyer may complete at any moment — remains stated, because those are pending consequences of a sale in flight rather than the price of a choice still open; only an exit merely declined stops carrying its price. The test is whether the sale can still **complete**, not merely whether the position is still held: once a listing has ended without selling, no buyer can complete it, and its costs stop being pending consequences even though the position remains held until it is cancelled or cleaned up. And an explanation must never name a remedy the surface cannot actually offer: where an exit is blocked by the lender's own standing listing but this session cannot act on that listing, the explanation says where it can be acted on instead of naming a control that will not be there. Where several reasons apply at once, the most structural is the one stated, so a lender hears what they cannot change before what they can — and once the loan has reached its due date, every exit reports that the loan is now resolved by repayment or the default process rather than naming a narrower obstacle, since no new sale can be created at that point regardless. This surface never itself commits anything: it explains and it navigates, and every gate is still enforced by the tool that performs the action.
...
The **direct sale** — the one-transaction route that spends a standing buy offer rather than resting a listing — carries the same optional bound, because the two sale routes must let a seller bind their economics identically or the same position could be sold on different terms depending on which route it left by. Here there is no listing window; the drift the bound guards is the interval between the seller reading a quote and their own transaction mining, in which a settlement can move the live state the sale recomputes from. The bound submission carries the same figures the listed route binds — the seller's reviewed minimum net receipt and a ceiling on the held-for-lender balance that migrates to the buyer — plus a required deadline, and the sale is refused when execution would be worse for the seller: a net below the floor, a migrating held balance above the ceiling (parked interest that grows between quote and fill is forfeited whole to the buyer, so the ceiling is the seller's protection against it), or a fill past the deadline. It passes when execution is at least as good, each refusal naming its condition. The bound is checked against the very figures the settlement uses, so it cannot diverge from what the seller actually receives. The original unbound direct sale is unchanged and remains available.

The deadline is required on the bound route rather than optional, and it caps a third cost the other two bounds cannot: the sale forfeits the seller's pending usage reward measured at the day it settles, a loss that grows the longer the transaction is delayed. A finite deadline bounds that forfeiture to the window the seller chose, mirroring the mandatory finite expiry the listed route carries. The two routes must cap the same three costs — settlement, transferred held balance, and reward forfeiture — or the same position could be sold on materially different terms by choosing a route; a seller who wants none of the three capped uses the unbound sale, where the unbounded reward loss is disclosed rather than consented to.
...
- **Borrower Loan Initiation Fee Discount:**
  ...
  > **SUPERSEDED for new loans — this describes the retired up-front-then-rebate path.** For **every** new loan, no VPFI enters the up-front-fee-and-rebate custody and **there is no rebate to claim later**. How the fee itself is charged depends on the mode: under **HoldOnly** the tier discount is applied **directly to the lending-asset fee at acceptance** and no VPFI moves out of the borrower's vault at all; where the **per-party Full tariff** is enabled, the party's tariff *is* charged in VPFI into protocol custody, but it is credited to the recycled ledger rather than held against the loan for a rebate. So "no VPFI movement" is a HoldOnly statement; "no rebate custody" is the statement that holds for all new loans. Everything below — the up-front full-fee deduction, the custody-for-the-life-of-the-loan holding, the rebate at proper close, and the forfeit-on-default treatment of that held VPFI — applies **only to loans already open on the retired custody path** at the point it was retired, which still settle through their original lifecycle. A platform deployed fresh has no such loans and this block describes nothing that can occur on it. The tier *schedule* below still governs the discount percentages; only the mechanism by which the discount is delivered changed.
...
- **CCIP Lane Hardening:** Messenger configuration should allow only approved source chains, remote messengers, channel peers, and token pools. Every one of those four lane settings should behave the same way when an operator changes it: an assignment that conflicts with a live value is rejected rather than silently overwritten, re-stating a setting's existing value stays permitted and inert so that reasserting a known configuration is safe, and a genuine change is made by clearing the setting and then assigning the new value — two deliberate actions, distinguishable in the event history from a first-time assignment. The one-to-one requirement should hold in both directions for each pairing: a chain and its selector, a channel and its local handler, and a remote address and the channel it is declared to serve. The uniformity matters because the intuition that a misconfiguration announces itself is not reliable — a lane pointed at a wrong but otherwise compatible address delivers messages and tokens successfully, and the peer setting in particular is what an inbound message is authenticated against, so overwriting it re-points the lane's trust anchor while every other observable stays identical.
- **Cross-Chain Originator Authentication:** A cross-chain message should carry the identity of the contract that sent it, and the receiving messenger should verify that identity against the peer configured for the message's channel and source chain, rejecting any mismatch. The originator a receiving contract acts on must be a fact recovered from the message rather than a value read from local configuration; otherwise a configuration mistake is reported to the handler as though it were the truth about who sent the message. A rejected message should fail in a way the transport records as a failed delivery, re-executable once the configuration is corrected, rather than being consumed. The message format should be versioned, and a version the receiver does not recognise should be rejected rather than interpreted — reading authentication data out of an unknown layout is a guess.
- **Originator Authentication Happens Once, at the Messenger:** A receiving domain contract should not hold its own copy of the address it expects a cross-chain message to have come from, and should not re-check the originator the messenger has already verified. Re-checking a verified originator against a second locally stored address does not examine the message at all — it examines whether two pieces of local configuration agree with each other, which is a deployment-consistency assertion presented as an authentication step, and which introduces a way for a lane to stop delivering because two records of one address disagree with no indication of which is stale. A receiving contract should instead check the things the messenger cannot: that its caller is the messenger it is paired with, that the source chain is the one this kind of message is only ever legitimate from, and that the message is not a replay or out-of-order delivery. The source-chain check is defence in depth rather than the sole constraint — the messenger already refuses a message from a chain it has no partner configured for on that conversation, and correct configuration gives a mirror exactly one partner — but it should still be made, because it is what continues to hold if a partner is ever configured for a chain that should not be sending that kind of message at all. Nothing here weakens the rule above: an originator the messenger has already verified is not re-checked, whereas the source chain is a distinct fact that a receiving contract can constrain more tightly than configuration alone does.
- **Rotation Requires a Drain:** Changing either end of a live conversation should be an operational procedure with a drain step — stop the old contract sending, let in-flight messages reach finality or be abandoned deliberately, then make the change. Two distinct reasons make this necessary. A message in flight from a previous partner carries that partner's identity, so it is rejected once a new partner is installed. A message in flight to a previous local handler names only the conversation, so it is delivered to the replacement handler along with anything it carries — nothing rejects it, because it is a valid message for that conversation. The second case cannot be closed by carrying more information on the wire: a sender can attest to its own identity but cannot know which contract the receiving side has appointed. Introducing or changing the message format should likewise be treated as a lockstep upgrade across a lane, on a drained lane, since messengers on different formats cannot interpret each other's messages.
- **Stranded Messages Are Recoverable, and Should Be Described That Way:** Where a message is rejected because configuration moved underneath it, the rejection should be a refusal rather than a destruction: the transport records a failure that can be re-executed once the configuration accepts it again. Partner assignment should carry no expiry or revocation that would prevent restoring a previous value, so an operator who skipped a drain can roll the setting back, re-execute the stranded message, and redo the change properly. Operator documentation should state this explicitly, because describing a recoverable delivery as lost invites an operator to abandon funds that are retrievable. Recoverability is not a substitute for the drain — the recovery requires pointing a live lane's trust anchor backwards for its duration.
- **Adding an Invariant to Deployed Configuration Requires a Migration:** Where a new uniqueness rule is enforced through an index that is derived from configuration rather than supplied with it, upgrading an already-configured deployment should populate that index from existing configuration as part of the upgrade itself, not as a later step — until it is populated, the rule is not in force. Because a deployment cannot enumerate its own configuration, the set of entries to migrate must be supplied by the operator and derived from the deployment's own recorded history; an omitted entry stays unprotected, and the migration should reject an entry that does not correspond to live configuration rather than skipping it.
...
- **An Offered Channel Must Be A Channel That Can Send:** A delivery rail is offered to users only while the platform can actually issue on it. A rail that is configured but structurally unable to send — for example because its client library and the signing library it is given disagree about how to sign — is an outage, not a staged feature, and must be reported as one: the rail stops being counted as a delivery that happened, stops consuming any per-run delivery allowance it never spent, and must not suppress the retry of a rail that does work. Whether a rail can send is decided by asking whether the signer exposes what the client will use, not by interpreting the text of a failure after the fact. Where a rail is deliberately staged, the surface says the channel is unavailable rather than accepting an opt-in it cannot honour.
...
- **Cloudflare Surface Separation:** Public hosted infrastructure should keep read-only, read/index, write/act, and ingestion duties separated. The intended split is `vaipakam.com` for the marketing / docs surface with no chain credentials, `app.vaipakam.com` for the connected app surface, `agent.vaipakam.com` for read/index APIs and diagnostics endpoints, `keeper.vaipakam.com` for autonomous write actions with signing and aggregator credentials, and `indexer.vaipakam.com` for chain-to-archive ingestion with database write credentials. This describes the target separation, not necessarily today's bindings: the connected app's hostname is bound and serving, while surfaces with no public entry point (the keeper is cron-only) need no hostname at all — the requirement is the separation of duties and credentials, not that every surface answer on a URL. Shared archive storage such as the `vaipakam-warm` D1 database should be accessed with least-privilege credentials per Worker, and Worker bindings should grant only the read or write capability each surface needs. Deployment scripts should expose per-surface Cloudflare phases (`cf-app`, `cf-www`, `cf-keeper`, `cf-indexer`, `cf-agent`) plus an all-Cloudflare skip alias for rehearsals that do not need hosted-app redeploys.
...
- **Core Contracts (Examples):**
  ...
  - **Deployability splits, generally.** The bytecode ceiling is a real-chain constraint, not a design preference, so a facet that outgrows it is expected to be split rather than trimmed of behaviour. Two rules govern where the seam goes. First, the seam follows a boundary the product already has — a distinct user-facing route, a distinct actor, a distinct lifecycle stage — never an arbitrary bisection chosen to balance byte counts. Second, halves that are only correct when read together stay together: where two entry points share an invariant (a link one writes and the other clears, a one-at-a-time rule, a cooldown), splitting between them turns a locally-checkable rule into a cross-facet one and is the wrong seam even when it would free more space. Third, and cheapest where it is available: where a facet already reaches part of its own work through an internal self-call — a step deliberately run in its own execution context, typically to bound how much of the work is charged to one call frame — that self-call is a boundary the code already crosses on every run, and moving the step to the other side of it is the least disruptive seam there is. Nothing about the observable sequence changes, because the boundary is not new; only which address hosts the step beyond it. This third case is not an exception to the second rule but a limit of it: what the second rule protects is the ability to check a rule by reading one place, and a step that already executes behind a self-call is already being read that way. A split changes deployment shape only — the routes it separates keep the same behaviour, the same shared state, the same single transaction (any failure past the boundary still unwinds the whole acceptance), and the same single address that callers use.
...
- **Security Considerations:**
  ...
  - **Atomic Configure Spell:** Post-deploy Diamond configuration should be available through a compositional `DiamondConfigSpell` that sequences VPFI-token registration, oracle, reward-reporter, and NFT-image-URI configuration in one deterministic operator action. Chain-specific branches must skip canonical-only configuration, on mirror chains while continuing the remaining configure steps. CCIP lane, pool, and messenger configuration remains a separate phase because it uses cross-chain-surface authority and targets messenger / token-pool contracts rather than Diamond admin setters.
  - **The Discount Peg Is Not Part Of That Sequence:** Setting the VPFI discount price is an explicit, opt-in operator step, not one of the steps a deploy performs on its own, and a deploy that is not asked for it must finish with VPFI unpriced. This is a product decision expressed as a deploy default, not a deploy convenience: while the price is unset the lender's yield-fee discount is delivered as a direct reduction and carries the entire discount the lender is entitled to, including the consent-gated holding slice; once the price is set, that slice becomes payable in VPFI instead, so a consenting lender must spend VPFI to receive what the unpriced posture gave them outright. A launch that silently priced VPFI would therefore ship a materially different product from the one specified. The opt-in must be a single unambiguous value; any other value, including a plausible-looking one, leaves the price unset. Skipping the whole VPFI stack is not an acceptable substitute for declining the price, because that also skips token registration and reward-reporter configuration, which every VPFI chain needs.
  - **A Deploy Decision Stated At The Command Line Is Not Overridable By Configuration:** Where an operator declares an intention for a single run — which chain, which phase, whether to wipe and redeploy, whether they have reviewed the state a destructive step will abandon, whether the signing path is a hardware wallet — that declaration must hold for that run regardless of what any shared configuration file says. Such declarations are per-run assertions about what the operator has checked, and several are recorded afterwards as evidence the check happened, so a stored value that silently supplies or withdraws one produces a record of a confirmation nobody gave. The requirement is symmetric: stored configuration must neither switch a declaration on for a run that did not make it, nor discard one that was made.
  - **Stored Configuration Is Data, Not Instructions:** A deployment tool must read a configuration file as a set of name-and-value settings and must never execute it. Executing it makes the guarantee above unenforceable, because a file that runs code can rewrite the command line itself, disable the checks meant to catch it, or replace the machinery used to apply the operator's choices. Reading order matters for the same reason: settings are read before the operator's choices, so the choices are applied last and win. This is a requirement on the deployment tooling; operator procedures written elsewhere are held to it separately, and where they are not yet, that is a known gap rather than a satisfied requirement.
  - **A Setting Must Not Be Able To Start A Program, Or Choose Who Receives A Credential:** Two kinds of name are acted on by other programs rather than merely read. Some are treated as instructions to run something at start-up. Others do not run anything, but decide where an authenticated request is sent — the address of a service's API, or a proxy every request passes through — so a configuration file can direct a deployment's own credentials to a recipient of its choosing. A configuration reader must refuse both rather than pass them on, because the processes a deployment starts carry its credentials and the requests it makes present them. Both sets are open-ended, so this is stated as an obligation to refuse the known ones and to keep the list current — not as a claim that any implementation of it is complete.
...
- **Marketing Search and AI Discoverability:** The marketing site should be discoverable without client-side JavaScript. Public marketing routes should be prerendered for every supported locale during deployment so crawlers receive full page content, locale-specific metadata, canonical links, and alternate-language references. Social previews should include real title, description, and image metadata, and public educational pages should expose structured page identity where appropriate. Canonical long-form docs should also be available as plain Markdown through stable public URLs, with a concise crawler index and a complete single-fetch variant for AI agents that prefer text sources.
...
- **A Service That Cannot Reach Its Data Says So:** The platform's off-chain data services — the ones that record alert preferences, signed offers, support requests, diagnostics records and legal holds — must never answer from a database they are not certain of. When such a service is deliberately held off its database, every entry point refuses: a caller receives a temporary-unavailable answer that states plainly that nothing they sent was recorded and that nothing read back would be current, and background work declines to start rather than half-running. A refusal a user can act on is better product than an error that leaves them unable to tell a rejected write from a half-applied one.
- **Holding A Service Off Its Data Is Done By Removing The Capability:** Where the platform must stop every writer to an off-chain database — moving one, for example — it does so by depriving the service of the ability to reach any database at all, not by listing the places in the service that write. A list of writers is never provably complete: it cannot cover scheduled work that traverses no route, self-rescheduling background work, or an entry point added later. Removing the capability covers all of them without naming any, and is therefore the only form of this guarantee the platform will claim. The guarantee is over work the service admits once it has been held off its data; work already running when that happens is the residual named in the next point, and this point does not cover it.
- **The Residual Is Named, Not Absorbed:** Work already running when a service is held off its data still holds what it was given and can still write. That exposure is never presented as eliminated, and it is not presently bounded either: how long such work takes to finish has not been measured, and no operating procedure yet states how to wait it out. Naming an unmeasured exposure as open is the required behaviour; stating a waiting time nobody has measured would be the defect. An operating procedure that claims a stronger guarantee than the mechanism provides is a defect in the procedure.
- **A Refusal Claims Only What It Can Substantiate:** When a service refuses because it cannot reach its data, it speaks for the request in front of it — that this request reached no database, so nothing sent was recorded — and not for the service as a whole, which cannot exclude the residual above. For the same reason it does not tell the caller when to come back: how long such a hold lasts is not something the service knows, so callers retry on their own terms rather than on an invented estimate.
- **Moving A Shared Database Moves Every Reference To It At Once:** Where several services share one off-chain database, the platform treats the database's identity as a single fact stated in many places — service configurations, operator commands in runbooks and deploy scripts, and any script that builds such a command. A move that reaches some of those and not others is not a partial success but a specific defect: schema changes get applied to a database nothing reads, while the services keep reading the one nobody is changing, and each half looks correct when examined alone. The platform therefore refuses to hold a state in which those references disagree, so that such a move cannot be performed in pieces.
- **Data Carried Between Databases Is Verified Against The Source, Not Against The Tool's Report:** When records are copied from one off-chain database to another, the copy is confirmed by comparing what the destination holds against what the source holds, table by table — never by the copying tool reporting that it finished. A tool can complete successfully and still have lost rows, because a write that replaces an existing record may delete it first, and deleting a record can remove other records that depend on it. Copies are therefore performed as in-place updates, which cannot trigger that removal and can be repeated safely, and the row-by-row comparison afterwards is part of the move rather than a precaution. Because the move requires the copy to be run again once the services stop, and again after the switch, the copying step is a recorded, repeatable procedure rather than an improvised one. It carries records in dependency order, so that a record referring to another is never carried before the one it refers to and rejected. It treats a record deleted on the original as a difference like any other and removes it from the destination, since otherwise the two sides can never be brought into agreement at all. **Both ends of a copy are named in advance** — the platform's shared database and the specific database being moved to or from — and each is identified by more than its name, since a name can be reissued to a different database after a deletion while an identity cannot. Either direction between those two is permitted, which is what leaves the documented way back possible to perform; no other pairing is, in either direction. Requiring only that the shared database be *one* end is **not** sufficient and must not be restated as though it were: it would allow an unrelated database to be copied over the shared one, and the shared one's records to be copied into an unrelated database, while appearing to be a restriction. A table it cannot copy safely, because nothing in it identifies a record uniquely, is named as not copied rather than copied in a way that would duplicate records the next time.
- **The Database Being Moved Away From Is Not Destroyed By The Move:** Moving to a new database leaves the previous one intact and holding its records. Retiring it is a later and separate decision, taken when someone is satisfied it is no longer needed — never folded into the move itself, where it would convert a recoverable change into an irreversible one.
- **Going Back Is Another Move, Not An Undo:** That the previous database survives does not make the move reversible by pointing the services back at it. From the moment the services begin writing to the new database, the records they write — support requests, alert preferences, signed offers, notification state, the markers recording how far the chain has been read — exist only there. Pointing the configuration back without carrying those records across strands them exactly as moving forward without a copy would have stranded the originals. A return is therefore performed as a move in its own right, by the same sequence: stop every writer, confirm the source is no longer changing, carry the records, then switch. The platform states the distinction plainly because the two are easy to conflate: nothing was destroyed, and the data is nonetheless in the wrong place until someone moves it.
- **Stopping A Writer Is Confirmed By Observation, Not By Elapsed Time:** Depriving a service of its database stops anything new from beginning, but work already under way still holds what it was given and may finish writing afterwards. Because the platform does not claim a duration it has not measured, a database is treated as still only when successive readings of its contents, taken a stated interval apart, are identical — and read once more after the records are carried across, so that anything committed during the copy is caught rather than assumed away. The residual is named: a write storing the value already stored changes nothing observable, so the guarantee is that no observed change was in flight, and not that nothing was running.
- **Reconciling Is A Three-Way Question, And No Answer Is Automatic:** With a record of what the copy carried, each record admits three facts — whether the copy carried it, whether the original still holds it, and whether the destination holds it. Those three answers are what let each divergence be named correctly; none of them is applied by the platform, which reads both databases and writes to neither. Even the simplest case — a record that appeared on the original after the copy and has never existed on the destination — is reported for a person to apply deliberately. Every divergence is a decision, because each can be made worse by guessing — two databases running independently can issue the same identifier to different records, a record the destination has since deleted is indistinguishable from one that never arrived unless the copy's record says otherwise (and a deletion may be an obligation rather than housekeeping), and a record deleted on the original after the copy appears in none of its records at all, so anything working through them never encounters it while the destination quietly keeps something that should be gone.
- **A Late Change Is Found By Comparing A Database With Its Own Past:** Reconciling after a move must account for records that were *changed* late, not only for records that arrived late. A record changed late still exists on both sides, so a reconciliation that asks only whether a record is present does nothing and reports that nothing needed doing — the same defect as counting records instead of comparing them, one level up. Comparing the two databases against each other cannot serve either, because the new one has legitimately moved on and almost every active record differs. What identifies a late change is that the record differs from what the copy recorded seeing, so the copy writes down what it saw and the reconciliation is performed against that record. Anything found is reported and left in place: which of the two values is correct is a decision for a person, and choosing silently would be the overwrite the reconciliation exists to avoid.
- **An Operation That Cannot Do All Of What Was Asked Does None Of It:** Where a step over records has been planned in full before it begins, and the plan contains anything the platform will not resolve on its own, nothing is applied. Carrying the safe part first and failing afterwards leaves a live database partly changed by a command that reported failure, which is the hardest state to reason about later: the operator cannot tell which of the records in front of them that run put there. Planning completely and then acting is what makes the promise "a conflicted run changes nothing" a property rather than an intention.
- **A Report Of A Conflict Identifies The Record, Not Its Contents:** Where the platform reports that two copies of a record disagree, it names the table and the identifier and stops. It does not print the record, because these reports are read in terminals, pasted into run logs and attached to issues, and the records in question include support messages, contact addresses and diagnostic traces. Someone who needs to see a value asks for it deliberately, which is a decision that leaves a record of itself.
- **Silence Is Never Reported As Success:** Where an operation cannot do part of what it was asked — a table it cannot carry, a record it cannot resolve, a service it cannot confirm — it fails, and its result says so. It does not print what it skipped and then report success, because a result is read by scripts and by people in a hurry, and both take a success as covering everything that was asked. For the same reason a destructive behaviour is never what an unrecognised or mistyped instruction falls back to: the operation states which mode it is in, and refuses instructions it does not understand rather than proceeding under an assumption the operator did not make.
- **A Move Is Confirmed Against What Is Serving, Never Against What Is Configured:** Checking that a service has moved means asking which database the version currently handling requests is attached to — not what the service's latest stored configuration says. The two routinely disagree: a configuration uploaded but not yet released describes a version nobody is served, and reading it reports the move as complete while every write still lands in the database being left behind. Where traffic is split across more than one version, every one of them is checked, since a move that reached most of the traffic and not all of it is not a move. A version deliberately attached to no database at all is reported as such — that is the held-off state, not a disagreement.
- **Watching Narrows The Window; Reconciling Afterwards Keeps Looking, And Nothing Closes It:** Observed stillness is evidence, never proof, and the platform does not let a move rest on it. Work suspended while waiting on something outside the platform can sit out every reading and commit later, so a move is not finished at the switch: once the services are running against the new database, the one they left is read again and anything that arrived late is reported for a person to apply, repeating until two consecutive attempts find nothing. **Two clean attempts pause that search; they do not end it.** Nothing available to the platform can withdraw access that already-running work holds on the database being left behind, and no limit on how long such work may run has been measured — so the comparison is repeated on a stated schedule, by a named owner, with every run recorded, for as long as the old database is kept. Retaining it makes a late record recoverable; only repeating the comparison makes one found. That reconciliation **reads and reports; it does not write.** A database other services are writing to cannot be checked and then safely written to on the strength of that check: another record can claim a uniqueness the platform enforces in the interval between the two, and no amount of checking first removes that interval. So every difference it finds is named for a person to act on, including the one case that could in principle be applied mechanically. Where the number of such records is expected to be zero, requiring a deliberate human step for each is a better trade than an automation racing a live database, and it places a decision on every record that moves after a switch. The database left behind is retained throughout, so a record noticed after the fact is still recoverable rather than lost. **Some differences have a legitimate resolution that changes no data** — a deletion the operator decides should stand, a clash resolved by placing the late record alongside rather than over the live one, a stale copy the operator decides to keep — and the comparison reports differences in data, not decisions about them, so those differences continue to be reported on every later pass. That is a stated limitation of the comparison and not an unresolved condition: such a difference is recorded once with the decision taken, and the repeated comparison continues, because its purpose is to surface what is new.
...
- **Worker Environment Preservation:** Each Worker that relies on operator-tuned environment values managed outside its checked-in configuration should declare, in that configuration, that such values survive a deployment.
  - Preservation is a property of the **Worker**, not of the command used to deploy it: any ordinary deployment of that Worker should leave operator-managed values intact, whatever route or wording is used to trigger it, and whether the deployment publishes immediately or stages a version.
  - This matters because those values govern live behaviour — liquidation thresholds and confidence windows for the keeper, recipient-token validation and marketplace pagination for the agent — so losing them silently reverts risk behaviour to defaults at the moment it starts mattering. Deleting such a value should be a deliberate operator action, never a side effect of shipping code.
  - Automated checks should verify this preservation is declared. The verification should read configurations **structurally**, discover them rather than trust a list, and require the declaration on **every deployment configuration in the tree** — not only the canonical one per Worker, and not only those a check judges to be for a Worker that has values to lose. Which configuration a deployment loads, and which Worker it targets, are decided on the command line: the stored name can be overridden, a missing compatibility date supplied, a named environment selected, and an arbitrary path chosen. A check cannot answer any of that from the files alone, and an attempt to do so is the same unbounded inference this specification rejects elsewhere. Requiring the declaration everywhere removes the question. A Worker with no operator-managed values today should declare it too, so that gaining one later is not also a moment of exposure. Two bounded exceptions follow from the deployment tool's own rules rather than from judgement about the file: a **project mode the tool refuses the declaration for** is exempt, identified by that mode's own marker, since requiring it would leave no version of the file that satisfies both the tool and the check; and a **named environment within a configuration** is not separately required to declare it, because the setting is top-level-only and the top-level value is what a deployment reads once an environment is selected. Where such a rule is unclear, the answer is to establish what the tool does — not to require the cautious-looking thing, which here would mean an unsupported field.
  - A secondary check **may** additionally reject individual deployment commands that would not preserve them, for Workers where the declaration is absent. That allowance stands and is deliberately not withdrawn; the platform simply does not exercise it, because the one case such a check demonstrably added — a command selecting a non-canonical configuration — is answerable as a property of configurations, above, and everything else it attempted was not. Any future secondary check must still meet the constraints the platform has already ratified for one: admit only a **semantics-preserving normalisation of a known notation**, never an inference about which text is executable or what a name stands for; **establish** a runner's behaviour rather than assume it, and where it cannot be established, judge the command on its own terms instead of guessing in either direction; prefer **missing an exotic spelling over reporting text that performs no write**, because it runs inside routine validation where a false report blocks correct work; and **record and pin by a test** every miss it knowingly keeps. The fuller account of why three attempts at such a check were withdrawn is in [`DeployGuardRewriteScanRecord.md`](../DesignsAndPlans/DeployGuardRewriteScanRecord.md), which is a record and binds nothing.
  - Where a check would need another system's execution model to answer correctly — a continuous-integration system's, a document format's grammar, a build tool's variable language — the correct answer is a **declaration from the deployment itself** rather than a better approximation in the check.
  - What the declaration does not cover should be **stated where the checking lives** rather than left to be inferred: a deployment that explicitly overrides the declaration on the command line, a configuration that does not exist in the tree at checking time because it is generated — which a command-level check could refuse by declining to trust an unreadable selection, so removing one is a real reduction in coverage and not merely an unchanged limitation — and a configuration checked in under a name that does not follow the deployment tool's own convention. The last two are **reductions** rather than limitations the platform always had — a command-level check covered both, one by declining to trust an unreadable selection and one by following whatever path a command named — and a specification that lists them without saying so understates what an approver is accepting. Recognising the naming case again would mean classifying arbitrary files by their contents, which is the failure mode that produces false reports on correct work, so the convention is stated as a rule for contributors instead.
...
- [contracts/test/DeployerZeroRolesTest.t.sol](../../contracts/test/DeployerZeroRolesTest.t.sol) — post-handover role invariant. (A `Create2DeploymentTest` was cited here for months after it left with the CREATE2 reward bootstrap in T-068; there is no deterministic-address deploy to test — see the retired CREATE2 bullets below.)
...
- **CREATE2 Versioning / Idempotency — RETIRED (T-068).** These two rules governed the LayerZero-era reward-proxy bootstrap, which needed one address on every chain for peer wiring. CCIP resolves chains through the messenger's own registry, so the reward messenger is now deployed with an ordinary `new` and **each chain's messenger address is its own**; there is no version salt to bump and no cross-chain address parity to verify. Kept struck so a reader who remembers the rules finds where they went rather than re-adding them. Recorded in `contracts/.env.example` (`REWARD_VERSION is gone`) and the dead-section banner in `docs/ops/DeploymentRunbook.md`.
...
- **Sanctions Oracle Availability:** Sanctions oracle configuration is per chain and optional. Chains without a configured oracle should behave as no-op for this check. On chains where the oracle is configured, value-moving sanctions checks that cannot determine a clean result should fail safe for value owed to a wallet the protocol has already confirmed flagged (frozen proceeds stay parked in that wallet's own vault) and for confirmed-flagged position movement, rather than route value to a possibly flagged wallet. A wallet that was never confirmed flagged keeps the liveness / fail-open posture during an oracle outage and is not frozen merely because the oracle is unavailable (#1006).
- **Terms Acceptance Gate:** App routes may be gated behind a versioned on-chain Terms of Service acceptance. `currentTosVersion == 0` represents a disabled/testnet state. Once activated, users must accept the current Terms version and content hash before using the connected app's gated routes; version bumps or hash changes invalidate prior acceptances. While the acceptance status is loading or cannot be read, the connected app must keep gated routes closed and show a retryable verification state rather than rendering gated content.
...
- **Frontend Warning:** Before signature, the refinance flow should tell the borrower that the old lender is repaid with principal plus the interest due under the loan's selected interest mode — for a full-term-interest loan that is the full-term interest (not merely accrued-to-date) — or more if the loan is already past maturity in its grace window, where accrual continues (`settlementInterest` uses `max(elapsedDays, floorDays)`) and the late fee applies (A1/D5) — while a pro-rata-opted loan settles only the accrued amount, after crediting any interest already settled (#1003). (Updated 2026-06-12 per [#411] / `RefinanceOldLenderOverpayFix.md`: the prior wording also required a rate-shortfall top-up; that addend has been DROPPED because the mode-appropriate interest is already the lender's maximum entitlement on this loan (full-term interest for a full-term loan; the accrued amount, net of already-settled interest, for a pro-rata-opted loan per #1003), and any additional shortfall over-compensates the exiting lender at borrower expense. The Original Lender Protection Rule below is structurally satisfied by that mode-appropriate payoff for an EXITING lender; the shortfall remains required only on the obligation-transfer / offset paths where the lender STAYS on the loan.)
- **Process:**
  ...
  4.  During old-loan payoff, the protocol repays Lender A (principal + the interest due to Lender A under the loan's interest mode — full-term interest for a full-term loan; accrued/pro-rata interest, net of already-settled interest, for a pro-rata-opted loan (#1003) — as per early repayment rules; any governance-approved alternative refinancing interest policy is Phase 2 scope), updates the old-loan NFTs, handles the collateral path described below, and closes the old loan.
  ...
  8.  The selected refinance offer must make the exiting original lender whole under the refinance protection rule below. No additional rate-shortfall top-up is charged on refinance beyond the original lender's principal plus their mode-appropriate interest entitlement (full-term interest for a full-term loan; accrued/pro-rata interest, net of already-settled interest, for a pro-rata-opted loan — #1003, settled through `settlementInterestNet`); shortfall remains a rule for obligation-transfer / offset paths where the old lender stays on the continuing loan.
