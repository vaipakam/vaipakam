...
- **Worker Environment Preservation:** Each Worker that relies on operator-tuned environment values managed outside its checked-in configuration should declare, in that configuration, that such values survive a deployment. Preservation is a property of the Worker, not of the command used to deploy it: any ordinary deployment of that Worker should leave operator-managed values intact, whatever route or wording is used to trigger it, and whether the deployment publishes immediately or stages a version. This matters because those values govern live behaviour — liquidation thresholds and confidence windows for the keeper, recipient-token validation and marketplace pagination for the agent — so losing them silently reverts risk behaviour to defaults at the moment it starts mattering. Deleting such a value should therefore be a deliberate operator action rather than a side effect of shipping code. Automated checks should verify that every Worker holding operator-managed values declares this preservation, and a secondary check may additionally reject individual deployment commands that would not preserve them, for Workers where the declaration is absent. Where such a check reads a configuration file to decide either question, it should not trust the checked-in copy of a configuration that the deploying file rewrites before the deployment runs, since that copy is then not the file the deployment loads; a configuration whose contents cannot be relied upon should leave the command to be judged on its own terms rather than be treated as preserving. Deciding that a file rewrites a configuration should rest on the file actually performing a write: text that merely matches or shares a name with one — a pattern being matched, a comparison between two values, a locally defined function named after a copying operation, a quoted example in a language where a quoted value is inert — is not a rewrite. A shell is the exception and should be read as one: there a quoted string handed to a command may well be executed by it, so such a check should treat quoted shell text as executable unless it is plainly being stored, and accept that a script quoting an example of a write to its own configuration will be reported. Whether text is shell should be decided by whether something runs it rather than by the kind of file holding it: a command handed to an interpreter from inside a program written in another language is still a command, so shell spellings of a write should be looked for there too, while the same characters standing as that program's own syntax should be read in that program's language. That rule runs both ways: a spelling of a write excluded in one language because it means something else there should still be recognised inside text that another language executes. Where what the file runs differs from what it contains, the rewrite question should nonetheless be asked of the file essentially as written. Two established exceptions predate this rule and are not what it is about. A helper written for a Windows shell is normalised into the same form its workflow-body equivalent already takes, so that one command is read the same way wherever it appears. Both Windows shells have path separators normalised, and the command name lowered where it is spelled in title case — only that spelling, so other casings the platform accepts are still not recognised, which is a known miss rather than an intended boundary. Beyond that the two differ, and the difference is worth stating because each rewrite is load-bearing for a different reason. The command shell's line-continuation character is folded away, so a continued command reads as the one command it is. The PowerShell dialect instead has a literal variable binding rewritten into the ordinary assignment spelling, so that the same variable model which resolves a directory or a command name elsewhere resolves it here too — without that, a deployment run from a directory held in such a binding is read as running somewhere unknown. Two ordinary spellings defeat that rewrite and both are recorded as defects rather than presented as covered: a binding closed with that shell's ordinary statement terminator is not recognised at all, and a binding whose name differs in case from the use that reads it is recognised but never found, because the shared model matches names exactly while that shell does not. Each leaves the deployment attributed to nowhere, which is a SILENT PASS, and each is ordinary style rather than an exotic spelling — so the rewrite fails in precisely the situation it was written for. Neither rewrite is applied to the other shell. Nor does the PowerShell dialect get a continuation rule of its own — but stating that as "no folding happens for it" is wrong, and the difference is a live defect rather than a wording nicety: a later shared pass folds a trailing backslash, and that shell does not treat a backslash as a continuation at all. A deployment that follows an unrelated line ending in one is therefore read as a single command with it, so a safety flag belonging to the earlier line is taken to cover the deployment below it, and an unsafe deployment passes. That is true wherever such a body is read — a standalone helper written for either Windows shell, and equally a step in a continuous-integration workflow, which is the form that actually deploys; scoping it to standalone files, as this text did, would send a correction to the narrower of the two. That is a SILENT PASS — the direction this check must not fail in — and it is recorded as a defect rather than defended. Two limits on it belong in the record, so that a fix is aimed where the fault actually is rather than at every reader in the check: the folding applies only where a body is split into lines at all — a shell file, a Windows helper, or a shell fragment lifted out of a file written in something else — and not to a document's or a program's own lines, which are read whole; and it requires the line to end immediately after the backslash, so a body that reaches the tree with the other common convention for ending a line escapes it. That second limit is about the bytes in the file and NOT about the platform anyone works on: this repository normalises tracked text to one convention on every platform, including the one whose native convention differs, so a tracked helper is exposed wherever it is edited. Reading it as "users of that platform are unaffected" would be wrong, and was. The defect is real, and narrower than "every file". Stating that bound as "everything read AS A SHELL" would be wrong, and wrong in the dangerous direction: a script declared inside a package manifest is read as a shell command and is NOT split into lines at all, so a newline inside it — which the package manager honours as a command boundary — is invisible, and a safety flag on one of its lines is taken to cover a deployment on another. That is a SILENT PASS of its own and is recorded as one. The two are the same mistake seen from opposite sides: joining lines that should stay apart, and never separating lines that were always apart. Line splitting is a property of the reader a body happens to pass through rather than of the body, and that is the thing to fix rather than either symptom. And a deployment declared as a script inside a package manifest is judged against THAT DECLARED VALUE rather than the whole manifest, because a manifest is a list of independently invocable scripts and a write in an UNRELATED one is not part of the script being judged; that is a selection, but of a boundary the format itself draws, not of "the parts that look executable". The reason is sound and the implementation does not match its shape: what is actually read this way is every property value that is a single piece of text, in ANY file of that format — not the declared scripts, and not a manifest in particular. So a manifest whose description merely names the command is reported as performing a deployment that nothing runs, and so is an unrelated data file of the same format that happens to quote the command. It is not "every string" either, though the exception is narrower than it first appears and depends on the file's LAYOUT rather than on the value's kind: a list of texts is passed over only where the line it sits on also carries an ordinary property value, so the same words inside a list are unread when the file is written compactly and ARE read when the file is written across several lines, one element to a line. Recording that as "a list value is not read at all" was wrong, and the fixture that appeared to establish it only did so because it was written compactly. The same reading also takes in text that is COMMENTED OUT, in the variant of the format that permits comments — so a commented-out line naming the command is reported although it is not a property at all. That matters beyond accuracy: the remedy sketched for this defect is expressed in terms of which named entries hold scripts, and a commented line has no name, so a remedy built that way can satisfy itself while leaving this behaviour exactly as it is. Whether the two reports share a cause depends on how a correction is scoped, and saying flatly that they do not would be wrong. Reading a file value by value is chosen INSTEAD of reading it line by line, so a file that stops being read the first way is read the second way: a correction that simply stops applying the value-by-value reading to files other than manifests leaves the data file reporting, now through the line-by-line reading, while a correction that keeps reading every file that way but declines to treat a non-script value as a command removes both reports. The first is a partial fix and should not be recorded as a whole one; the second is a whole fix for this symptom. Which is which can be told from what the report quotes — the value alone, or the entire line it sits on. Judging a declared script against its own value is right; treating every value as a declared script is the defect, and it is recorded as one rather than presented as the boundary just described. That reasoning holds only for siblings nothing invokes: a script that CALLS a sibling by name really does run that sibling's write first, and judging it against its own declared value alone misses that — a known gap, recorded rather than intended. Following such an invocation into the named script should NOT be treated as the obvious remedy: it was implemented once and withdrawn, because it is inter-procedural reasoning of the kind this check declines everywhere else, and correcting it one case at a time produced invocation ordering, package selection, quoted mentions, command position, chain depth, short-circuited branches and pre/post hooks as successive findings. A check that declines to build a call graph for one language should not build one for another; the remedy for this gap is the declaration below, not a resolver. Three transformations of the text were attempted and all three withdrawn, and the reason is a single one worth stating as intent: each asked a question about the file that can only be answered by modelling another system — a continuous-integration system's execution model, a document format's grammar, a build tool's variable semantics — and this check is a scanner, not an interpreter for any of them. Selecting "the parts that can run" is the worst of the three and should never be attempted: a selection turns every construct it fails to recognise into a SILENT PASS, which is the failure this check exists to prevent, and the parts that can run cannot be enumerated — a command written inline in a sentence is an instruction this check already acts on, so prose is not the boundary it appears to be. Blanking a document's prose should likewise not be attempted, however conservatively its rule is stated, and not because such a rule is hard to get right but because it cannot exist: this check treats a bare, unindented line in a document as an actionable command, which is why a runbook's copy command is reported, and a prose sentence naming a write has that same shape, so separating them is the very judgement whose answer produced the unwanted report. Expanding a build file's recipe variables is different in kind and still should not be attempted at this scope: being right about it requires implementing the build tool's variable language — conditionals whose outcome depends on an environment the check cannot see, explicit removal, stored variable bodies, a settable recipe marker, and the syntax of assignment itself — and attempts to approximate it produced corrections whose own edges were the next round's findings. A check of this kind should therefore admit a transformation only where it is a SEMANTICS-PRESERVING NORMALISATION of a known notation — a total, deterministic rewriting of one spelling of a command into another — and never where it requires inferring which text is executable or what a name stands for. Whether characters are added or removed is not the test, and stating it that way would be wrong: the Windows normalisation substitutes separators and, for the command shell, folds a continuation, so it removes and replaces text, yet it stands because it decides nothing about what runs. Its separator and casing rules were defended as being of that kind, on the grounds that the platform resolves either spelling of the command to the same program — and that defence is withdrawn, because it holds only where the text actually runs on that platform. The shell these rules key on is available on other platforms too, and on a runner whose executable lookup is CASE-SENSITIVE the two spellings of the COMMAND NAME denote different programs — the runner distinguishes them where the rules assume it does not. That qualifier is load-bearing and should not be relaxed to "other platforms": the discriminator is how a runner RESOLVES COMMAND NAMES, not which platform family it belongs to, and a host with case-insensitive lookup resolves both spellings to one program, where the missed spelling is a genuine unsafe deployment again. A correction that stops recognising the uncovered spellings wherever the platform is not the one the rules were written for would therefore introduce a SILENT PASS on such a runner. Resolution modes other than the case-sensitive one are OPEN, and an implementation should establish which applies rather than infer it from the platform. The separator half of this does not have that problem IN THE CASE THAT WAS ESTABLISHED, and the scope matters: for a path handed to one of that shell's OWN commands, such as the change of directory, the shell reads either separator the same way everywhere, so normalising it asserts nothing the shell does not already do. That is the whole of what is settled; a path handed to the deployment tool itself is a different question, taken up below and deliberately left open. So the rules are applied by asking which shell will interpret the text, where the reason for having them is a claim about which PLATFORM will run it, and those are not the same question. The check's own reasoning already says a rewrite of this kind must not be applied where that platform's rules do not hold, which makes this a case of a stated principle and its implementation disagreeing rather than an unconsidered gap. A correction must also decide what to do when the platform cannot be determined — a deployment whose target is chosen from a set, or named indirectly — and the answer must not be to assume the platform the rules were written for, since that is exactly the present behaviour. But it would be wrong to say their incompleteness costs only recognition, and that claim is withdrawn: the CASING rule errs in both directions, and the record briefly said both rules did — which overstated it in the opposite way, and is withdrawn. The command spellings the casing rule does not cover are missed, and the one it does cover is also matched inside text that merely MENTIONS the command in a block the shell never executes. No such fault is demonstrated for the separator rule, and two stronger claims made for it are withdrawn. It does not invent a directory change the shell would not perform: the shell in question accepts either separator on every platform it runs on, so the change of directory really happens and the attribution is right. Nor is any separator-caused MISS shown. Where such a path appears inside an inert block and is reported, the cause is the same missing model of that shell's quoting described above, which reports the lower-case spelling there with no rewrite involved at all. So a correction aimed at the separator rule WHERE IT REWRITES A PATH GIVEN TO THE SHELL'S OWN COMMANDS would be changing behaviour that is correct, and the record says so rather than leaving a symmetry that reads as licence. That approval extends no further, and the remaining case is UNSUBSTANTIATED rather than cleared: the same source notes the two separators are interchangeable for that shell's own commands but may not be for a program it launches, and this rewrite also touches path-shaped arguments handed to the deployment tool itself, which is such a program. Nothing here demonstrates that case in either direction, so an implementation should treat it as an OPEN question to settle — never as behaviour this specification has endorsed preserving. Neither direction is CREATED by these rewrites: such a mention is already read as a command when it is spelled in the form the reader already accepts, because the reader has no model of that shell's quoting. So what both rules add is REACH into an already fallible reading rather than a fault of their own. That distinction is worth keeping rather than collapsing into "these rewrites are unsafe", because widening a known imperfection and introducing one call for different remedies — but it must not be used to excuse either rule, and it should be stated of both or of neither. Its PowerShell assignment rule is NOT: recognising an assignment requires a model of that shell's quoting, and without one an assignment written inside a here-string is rewritten as though it ran, which can invent a deployment the file never performs. That rule is therefore an exception carried for historical reasons rather than an illustration of the standard, and it is recorded as a defect rather than defended. The three withdrawn transformations each failed the real test rather than the character-count one — a selection had to judge which parts execute, blanking had to judge which lines are prose, expansion had to judge what a name holds — and any of those judgements being wrong changes which text the check reads as code. Where a transformation cannot meet that bar, the text should be left alone and the resulting WRONG VERDICT recorded — which is not always a miss, and writing it as one reverses a third of the record: leaving a document's prose alone preserves a FALSE REPORT, and that is exactly the outcome the withdrawn blanking rule was reaching for. Where a build file's variables are modelled at all — for finding deployments, which this check has always done — that model should not be extended on the reasoning that a correction is "merely lexical" or self-evidently right, because two such corrections were attempted and each regressed in the OPPOSITE direction from the other — which is why neither is a template for judging the next one. Treating the tool's escaped currency symbol as inert is wrong because the tool reduces it to a single symbol and hands that to the shell, which may expand it, so a deployment spelled that way really runs and the check would stop seeing it — a SILENT MISS. Adding the tool's other canonical default filenames is right in itself but routes those files through a model already known to be imperfect, extending its FALSE REPORTS to files that previously escaped them — the opposite failure, reached by widening the model's scope rather than by sharpening it. The general lesson is that this model's imperfection is load-bearing: its one consumer has been calibrated around it, so making it locally more faithful can be a regression. A miss the check knowingly keeps should be recorded and pinned by a test that ASSERTS the miss, so that a later correct fix announces itself rather than passing unnoticed. Where the check would need another system's execution model to answer correctly, the correct answer is a declaration from the deployment itself rather than a better approximation here. An executable helper carrying no file extension should be classified by the interpreter its opening line names, for every interpreter such a check knows, since a file read in the wrong language misreads what is data and what is code in both directions. Because the check runs inside routine validation and a report blocks work that is correct, it may otherwise miss an exotic spelling of a write, which the preservation declaration already covers, rather than report text that performs no write at all. That restraint is about what the text IS, not about what it would do when run: a command that genuinely copies or replaces a file should be treated as writing even where its options or the state of the target might mean it does not, because deciding otherwise means modelling how each command's options override one another, and such tables do not stay correct. A command told not to overwrite is therefore reported, and the consequence is only that the configuration's contents go unread.
