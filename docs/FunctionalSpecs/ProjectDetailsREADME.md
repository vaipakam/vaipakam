...
- **RPC Secret Preflight:** Deployment scripts should check for the expected per-chain worker RPC secrets and warn with the exact operator command when a secret is missing. Scripts must not auto-populate RPC secrets because those values may contain provider API keys.
