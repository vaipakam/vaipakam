...
- **Per-Chain Flip Discipline:** `setDepthTieredLtvEnabled(true)` remains a manual Timelock / Safe-governed rollout per chain after audit sign-off. Emergency controls remain available through per-asset pause, master switch disable, and anomaly auto-pause.
