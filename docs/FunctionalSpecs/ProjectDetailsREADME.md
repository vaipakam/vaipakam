...
- **Security Considerations:**
  ...
  - **A Deploy Decision Stated At The Command Line Is Not Overridable By Configuration:** Where an operator declares an intention for a single run — which phase to run, whether to wipe and redeploy, whether they have reviewed the state a destructive step will abandon, whether the signing path is a hardware wallet — that declaration must hold for that run regardless of what any shared configuration file says. Such declarations are per-run assertions about what the operator has checked, and several of them are recorded afterwards as evidence that the check happened, so a stored value that silently supplies or withdraws one produces a record of a confirmation nobody gave. The requirement is symmetric and both halves matter: stored configuration must not switch a declaration on for a run that did not make it, and must not discard one that was made. Declarations of this kind are therefore identified explicitly rather than by convention, and the identification is verified against the actual set of command-line options rather than maintained by hand — a list that silently falls behind the options it protects re-creates the defect while appearing to prevent it.
