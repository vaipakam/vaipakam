...
- Where a leg is **not** measurable, the position cannot be assessed, and what
  follows depends on whether a consent regime is in force:
  - With progressive risk access **enabled**, the sale is admissible to this
    assessment and the incoming buyer's own risk-access gate governs, exactly as
    it does for any illiquid-backed pair — a buyer who has not opted into that
    risk cannot acquire the position, and one who has may.
  - With progressive risk access **disabled** (the default), no consent surface
    is in force, so the sale is **refused**. Admitting it would hand an
    unpriceable — in the worst case worthless — position to whoever authored a
    standing offer, with no loan-specific or pair-level agreement anywhere in
    the flow.
  Neither outcome is silent: a refusal states that a specific leg is not
  priceable and names which one. The surface must never show a health figure for
  a position that has none — "this position has no price-based safety check" is
  the honest line, not a blank or a zero — and the accept preview reports this
  case as a plain block rather than as a measured health shortfall.
