...
Range Orders extend the single-value offer model without changing the default Phase 1 path for basic users.

- **Master Flags:** Governance controls three independent flags: amount ranges, rate ranges, and partial-fill matching. Fresh deployments default all three flags to `false`, so single-value offers remain the only active path until governance intentionally enables a Range Orders mechanic.
- **Auto-Collapse Backcompat:** A blank or zero upper bound collapses to the single-value amount or rate. Existing callers and basic-mode UI flows can continue to create ordinary offers without learning range semantics.
...
- **Internal-Match Incentive and Surplus Handling:** The internal matcher may receive a bounded per-leg incentive from each matched notional amount. The incentive must be lower than the typical external liquidation cost surface and must be range-bounded by governance. Any residual borrower collateral after a partial internal match remains claimable through the ordinary borrower claim path.
...
- **Flash-Loan Receiver Support:** A standalone `FlashLoanLiquidator` receiver may support Aave V3 `flashLoanSimple` and Balancer V2 `flashLoan` callbacks for keeper-bot funded discounted liquidations. The receiver should initiate a flash loan, call `triggerLiquidationDiscounted` with itself as recipient, swap seized collateral back to principal through off-chain-supplied DEX-direct calldata, and revert the whole transaction unless proceeds cover debt plus flash-loan fee and configured gas/profit headroom. Owner-gated receiver entry points are keeper operational controls only; the Diamond discounted liquidation entry point remains permissionless.
...
- **Default Buy Options:** Buy-adapter deployment scripts should set sane default LayerZero buy options inline at deploy time so a freshly deployed adapter can quote and execute buys before any optional post-deploy gas-budget tuning. Deploy-time defaults and post-deploy update scripts should share the same `OptionsBuilder` encoding path so equivalent inputs produce byte-identical options.
...
- **Buy-Flow Compose Settlement:** Cross-chain fixed-rate buys should return purchased VPFI through the source-chain buy adapter using LayerZero OFT compose, not by sending directly to the buyer wallet. The adapter must cross-check the compose request id against local pending buy state before forwarding VPFI to the recorded buyer; forged or replayed arrivals should be marked as stuck VPFI and recoverable only through owner / governance controls.
- **Buy Reconciliation Watchdog:** The private ops watcher should cross-check canonical-chain processed buy events against matching source-chain buy requests by request id, buyer, and amount. The receiver should expose an owner-controlled reconciliation-watchdog enable flag for planned ceremonies; mismatch detection should alert operators, while automatic pausing remains a later operational policy choice.
...
- **Types of Notifications (Examples):**
  ...
  - **Cross-chain VPFI Buy Received:** Confirmation when a LayerZero-routed buy is received and fulfilled.
...
- **Security Considerations:**
  ...
  - **Atomic Configure Spell:** Post-deploy Diamond configuration should be available through a compositional `DiamondConfigSpell` that sequences oracle, reward-reporter, fixed-rate buy, and NFT-image-URI configuration in one deterministic operator action. Chain-specific branches must skip canonical-only configuration, such as VPFI buy receiver setup, on mirror chains while continuing the remaining configure steps. LayerZero endpoint / DVN configuration remains a separate phase because it uses OApp-owner authority and targets LayerZero endpoint contracts rather than Diamond admin setters.
...
- **Internal-Match Config Visibility:** Frontend and operator surfaces should read internal-match enablement, per-tier liquidation thresholds, the external-liquidation priority window, and the per-leg matcher incentive from live protocol configuration. Hardcoded liquidation-threshold copy must not survive once the tiered threshold surface is active.
...
- active-loan discovery should include a paginated, current-state view for match-eligible loans while internal matching is enabled. This view should filter active loans by current LTV against their snapshotted liquidation threshold and should return empty when the internal-match switch is off.
...
- **Internal-Match Liquidation Tests:** Tests should cover internal-match enablement defaulting off, per-tier liquidation-threshold snapshots at loan initiation, two-loan reciprocal matches, three-loan cycle matches, priority-window blocking of external liquidation, reopening external liquidation after the window, per-leg incentive bounds, partial-match residual claimability, terminal-state claim behavior, and event/indexer compatibility.
...
- **Testnet Master Flags:** Testnet deployment automation may flip the Range Orders master flags (`rangeAmountEnabled`, `rangeRateEnabled`, and `partialFillEnabled`) on after deployment so flow scripts can exercise the enabled feature surface. Mainnet deployment automation must not auto-flip these flags; production rollout remains governance-controlled through the Timelock / Safe path.
- **Phase-Based Deploy Ceremony:** `deploy-testnet.sh` should mirror `deploy-mainnet.sh` phase-for-phase so rehearsals exercise the same operator flow: preflight, contracts, mocks where applicable, LayerZero config, Diamond configure spell, handover, ABI sync, verification, and per-surface Cloudflare deploys. Testnet may additionally expose a `pause-rehearsal` phase; mainnet pause flows must remain standalone incident-response scripts, not ordinary deploy side effects.
- **Pause Runbooks:** Production incident response should use a standalone `pause-all-chains.sh` or equivalent that prints Diamond and OApp `pause()` calldata for every deployed chain, records a timestamp for the five-minute response budget, supports state verification, and can print unpause calldata for cleanup. Mainnet unpause still routes through the Timelock-controlled unpauser path.
- **LayerZero Owner Separation:** `ConfigureLZConfig` and OApp ownership transfers should run under the documented OApp owner key (`DEPLOYER_PRIVATE_KEY`, `VPFI_OWNER_PRIVATE_KEY`, or `REWARD_OWNER_PRIVATE_KEY` as applicable), not the Diamond admin key by assumption. Handover scripts should read the current on-chain `owner()` first and skip with operator guidance when the signer does not own that OApp.
- **Mixed-Authority Peer Wiring:** Cross-chain peer wiring must route each `setPeer` by the current on-chain owner of the source OApp. During phased rehearsals this can mean some legs are direct admin-EOA broadcasts while other legs require Safe Transaction Builder batches. Runbooks should include decoded `to`, `_eid`, and `_peer` review tables, post-execute `peers(uint32)` verification, and a clear note that deterministic CREATE2 meshes can make some Reward-OApp peer addresses equal the local target address.
- **LayerZero DVN Policy Modes:** Mainnet LayerZero configuration defaults to the hardened 3 required DVNs plus 2 optional DVNs with a 1-of-2 optional threshold. Testnet scripts may expose a clearly named mode such as `DVN_POLICY_MODE=testnet1of1` that reads only `DVN_REQUIRED_1` and `DVN_OPTIONAL_1` and applies a 1 required plus 1 optional / threshold-1 shape. Duplicate-DVN checks, per-chain confirmation defaults, and explicit operator confirmation should run in both modes. Testnet DVN broadcasts may be parked when current endpoint defaults already provide equivalent rehearsal value.
...
- **Operations Export Phase:** Deploy scripts should export the subgraph event ABI / per-chain YAML, Tenderly alert YAML, and LayerZero watcher secret snippets from the same deployment artifacts used by the apps. These exports prevent monitoring, indexing, and OApp watcher configuration from drifting away from the deployed Diamond and OApp addresses.
...
- **LayerZero Watcher Scope:** The private LayerZero watcher may remain mainnet-only until mainnet cutover. Testnet OApp addresses should not be stuffed into mainnet-shaped `OAPP_*` secrets because that produces misleading drift reports against the wrong endpoint ids and dashboards.
...
- **CREATE2 Idempotency:** CREATE2 deployment scripts should check both implementation and proxy predicted addresses before deployment and skip existing bytecode where the intended deterministic deployment already exists. Rehearsals should verify Reward OApp address parity across mirror chains when a shared salt is intended.
