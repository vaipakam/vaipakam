...
- a token that does not publish a symbol or a decimal format must stay recoverable — amounts for it are entered and shown in raw smallest units, whole numbers only — but that treatment may only follow from the TOKEN declining to answer, never from a failed read. A network or node failure while reading those details must fail the lookup and say the token's details couldn't be read, so the user can retry. Silently taking a failed read as "no decimal format" would make an ordinary token accept an amount interpreted in its smallest units, so a user entering `1` would authorise a fraction of a token instead of one — a wrong amount the user cannot see in what they sign
