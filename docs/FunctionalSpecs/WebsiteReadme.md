...
- when a borrower creates an NFT-collateral offer and the lender side allows it, `Create Offer` may expose a `Borrow or sell` option that lists the NFT collateral through the supported marketplace path at offer creation time. The review should explain that whichever happens first wins: lender acceptance creates the loan, while a marketplace buyer consumes the offer by sale.
- lender-facing offer cards and accept-review surfaces should visibly identify when an NFT-collateral offer has a live parallel sale listing, including the ask style, current ask where available, and the fact that a buyer fill can consume the offer before lender acceptance.
