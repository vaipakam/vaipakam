...
- because that claim exists before anything is authorised, it must be released whenever the authorisation does not happen: a declined or failed signature prompt releases it, and so does a change of connected account or network while the prompt is open, since nothing has been sent in either case. Leaving it in place would lock the wallet out of recovery until the approval it never signed expired. Once the signature exists and the send has been handed to the wallet, the ordinary possibly-live treatment takes over and the claim may only be released by evidence
