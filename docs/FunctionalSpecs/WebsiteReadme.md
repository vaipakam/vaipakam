...
- the auto-lifecycle caps card should also make the keeper kill-switch visible at the point of configuration: when the connected holder's keeper cannot act (their keeper master switch is off, or they have approved no keeper), the card should warn that any enabled auto-refinance / auto-extend cap is inert until keeper access is restored, since those actions are keeper-executed — so an enabled cap is never mistaken for active protection.
