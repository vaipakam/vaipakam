...
- the protocol console must report how old its published values are beside the values themselves, and must never present an age it cannot determine as though it were one. A snapshot with no capture time, one carrying the indexer's unknown-capture sentinel, and one whose capture time lies in the future beyond ordinary clock difference are all reported as an unknown age with the values marked unverified; the `more than a day old` warning is reserved for a capture time that is genuinely that old. The age stated in the provenance line and the warning shown above it must never disagree, which requires both to be decided from the same clock reading.
...
- the connected app's wallet-connection SDKs (e.g. the WalletConnect and Coinbase Wallet connectors) must have their own third-party analytics / telemetry beacons disabled, so usage data the user never consented to is not phoned home — and locked-down networks are not spammed with failed analytics requests. This is not confined to opening the connect modal: an app that restores a previous wallet session on start-up constructs every configured connector to check for one, which is where each SDK's reporting begins, so the exposure otherwise covers every visitor. The setting must be stated explicitly rather than left absent — at least one of these SDKs treats a missing option as a request for its default, and that default is on — and it applies to every wallet-connecting app the project ships, not only the flagship one
...
- while the app is checking the connected wallet's Terms acceptance, gated app routes should show a neutral verification state rather than rendering the protected content
- if the acceptance read fails, the app should keep gated routes closed and show a clear retry state; a read failure must not be treated as either accepted or gate-disabled
...
- the Privacy page should describe only collection that a shipping client actually performs. Describing a collection path that no client exercises is a defect even though it over-discloses rather than under-discloses: a reader deciding whether to connect a wallet is owed an accurate account, and a policy that already claims a behaviour will not be re-checked when that behaviour is genuinely introduced. Where a described collection path is paused rather than removed, the page should say plainly that it is not currently collecting, and should keep the description of what such a record would contain — that text still governs records captured earlier, and must be accurate again before capture resumes
- a data-subject control that is live should be described as live even while the collection it acts on is paused, since the control still reaches records captured earlier
- "we do not collect X" must be scoped to the mechanism that stopped, never stated absolutely, whenever a consented path can still send the same data. Pausing automatic capture does not make a user-initiated attachment of the same payload untrue, and an absolute denial beside a live consented path is a worse inaccuracy than the over-disclosure it replaced
- where the app previews data before the user sends it, the policy should not claim the preview is complete unless it is. A preview that truncates or omits part of the outgoing payload should be described as a preview, with the fuller extent of what is sent stated, so consent is informed by what leaves rather than by what was shown
- a described capture scope should name the failure classes actually recorded rather than implying all of them. "Whenever something fails" is broader than the client supports, but so is any wording limited to crashed screens and *submitted transactions*: the sink also receives failures from write flows that never reach a chain, including a rejected typed-data signature and a failed off-chain post. Describe it as crashed screens plus write-flow failures, pre-submit and off-chain ones included, so a later rewrite of the policy from this spec cannot narrow the disclosure back
- an export presented as a copy of stored data should disclose any field the export itself generates. Where the file records its own origin, timestamp and the browser's user-agent string, that is an added browser and OS fingerprint the user carries when they share the file, and it should be named rather than covered by a categorical "this is your browser storage"
- an erasure control should name the categories it does NOT reach, not only the ones it does. Third-party libraries persist under their own key namespaces — a wallet-connection library's stored session being the case that matters here, since leaving it lets a reload restore the connection the user believed they had removed
- scope claims should distinguish per-origin browser storage from parent-domain cookies shared across the site's subdomains, and per-origin storage from per-tab storage. An export that cannot reach other tabs should not be described as covering the whole browser, and an erasure that does clear shared cookies should say so rather than resting on a blanket "per-origin" characterisation
- where an erasure reaches other tabs on a best-effort announcement, the policy should say so rather than claiming a device-wide guarantee. A tab that cannot receive the announcement keeps its own per-tab data, and a data-rights control must not promise a completeness its delivery mechanism does not provide
- the moment data leaves the device is the moment the policy must name, and it is not always the moment the user thinks they are deciding. Where a report is handed to a third party by opening a pre-filled link, the details travel in that link and reach the third party when the form opens — not when the user submits it. The disclosure should state the recipient and that timing, and should be clear that the remaining choice is whether to file, not whether the recipient receives
- where the policy invites the user to inspect something before deciding, it must point at a control that exists and that does not itself perform the disclosure. Telling a reader to review a payload at its destination is not an inspection opportunity when arriving at the destination is the transmission; the review step has to be a local one, and the policy should name it
- an erasure whose authority is established by a signature should disclose the scope that signature carries. A contract-wallet signature is verified on one network at a time, so the request reaches only that network's records and the user must repeat it elsewhere or ask for an out-of-band sweep; an ordinary wallet signature is not network-bound. A policy that describes only the second case misleads holders of the first
- the published Privacy page and the canonical Privacy Policy document are a hand-maintained mirror with no automated equality guard (unlike Terms versions, which are frozen byte-copies pinned by fingerprint). Both should be edited together, and a substantive change to what the policy claims should bump the policy version and effective date
- the Privacy page is not the only surface that makes privacy claims. The Data Rights page carries its own legal footer from the locale bundle, in every translated language, so a claim corrected in the policy but left there produces two live pages telling the same user opposite things about what is collected. A change to what the platform collects should sweep every user-facing surface that describes collection — translations included — rather than the policy alone
...
- testnet network behavior, including user-facing mirror testnets only after their oracle, liquidation-route, deployment-artifact, indexer, and wallet-switch metadata are complete enough for ordinary lending and risk flows
- create offer flows
- accept offer flows
...
- the issue / diagnostics drawer should include a live-updates section that reports whether the app is using push delivery or polling fallback, the age of the last push signal, session reconnect count, the effective poll interval, whether that interval is push-backed, and the measured delay from push signal to refreshed data
- live-update diagnostics should reset on chain switch so reconnect counts, latency, and freshness state from the previous chain are never attributed to the newly selected chain
- when the realtime push channel is healthy, ordinary watermark polling should relax to a backstop interval; disconnects, fallbacks, and chain switches should immediately restore the normal page cadence. Push signals are invalidation hints only, and authoritative data still comes from the normal indexed or direct-chain reads.
- `Offer Book` should be wallet-gated inside the connected app; after connection it should keep market browsing filterable by side, asset, status, liquidity, duration, and per-side count; market-rate annotations should use a filter-scoped recent-acceptance anchor with signed deltas and a mobile-friendly explanatory tooltip
...
- Basic mode should show newest-first market rows with plain language and minimal controls. Advanced mode may reveal side filters, asset filters, rate and duration sorting, exact rate, offer identity, expiry, range bounds, and partial-repay status. A filter that matches nothing should say that no row matches the filter, not imply the whole market is empty.
- offer rows should exclude self-authored offers from direct-accept pickers for the connected wallet, and accept review should always bind the signed terms to the specific visible offer and active chain before any approval or transaction is requested
...
- when depth-tiered LTV is enabled, Create Offer and accept-review risk previews should show the collateral's on-chain tier, effective tier after the keeper confidence floor, applicable tier max-init-LTV, whether that value came from a fresh peer-derived cache or the governance-configured per-tier max-init-LTV cap fallback (defaults `50%` / `60%` / `65%`), cache age, and whether the requested principal exceeds the active tier cap. While the master switch is disabled, the UI should continue to present the existing conservative LTV / Health Factor rules and may show tier data only as non-binding diagnostics.
...
- An empty activity feed must not overstate what it knows. Because the feed is scanned from the busy protocol-wide event stream, the scan legitimately stops before the feed's end whenever the network holds more events than one scan window; in that case the page cannot prove the wallet has no history, so it must not claim a definitive "no activity yet" (which would be false for a returning wallet whose only positions are closed or transferred and older than the scan window). Instead it states the page's recent-only scope — that it lists recent activity and anything older is not shown here — wording that is true whether or not the wallet has older history. A definitive "no activity yet" is shown only when the whole feed was scanned to its end. Every empty variant offers the same first-move Borrow / Lend calls to action.
...
- No empty or failing state may dead-end: an empty vault, claim list, rental market, or a failing balance check each offer the concrete next step (on test networks, that includes the faucet), and a successful faucet mint offers the next hop into borrowing or lending. Empty states across the guided flows read consistently — the same icon-and-heading treatment, never bespoke bare text.
- An action's label states the money direction it commits the user to: taking a lender's offer is presented as borrowing, and taking a borrow request is presented as funding it — never a single direction-blind "use this offer" that hides which side of the loan the user ends up on.
- The wallet requirement is surfaced at the start of a guided flow, not only at the final review: a disconnected user is prompted to connect up front (without being blocked from browsing), so "connect a wallet" is never a last-step surprise. After successfully posting an offer, the user can start another without leaving the flow.
- On a position's detail page, while the app is still confirming whether the connected wallet holds the position, the action area shows a clearly disabled "confirming your role" placeholder rather than nothing, so the primary action never silently disappears mid-read.
...
- The app must show signs of life immediately on a cold load, even on a slow connection: a lightweight branded loading state paints from the initial HTML before the application code runs, and is replaced once the app is ready — a blank white screen during startup is a defect. Screens beyond the common entry points may load on demand behind an in-shell loading state. The startup splash must not strand the user indefinitely when the application code fails to load at all (a dropped bundle chunk, a stalled network): after a bounded wait it stops implying progress and offers a plain-language "taking longer than expected — check your connection and reload" hint with a reload action. This pre-app fallback is distinct from the post-load recovery card, which can only appear once the application has already booted.
...
- the in-app logo should route to the connected app's dashboard index so connected users return to the dashboard shell; the public navbar logo should continue to route to the public landing page
...
- Testnet-only faucet access should be shown only when the active chain is a testnet and its deployment metadata publishes faucet assets. The faucet should let connected reviewers mint the configured test assets needed for demo flows: at least two oracle-priced (liquid) test tokens so a deal can carry a different liquid token on each side, and at least two unpriced (illiquid) test tokens so a deal where NEITHER side has a price is testable (dual-consent, no health factor, in-kind default hand-over), plus the rentable test NFTs. On mainnet routes, or on testnets without the required metadata, the surface should stay unavailable rather than exposing unrestricted mint controls.
- Tokens outside the curated list must pass an independent contract-security screen before a deal that carries them can be accepted: a token flagged as unsellable or booby-trapped (honeypot, sell restrictions, counterfeit of a well-known token, self-destruct capability, per-address tax control, owner blacklist-plus-pause, punitive buy/sell taxes, or any fee on plain transfers — the protocol's vault accounting cannot absorb fee-on-transfer tokens) blocks acceptance with a plain-language reason, and softer owner-power and structural findings (upgradeable proxy, hidden or reclaimable ownership, owner-modifiable taxes or limits, anti-whale limits, cooldowns, whitelists, minting, balance rewrites) are disclosed as warnings the user must knowingly proceed past. "Couldn't verify" is never treated as clean: an unverified contract source or an unevaluated honeypot check blocks acceptance, secondary trade checks or taxes the screen could not evaluate are disclosed as warnings rather than assumed clear, and a screening-service outage holds acceptance back while offering a working retry (the check re-probes on its own and the review exposes a manual retry). Consent given against disclosed warnings is re-collected whenever the disclosed content changes. The screen re-verifies at signing time: a flag or a previously undisclosed warning landing after review aborts before any signature, and the review then shows the new finding so consent is always given against current knowledge. Curated tokens are pre-vetted and exempt; networks the screen does not cover (test networks) show a notice instead of a block. Pasting an unknown token address surfaces the same screen's verdict at entry, and posting an offer is gated and re-verified at signing time the same way accepting one is — the creator's own deposit is exposed to the same token risks. On networks with market data, the paste entry additionally shows a market-listing SOFT signal beside the security verdict: a listed token's name, symbol, and market rank (identity confirmation against paste mistakes), a caution for listed tokens outside the top 200, and a verify-the-contract prompt for addresses the market doesn't know at all. The listing line never blocks or gates anything, stays silent when its own lookup fails, and is absent on test networks — where no test token has a listing and the line would train users to ignore it. The screen also warns while the user is still BROWSING: Offer Book rows and guided-match cards carry a compact badge when a non-curated token in the offer has a concrete finding (danger, caution, or could-not-be-screened), and the guided matcher withholds danger-flagged offers from its shortlist while stating how many it hid — a thinner list must never pass silently as the whole market. The browse tier is early warning only: the book never hides rows, a still-loading screen shows no badge rather than a false alarm, and enforcement stays at the accept review. Screening a page of offers happens as one batched lookup, and verdicts are shared between the browsing badges and the review gates so each token is screened once, whichever surface asked first.
- Users can opt into off-site alerts (Telegram, with a Push Protocol channel as an alternative rail) framed as plain-language outcomes — a message before an interest payment comes due, a message when a loan gets risky — with sensible fixed risk thresholds behind the toggles; raw threshold numbers are an advanced-mode control. Every offered toggle is a real control: an opt-out must be stored and honored by EVERY detector that delivers that outcome (the due-date opt-out covers both the interest-payment reminder and the pre-grace warning), a stored preference changes only when the user actually flips its control — a device without a server read-back must not overwrite stored preferences with its local defaults, and alert outcomes that have no working delivery path are not offered. Turning the risk toggle off still leaves a final pre-liquidation warning, and the surface says so. Starting a Telegram link, unlinking, sending a test alert, and switching a warning lane off (such as the due-date reminder opt-out) each require a free wallet signature proving ownership of the wallet — the alert service must refuse to issue a link code, clear a link, send a test message, or store an opt-out without it, so nobody can point another wallet's alert stream at their own chat, spam another wallet's chat, or silently switch off another wallet's warnings. A Telegram link is confirmed by a real delivery round-trip, not the user's word: after receiving the handshake code the user triggers a test alert, the alert service sends one actual "your alerts are working" message to the linked chat, and the surface treats the wallet as linked only once that send succeeds — a handshake that never reached the bot (no stored chat) is reported plainly and the wallet stays unlinked, rather than being marked linked and silently dropping every later alert. The setup states in one honest sentence what linking stores (wallet address, alert preferences, chat id, and a small per-alert delivery record — which loan, which level, when — kept so the same event is never delivered twice) and provides a working unlink, presented as a clearly-labelled control, that remains reachable even when the link was made on another device. Enabling Push delivery records the opt-in on the alert service in addition to linking out to the channel. In builds with no alerts backend configured, the surface says so and sends nothing rather than defaulting to another environment's backend. A borrower viewing an active loan is nudged, in one line, toward the alert setup.
- the website should expose the self-sovereign risk-access surface (see ProjectDetailsREADME "Progressive Risk Access") as a dedicated wallet-gated settings page reachable from Settings: it shows the vault's EFFECTIVE risk level (read-time re-locked) in plain language, lets the user choose among the three levels with honest per-level explanations of what each additionally allows (including that illiquid pairs settle in-kind on default), states truthfully whether the choice is currently enforced on the active network (the retail deploy's master switch is off — the page must say the choice is saved but not yet enforced, never imply active enforcement), distinguishes a held-but-not-yet-active level as cooling-down versus stale-after-a-terms-change ONLY from trustworthy on-chain reads (offering the one-click in-place re-affirm only for the stale case, since re-submitting a merely-cooling level restarts its cooldown; when either supporting read fails the page says it cannot tell and suggests the lower-then-re-raise fallback), never renders the level controls over a failed tier read (a wrong default could drive a cooldown-restarting write), and exposes the strict-mode state with the disable-linger note (a recent turn-off keeps the mid-tier acknowledgement requirement in force through the configured cooldown; a failed linger read says "couldn't check" rather than implying absence). A surface that cannot yet collect the per-deal mid-tier acknowledgement strict mode demands must NOT offer enabling strict mode (that would lock the user out of their own mid-tier deals once enforcement is on) — it explains why enabling is unavailable while always offering the disable as the recovery path for a vault that enabled strict mode on another surface — presented honestly as a risk-increasing change (it removes the extra per-pair acknowledgement requirement), whose disable-linger cooldown keeps that requirement in force for the configured window. Per-pair illiquid consent and the strict-mode mid-tier acknowledgement remain contextual in the create/accept flows — the page owns only the two global controls
- Operators can disable individual position-opening write flows (posting offers, accepting offers, listing NFTs, renting NFTs, depositing VPFI) via deploy configuration during an incident; a disabled flow explains itself in plain words and states that repayments, claims, and withdrawals are unaffected. Close-out flows are structurally outside the switch's reach — an operator precaution must never trap funds or cause missed deadlines (the same entry-closes/exit-stays-open tier principle the sanctions gate follows).
- Before asking the wallet to sign, the review step should rehearse the exact pending transaction as a free read-only dry run and surface the verdict in plain words — passed, would fail (with the reason and a statement that nothing was sent), or a benign note when the only blocker is an approval the submission grants first. The rehearsal is advisory only: it must never disable signing, must say so quietly when no verdict is available, and must not be shown for transactions whose content only exists at signing time (signed attestations, state written earlier in the same submission) — a preview that routinely cries wolf trains users to ignore real warnings.
- Before any multi-step signing sequence begins (posting an offer, accepting a deal), the review screen must state how many wallet confirmations the submission will take and what each one is for — including when a token approval needs an extra reset confirmation — and while the sequence runs, the submit control must show which step the user is on rather than a single undifferentiated waiting state. The count must reflect live state: an approval already in place drops out of the list.
- A page that fails to render must never leave the user on a blank screen: the app shows a recovery card in place of the failed page — stating plainly that the fault is display-side, that funds and on-chain positions are unaffected, and that a just-signed transaction may still have gone through — with reload and go-home actions, while the surrounding navigation stays usable. Navigating to another page recovers without a full reload.
...
- keeper and operator surfaces should show tier-LTV cache freshness, peer-derived values, the governance-configured per-tier fallback caps, refresh rejection reasons, and whether the cache has passed the stale-warning threshold so humans can refresh before loan initiation falls back to those caps.
...
- **M2 PR-1 supersession note (D1 decided (b); see TokenomicsTechSpec §6/§6b):** for **new** loans the borrower fee model is the two-mode **HoldOnly** (tier discount delivered as a direct reduction of the lending-asset LIF — no VPFI moved) / **Full** (an optional per-party native-VPFI **tariff** paid into the recycle bucket for an extra own-side discount) model, not the "pay the full `0.1%` LIF up front in VPFI → rebate" custody model. The up-front-VPFI-with-rebate copy in the bullets below describes the **legacy / open-loan (grandfathered) path ONLY** — it must **not** be shown for new loans. Once the HoldOnly/Full backend lands, new-loan copy must follow the two-mode model (HoldOnly moves no VPFI; Full is a non-refundable tariff with no rebate) even before the dedicated M2 frontend copy card (#1355) ships the full treatment; there is no window in which a new borrower is told they pay the full `0.1%` LIF in VPFI and earn a rebate. **Copy rule for the Full tariff:** it is a fee (a native-VPFI quantity), never framed as buying/redeeming VPFI, a token price, a yield, or a rebate on itself — the same no-purchase-price, no-APR discipline as the deposit/hold copy above.
...
- when proceeds are owed to a flagged wallet, the UI should describe them as locked in that wallet's Vaipakam Vault until the flag clears, not as withdrawable or lost
- when a flagged wallet owns a Vaipakam position NFT, the UI should explain that the position is frozen in place: it cannot be transferred or claimed by that wallet while flagged, but protocol settlement may still complete so clean counterparties are not stranded
...
- the route should not appear in the main nav, footer, Dashboard shortcuts, Asset Viewer actions, or basic user guide; the Advanced User Guide — or an equivalent in-app explainer section (e.g. a Help-page card) that first explains the dust-poisoning risk — may deep-link to it for users who already understand the risk. Because the Advanced User Guide is served from the marketing site and the recovery route lives on the connected app, that deep link must be the recovery page's full address on the connected app — a path relative to the guide's own site would be a dead link, leaving a reader who followed the app's link out to the guide with no way back into the flow. Every language edition of the guide must carry the same working link
...
- a token that does not publish a symbol or a decimal format must stay recoverable — amounts for it are entered and shown in raw smallest units, whole numbers only — but that treatment may only follow from a POSITIVELY RECOGNISED answer by the token itself — it declined, returned nothing, or answered in a format that cannot be read — and never from a failed read or an unrecognised failure. Any failure to reach an answer (network unreachable, and equally a node refusing or erroring on the request, such as a rate limit) must fail the lookup and say the token's details couldn't be read, so the user can retry. Silently taking a failed read as "no decimal format" would make an ordinary token accept an amount interpreted in its smallest units, so a user entering `1` would authorise a fraction of a token instead of one — a wrong amount the user cannot see in what they sign
- "declining to answer" covers every answer the app cannot use, not only a refusal to answer: some long-standing tokens publish their symbol in an older format the current standard replaced, and the network returns it perfectly well even though the app cannot read it as a name. Because these details are optional decoration, any answer the network delivers — however unusable — may fall back to the shortened token address for a symbol and to raw smallest units for a decimal format. Only a failure to REACH the network keeps the stricter treatment above, since that alone says nothing about the token. Where the older symbol format is the cause, the app should prefer reading it in that format and showing the real ticker, falling back to the shortened address only if that read fails too; doing so must not slow down a token that publishes its symbol normally
...
- the blocked outcome must be described in full, not as a limit on this page: declaring a sanctions-listed sender flags the WALLET and blocks every new-position action across the protocol — creating or accepting offers, deposits, recovery and the like — while existing loans stay repayable and closable, and the block lifts by itself if that address is later removed from the sanctions list. Because outcome cards are deliberately shown ahead of every other check, this card is the only status the user gets after such a transaction: a wording that mentions only the recovery page leaves them to discover the wallet-wide block by hitting it elsewhere with no explanation. The same three facts should be worded consistently wherever the risk is stated in advance (the standing warning on the form, the in-app explainer) so the surfaces do not appear to describe different consequences
- a result must be described from what the transaction actually recorded, not from what the user submitted. The amount, the token and the declared sender shown on a result — and repeated in any success or blocked-recovery wording — should come from the outcome the transaction emitted, using the submitted details only for what that outcome does not carry. This matters because a wallet may replace a transaction with one carrying different values, and reporting the submitted values beside a link to a transaction showing different ones is a false statement, not merely a stale one. If the recorded outcome names a token the app holds no decimal format or symbol for, it should say so and state the amount in that token's smallest units next to its address, never format it with another token's decimals
- because the signed declaration states the user has read the Advanced User Guide's stuck-token-recovery section, the confirmation step should link that section directly, and the in-app explainer should link it too
- if the user's wallet replaces the submitted transaction (a speed-up or a cancel), the app should follow the replacement: every reported outcome and transaction link should name the transaction that actually mined. How that replacement is reported depends on what the wallet actually did, and only a cancellation is definite: a cancellation that mines without performing a recovery should be reported plainly as "nothing was recovered". A mere fee bump is not that — it re-sends the same instructions at a higher price, so the transaction that mines is still the recovery. Neither is a replacement that merely carries different instructions: "different instructions" also describes a second recovery for a different token or amount, so a replacement the app cannot decode a result from is an unreadable outcome, never a statement that nothing happened
- every settled result must offer a plain way to begin a new recovery, because one wallet can hold unsolicited tokens from more than one sender and the page stays open on the result it just produced. A completed recovery is a settled result for this purpose: it was read from what the transaction recorded, its remembered record is already forgotten, and a fresh attempt reads the remaining recoverable amount and is authorised on its own. The single deliberate exception is the blocked outcome — a wallet flagged that way is barred from new positions protocol-wide until the declared address is de-listed, so offering a fresh form there could only walk the user into another attempt that cannot succeed; that card ends with the standing explanation and the note that the block lifts by itself. The processed-but-unreadable result keeps its own two-step way out described above
- once a recovery transaction has been broadcast, the app must not offer an action that discards its identity while its result is still unknown; the only way forward from an unconfirmed submission should be re-checking that same transaction, and only a resolved result — recovered, blocked, rejected, or confirmed-but-undecodable — may unlock starting a fresh recovery, so a transaction that quietly succeeded cannot be recovered a second time
- an unresolved submission must survive leaving the page: the app should remember it against the wallet and network that sent it, so a reload, an account switch, or a network switch returns to the same unresolved-submission state instead of a blank form that invites a second recovery, and should forget it as soon as the result is known. A remembered submission belongs to one wallet on one network and must never be shown to any other
- every result card belongs to the wallet and network it was produced for, and the app must stop showing it the instant the connected wallet or network differs — in the same redraw, not once some later tidying-up runs. Result cards are deliberately shown ahead of every other check on the page, so that a wallet blocked BY its own recovery still gets the explanation rather than a generic block; that ordering means a direct switch from one account to another, or from one network to another, would otherwise redraw the previous wallet's result for the new one. The transaction link on such a card must also be built from the network the card itself belongs to, never from whichever network happens to be selected when it is drawn — pointing a real transaction at another network's explorer states something untrue about it. A result the app suppresses this way must read as though there were no result at all: the user lands on the ordinary starting state for the wallet they just switched to
- re-checking an unconfirmed submission must cope with a wallet that replaced it: looking up the original submission can then never succeed, so when it cannot be found the app should decide from the user's own on-chain recovery counter, which advances exactly once per processed attempt whether that attempt recovered the tokens or was blocked. If it advanced, the app should state plainly that an attempt was processed — without claiming which of the two results it had, because it cannot read that — and must not unlock a fresh recovery. If it has not advanced, "nothing was recovered" may only be stated on positive evidence, never on a failed reading: the network must positively answer that it does not hold the submission (a look-up that errors or goes unanswered is not an answer), and the approval the user signed must have expired, after which that submission can never be accepted again. Until both hold, the submission stays unresolved and the app should say when it will be able to give a definite answer rather than guess at one. Anything the app cannot read must leave the submission unresolved. A counter that reads BELOW the value the attempt was authorised against is one of those unreadable cases and not an unchanged counter: that counter can only ever advance, so a lower reading is proof the network answered inconsistently — a stale or lagging reply — and says nothing about the attempt. The app must keep the submission unresolved and keep its remembered record in that case, never concluding that nothing was recovered
- a "an attempt was processed" verdict is a lasting lock, not a notice that disappears on the next page view: that attempt may have moved only part of the stuck balance, so the app must remember the verdict against the same wallet and network, re-present it after a reload, and never re-open a fresh recovery by itself. The only way past it is an explicit two-step acknowledgement by the user — a first action confirming they have checked their wallet, then a separate confirmation stating that the earlier attempt already used up its approval and that anything from here is a new, separate recovery limited to what is still in the vault
- because recovery is authorised by an ordinary wallet signature that the protocol re-derives and compares against the sender, a smart-contract account (such as a Safe) cannot complete one; the app should say so before the form rather than after a typed confirmation and a doomed transaction. A failed check of the account type must not block an ordinary wallet — that check may only add a block, never remove one
- the account-type verdict must describe the wallet that is connected NOW. It belongs to the account and network it was read for, and while the connected account or network differs from that, the app must show its ordinary "checking" state rather than the previous account's answer — otherwise switching from an ordinary wallet to a smart-contract one presents the form to an account that can never complete a recovery, for as long as the fresh check takes to answer
- an ordinary wallet that has delegated its behaviour to a smart-account programme (the account-abstraction upgrade an everyday wallet can opt into without changing its address) is still an ordinary wallet for this purpose: it signs with the same key, so recovery works and the app must allow it. Only an account that is a contract in its own right may be blocked — the account-type check must recognise the delegation and not mistake it for a contract account
- the moment the user's signature exists and the send has been handed to the wallet, the app must treat the attempt as possibly-live even before the wallet names a transaction. A wallet that accepts the send and never answers leaves a recovery that may well be on its way, so a failure at that point must land on the unresolved-submission state — described honestly as "we don't know whether this was sent", with no transaction link to offer and the user pointed at their wallet's own activity list — and never back on a re-armed confirmation card. Re-checking such an attempt cannot look anything up by transaction, so it should decide from the user's on-chain recovery counter and the expiry of the signed approval alone: an advanced counter is the same lasting lock as any other processed attempt, an unchanged counter past the expiry means it can never go through, and anything else leaves it unresolved
- the app must not let a second recovery be signed while one is already outstanding for the same wallet and network, including from another browser tab. It should re-read the remembered submission immediately before asking for the signature and, if one exists, refuse to sign and present that submission instead; a tab that is sitting on the form or the confirmation step should also pick up a submission another tab records, so both tabs show the same state
- reading before the signature is not enough on its own: the app must CLAIM the outstanding-recovery record before the signature prompt opens, not after it returns. Two tabs can otherwise both find nothing and both proceed, and — because both attempts are authorised against the same approval counter — the one that finishes second can wipe the record protecting the other's live transaction. Each attempt should therefore carry its own identity, recorded with the claim, and every later clearing or updating of the record must apply only to the record still carrying that same attempt's identity. A record left by an older version of the app that has no such identity must still be recognised and resolved rather than stranded
- claiming early is still not enough unless the claim ELECTS a single claimant: looking for an outstanding attempt and then recording one is two separate steps over a store every tab of the site shares, so two tabs can interleave between them and both believe they claimed it. Per-attempt identities only make that collision survivable after the fact — they never decide who won. The claim must therefore be taken under a lock that spans every tab of the site for that wallet and network, held across the look-and-record pair alone. A tab that finds the lock already held must be told an attempt is in flight — the same answer as finding a recorded attempt — and must not sign. The lock must never be held across the wallet prompt, which the user may leave open indefinitely; the recorded claim is what protects everything after it. Where the browser offers no such lock, the app should record its claim and then read it back before signing, and a tab that reads back somebody else's attempt must stop without signing
- because that claim exists before anything is authorised, it must be released whenever the app OBSERVES that the authorisation did not happen: a signature prompt that comes back declined or failed releases it, and so does a prompt that comes back to find the connected account or network changed in the meantime, since nothing has been sent under the identity that made the claim in either case. Leaving it in place in those cases would lock the wallet out of recovery until an approval it never signed expired. A prompt the user simply abandons — never answered, or still open when the tab is closed — is a different case and must keep its claim: nothing came back from the wallet, so it cannot be known that nothing was signed, and the claim is left for the ordinary re-check to resolve at the expiry of the approval. Once the signature exists and the send has been handed to the wallet, the ordinary possibly-live treatment takes over and the claim may only be released by evidence
- a card describing a CONFIRMED transaction whose outcome could not be read must not tell the user to avoid acting while offering an action: this state is settled, starting over is deliberately permitted here, and the wording must say so. It should point the user at the transaction to see what happened and describe the action as beginning a separate new recovery limited to what is still in the vault. The warning against acting belongs only to a submission that has NOT confirmed, where it is true
- when the app resolves a submission and forgets it, it must forget only that submission — never a newer one recorded since (for example by another tab), because discarding that would silently disarm the protection the newer submission is under
- the reverse also holds: when another tab REMOVES the shared record — because it settled the attempt, or because the user declined the signature there — a tab still showing that attempt as unresolved must release it and return to a fresh form. Otherwise the user is left looking at a card describing something that no longer exists, and for an attempt declined before any transaction was ever sent there is nothing for "check again" to find, so the card would sit there until the signed approval expired. The release must be narrow: it applies only when the removed record is the attempt the card describes (a newer attempt must be left untouched), and only to an unresolved card — a settled result the user still needs to read, whether it succeeded, was blocked, or recorded a processed attempt, must never be cleared away by another tab tidying storage
- the release must also hold when the two pieces of news arrive together: another tab can record an attempt and remove it a moment later — a signature declined as soon as the wallet opened — and both changes reach this tab before it has had a chance to redraw. The tab must decide the release from the state it has just taken on, not from the state it was last drawn in, or it skips the release and is left showing a card for an attempt that was already withdrawn
- that release must be decided from what the CHANGE itself reports — which attempt left the shared record, and which one (if any) replaced it — never from re-reading the record after the fact. A tab in the background may only get to the news once the other tab has already removed the displayed attempt AND claimed a new one, and a re-read then sees only the newer attempt: the tab concludes the record is still occupied, keeps the stale card, and does the same when the news of the new attempt follows — stranding it permanently on an attempt that no longer exists. Releasing what left must also come BEFORE adopting what replaced it, or the stale card blocks the newer one from being picked up. All the narrowness above still applies to the release, and a change that names an attempt other than the one on screen must still leave the card alone
- if the browser refuses to store the remembered submission at all (private browsing, storage disabled or full), the app must say so plainly on the card the user is looking at rather than behave as if the record exists. It should not block the recovery, but it should state that closing or reloading the tab will lose the app's ability to pick the attempt back up, and that the user should check their wallet's activity before trying again
- when a re-check resolves to a rejected transaction and the app no longer holds the details the user originally reviewed — the usual case after a reload or an account switch — it should return the user to a fresh, usable form carrying the rejection reason, never to a confirmation card with nothing in it
...
- the last-moment re-screen of the connected wallet itself must fail CLOSED, exactly as the availability check does: recovery's outcome is decided by the screening service, so a wallet-specific screening query the app cannot read is not permission to sign. An unreadable screen should leave the user in the same retryable blocked state an unreachable availability check produces, while a wallet the service positively flags is reported as blocked
- that fail-safe blocked state should distinguish a screening service that is confirmed absent on the network from one the app simply could not reach. Both keep the form closed, but the first is a permanent property of the network and should be stated as such, while the second is a passing failure: it should say the check did not answer, retry a bounded number of times on its own, and offer a manual retry, so a momentary read failure does not read as a permanent verdict or require a page reload to clear. The same distinction must survive the last-moment re-check the app makes immediately before asking for the signature: neither answer may proceed to sign, but a service that did not answer must be reported as a retryable failure and leave the retry route open, while only a confirmed-absent service may be described as permanently unavailable here
...
- when the indexing service offers a realtime push channel, the app should subscribe to it and refresh the indexed market/history views within seconds of an ingest write, with the ordinary polling cadence always running underneath as the fallback; the push carries only a change signal, never authoritative data, and its absence must leave the app exactly as fresh as polling alone.
...
- staging note for the three-way verdict above: the burned-vs-never-minted distinction requires an on-chain mint-counter view the Diamond does not expose yet. Until that contract-level follow-up ships, verifier surfaces may state a two-way verdict — live, or "doesn't currently exist: either retired by the protocol or never minted, the network doesn't record which" — rather than guessing. "Retired by the protocol" is deliberately generic: a position token stops existing for more than one reason, and without a history lookup the verifier must not claim to know which. Once the mint-counter view exists, verifier surfaces adopt the full three-way distinction, and a fuller history lookup may then also name the retirement reason.
...
- the app must not stream chain-RPC polling: block-driven live refresh runs only over push (WebSocket) transports, and on HTTP-only deploys freshness rides the ordinary periodic refresh, the instant refresh after the user's own actions, and the indexer push channel. A hidden tab holds no chain subscription and drives no refreshes; a visible session with no interaction for a couple of minutes stretches its periodic refreshes several-fold and snaps back on the next interaction. Values that cannot change within a session (deploy-static addresses) are read at most once per session per network
- ENS reverse-name display must resolve against explicitly configured Ethereum endpoints (operator-overridable, with a public fallback behind the primary), never a chain library's implicit default endpoint; each address's name is resolved at most once per session, a failed lookup is not retried in a loop, and a failed or absent lookup always falls back to the shortened hex form
...
- on test networks, asset pickers must list the faucet's test tokens as first-class choices (clearly badged as test tokens, never mistakable for real assets) so a user who just minted from the faucet page never has to paste a contract address back by hand; mainnet pickers are unaffected
- dropdown controls should present options as designed menu rows (supporting a secondary line and badges, marking the current choice, themed for light and dark) rather than the browser's stock option list, while preserving native-equivalent keyboard interaction (arrow navigation, typeahead, Enter/Space select, Escape close) and standard combobox/listbox semantics for assistive technology
...
- interest-rate formatting in user-facing screens should be shown as percentages (`%`) rather than raw basis points (`BPS`) everywhere a rate renders as visible text — including advanced-mode detail lines (owner decision 2026-07-22, superseding the earlier advanced-view carve-out); an integer basis-point rate divides into at most two decimal places, so the percent form is exact. Raw basis points may still appear in hover / focus tooltips on trader-oriented surfaces (e.g. the Rate Desk's exact-value tooltips) and in explanatory "rates are stored in basis points" notes, but never as the primary visible value
...
- on the create-offer page, the loan-duration input must be restricted to a minimum of `1` day and the **live configured protocol maximum** read from protocol configuration — `365` days at launch by default, with governance headroom up to approximately `4385` days — rather than a hardcoded `365`
- if the user enters a duration outside that supported range, the page should flag it inline with a validation message derived from the live configured maximum (e.g., `Enter "Duration between 1 and 365"` while the launch default maximum of `365` is active), not a hardcoded upper bound
