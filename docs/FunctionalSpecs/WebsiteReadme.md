...
- for an active loan with an illiquid / no-oracle lending or collateral leg, `Loan Details` should keep the in-kind settlement risk visible for the life of the loan, not only at offer time: alongside the existing risk explainer it should show a prominent warning that on default the lender receives the raw collateral asset itself — not the lending asset, and regardless of market value — with no DEX swap and no LTV-based liquidation, so the downside is impossible to miss after the position is opened
...
- for a live NFT whose underlying loan settles in-kind on default — an NFT-collateral loan, or any loan with an illiquid / no-oracle lending or collateral leg — the verifier should disclose that settlement caveat: a labelled `Settlement on default` line distinguishing the liquid (collateral is swapped to the lending asset) from the in-kind (collateral transferred as-is) case, and a prominent warning that the position pays the raw collateral asset rather than the lending asset, regardless of market value, so a prospective NFT buyer sees the downside before acquiring the position
