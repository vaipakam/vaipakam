...
- the interest-implication copy on the Direct-preclose and refinance screens should reflect the affected loan's actual interest mode rather than always assuming full-term: a full-term loan owes interest as if it ran to maturity, but a pro-rata loan owes only interest accrued to date, and the screens (including the explicit old-lender-payout sentence on refinance) should not overstate the cost of exiting a pro-rata loan
- the full-term-vs-pro-rata interest mode should also be visible at a glance, not only inside the pre-signature warnings: Offer Book rows/cards should carry a small interest-mode chip next to the rate, Loan Details should show the mode in the loan terms for the life of the loan, and the swap-to-repay surface should disclose it because a full close honours the loan's mode. The indicator applies only to ERC-20 principal offers/loans, where the distinction is meaningful, and should be omitted otherwise
