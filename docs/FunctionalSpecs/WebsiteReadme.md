...
- The lender can instead LIST the position for sale at a rate of
  ...
  listing form may be shown for supported ERC-20-collateral loans once
  the active deployment exposes the complete listing lifecycle. The
  buyer review must quote the live principal and live collateral floor
  rather than stale listing-time facts, show that no fresh origination
  fee is charged on a secondary lender-position sale, block the
  current borrower from buying their own lender side, and stop before
  signing if the linked loan is no longer active.
