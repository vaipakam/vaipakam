...
- Users can opt into off-site alerts (Telegram, with a Push Protocol channel as an alternative rail) framed as plain-language outcomes — a message before a repayment or interest deadline, a message when a loan gets risky — with sensible fixed risk thresholds behind the toggles; raw threshold numbers are an advanced-mode control. Turning the risk toggle off still leaves a final pre-liquidation warning, and the surface says so. The setup states in one honest sentence what linking stores (wallet address and chat id, together, on the alert service) and provides a working unlink. Alert outcomes that have no working delivery path are not offered. In builds with no alerts backend configured, the surface says so and sends nothing rather than defaulting to another environment's backend. A borrower viewing an active loan is nudged, in one line, toward the alert setup.
