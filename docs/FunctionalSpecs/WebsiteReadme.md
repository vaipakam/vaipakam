...
- refinance is the exception: its old-lender payoff is computed full-term on-chain regardless of the loan's stored interest mode, so the refinance screen must ALWAYS disclose full-term interest (plus any rate shortfall) and must never switch to pro-rata copy, so the pre-sign disclosure matches what the transaction actually pulls
