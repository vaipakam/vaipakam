...
- that fail-safe blocked state should distinguish a screening service that is confirmed absent on the network from one the app simply could not reach. Both keep the form closed, but the first is a permanent property of the network and should be stated as such, while the second is a passing failure: it should say the check did not answer, retry a bounded number of times on its own, and offer a manual retry, so a momentary read failure does not read as a permanent verdict or require a page reload to clear
