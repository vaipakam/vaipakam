...
- the auto-lifecycle caps card should also make the keeper kill-switch visible at the point of configuration: when the connected holder's master keeper switch is off — an unambiguous hard gate that blocks every keeper action for that user regardless of which keeper or action permissions exist — the card should warn that any enabled auto-refinance / auto-extend cap is inert until keeper access is restored, since those actions are keeper-executed, so an enabled cap is never mistaken for active protection. (The finer per-keeper action-permission detail is surfaced by the per-keeper toggles on the same page rather than inferred here.)
