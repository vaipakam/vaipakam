...
- An empty activity feed must not hedge for a wallet that has simply never acted. Because the feed is scanned from the busy protocol-wide event stream, the scan legitimately stops before the feed's end whenever the network holds more events than one scan window — but that truncation only means "older history may exist" for a wallet that actually holds positions. A wallet with no loans reads the plain "no activity yet" empty state (with its first-move Borrow / Lend calls to action); the hedged "older events may exist that we couldn't scan" wording is reserved for a wallet that does hold loans whose history falls outside the recent scan window.
