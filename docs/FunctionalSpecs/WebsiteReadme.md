...
- Users can opt into off-site alerts (Telegram, with a Push Protocol channel as an alternative rail) framed as plain-language outcomes — a message before an interest payment comes due, a message when a loan gets risky — with sensible fixed risk thresholds behind the toggles; raw threshold numbers are an advanced-mode control. Every offered toggle is a real control: an opt-out must be stored and honored by the delivering detector, and alert outcomes that have no working delivery path are not offered. Turning the risk toggle off still leaves a final pre-liquidation warning, and the surface says so. Starting a Telegram link requires a free wallet signature proving ownership of the wallet — the alert service must refuse to issue a link code without it, so nobody can point another wallet's alert stream at their own chat. The setup states in one honest sentence what linking stores (wallet address, alert preferences, chat id, and a small per-alert delivery record — which loan, which level, when — kept so the same event is never delivered twice) and provides a working unlink that remains reachable even when the link was made on another device. Enabling Push delivery records the opt-in on the alert service in addition to linking out to the channel. In builds with no alerts backend configured, the surface says so and sends nothing rather than defaulting to another environment's backend. A borrower viewing an active loan is nudged, in one line, toward the alert setup.
