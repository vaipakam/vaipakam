...
- The sender must be the configured authorised CCIP messenger. The authentication split is: `CcipMessenger` validates that the CCIP message arrived from the **messenger** it has allowlisted for the source chain (`setRemoteMessenger`, on top of the router's own sender authentication) BEFORE forwarding the payload to `VaipakamRewardMessenger`; the Diamond's tier ingress facet then only checks that the immediate caller is the registered `rewardMessenger` contract on this chain. There is no second channel-peer check at the Diamond boundary.

  > **Correction (#1631).** This paragraph previously said `CcipMessenger` validates the CCIP **channel peer** — that the sending contract on the source chain is the registered Base-side messenger — and that the boundary is therefore delegated to it. Neither half held. `channelPeerOf` is asserted non-zero and then passed on as an advisory label; it is never compared against the address that sent the message, so there is no peer validation to delegate to. The messenger allowlist above is the boundary that actually holds, and it is a per-chain check on the *messenger*, not on the business peer. Do not implement or audit a tier-propagation handler on the assumption that a peer check has already happened. Whether the adapter should perform one is #1650.
...
Pushing a tier update to N mirrors costs N times the per-chain CCIP fee. The protocol holds a dedicated `protocolBroadcastBudget` in native gas on Base. Only rollups that actually broadcast (i.e., pass the de-dup gate from section 2 — a new push tuple, NOT a first-zero-tier user) pre-quote the fan-out fee, debit the budget, and forward exactly the quoted value. Silent-skipped rollups consume no budget at all, so operators sizing + monitoring the budget should size against the volume of tier-changing mutations + governance-bump-driven follow-ups, not the volume of every Base action.
...
The freshness gate's version-mismatch rejection guarantees stale-version discounts are never applied — once a mirror raises its tracked version, ANY cache entry with the old stamp is treated as tier 0 until a fresh per-user push carrying the new version lands.
