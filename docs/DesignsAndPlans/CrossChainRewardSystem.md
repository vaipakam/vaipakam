...
- **Full unstake — immediate EFFECTIVE_TIER drop (Codex round-7 P1 #5)**: when a user fully unstakes (balance transitions positive→0), `currentStakeStartDayId` clears to 0 AND `EFFECTIVE_TIER` snaps to 0 in the same transaction, regardless of where the raw TWA still sits. The push fires with `effectiveTier = 0` and the projected `tierExpirySec = now` (i.e., already expired). Without this immediate downgrade, a user who fully unstakes would keep the pre-unstake EFFECTIVE_TIER until the TWA decays past the boundary, granting them mirror-side discounts during the entire 30-day decay window with zero stake. The "post-unstake decay" mechanism only applies to PARTIAL unstakes where some balance remains; a full exit is a hard reset.
...
**Mirror knows the latest tier-table version via lazy adoption (Codex round-9 P1 #7)**: a mirror does not call back to Base to query the current `tierTableVersion`. Instead, every inbound `TierUpdated` payload carries the current version; the mirror maintains `s.currentTierTableVersion` as the maximum version seen across all `TierUpdated` payloads for any user. The cache-validity check at fee-application time is: `userTierCache[user].tierTableVersion == s.currentTierTableVersion` — anything older treats as tier 0. The first sweep after a `tierTableVersion` bump raises `s.currentTierTableVersion` (because the first payload carries the new version); from that moment, every other user whose cache is still on the old version reads as tier 0 until their own sweep entry catches up. This means the FIRST user the sweep processes pays for the catchup latency of every other user, which is fine because the sweep is permissionless + paid from the protocol budget + can complete in O(activeStakers / batchSize) sweep calls.
