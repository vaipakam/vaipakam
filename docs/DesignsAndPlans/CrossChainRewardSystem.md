...
**The payload carries EFFECTIVE_TIER AND EFFECTIVE_BPS, NOT raw TWA-derived tier (Codex round-5 P1 #1 + round-11 P1 #6)**. The mirror cache has no visibility into `cfgTwaMinStakedDays`, `stakedDays`, OR the per-tier discount BPS table. Two distinct concerns:

- Without EFFECTIVE_TIER propagation, a fresh wallet could broadcast a raw tier 4 to mirrors and bypass the min-history gate.
- Without EFFECTIVE_BPS propagation, governance could change `VPFI_TIER3_DISCOUNT_BPS` from 20% to 25% on Base, and mirrors would still apply 20% indefinitely from their local constants — the cached tier index is correct but the BPS table is stale.

Base computes BOTH values: `effectiveTier = (stakedDays >= cfgTwaMinStakedDays) ? min(rawTier, ringBufferMinTier) : 0` AND `effectiveBps = LibVaipakam.discountBpsForTier(effectiveTier)`. The payload carries both. Mirrors apply the EFFECTIVE_BPS directly to fee math (`fee = grossFee × (10_000 - effectiveBps) / 10_000`); they use EFFECTIVE_TIER only for UI display. This means the mirror's local `VPFI_TIER{N}_DISCOUNT_BPS` constants are NOT read at fee-application time on mirrors; only the propagated value matters. Governance can update BPS thresholds on Base alone; the version bump + sweep propagate the new BPS values to every mirror as part of the per-user push.
...
- The minimum-history gate uses ELAPSED SECONDS, not inclusive day buckets (Codex round-11 P2 #4): `block.timestamp - currentStakeStartSec >= cfgTwaMinStakedDays × 1 days`. A user who stakes just before midnight should NOT satisfy "3 days" after only ~24 hours of elapsed time — `currentDayId - currentStakeStartDayId + 1` would return 3 (D, D+1, D+2 buckets) for that case, but elapsed-time arithmetic correctly returns ~24 hours. The `currentStakeStartSec` tracker (uint40 seconds-since-epoch) is set alongside `currentStakeStartDayId` on every 0→positive transition.
- `stakedDays = min(cfgTwaWindowDays, currentDayId - currentStakeStartDayId + 1)` is used ONLY for the ring-buffer denominator computation, NOT for the min-history gate.
...
**Sweep is one-shot per (user, version, destination) — Codex round-10 P1 #4 + round-11 P2 #1**: `sweepTierTableUpdate(startIdx, count)` walks the registry slice; for each user, it iterates EVERY currently-live destination and skips ONLY the (user, version, dest) tuples already pushed. New mirrors added between sweep calls are picked up automatically because their `tierTableSweepDone[user][newVersion][newDest]` is still false. The per-(user, version, dest) state lives in `s.tierTableSweepDone` (mapping (address → mapping (uint16 → mapping (uint64 → bool)))), declared explicitly in §5. Without this per-destination granularity, a sweep that touched a user on the old destination set would skip the user entirely when a new destination joined post-sweep, leaving the new mirror permanently stale on that user.
...
3. On Base, a **new dedicated `BuybackRemittanceReceiver` contract** (NOT `VaipakamRewardMessenger`) accepts the inbound token transfer. `VaipakamRewardMessenger.onCrossChainMessage` is data-only and reverts on `tokens.length != 0` (Codex round-5 P1 #5); reusing it for buyback remittance would revert every inbound. The receiver:
   ...
   - Calls `TreasuryFacet.absorbRemittance(deliveredToken, deliveredAmount, srcChainId)` which credits `s.baseBuybackBudget[deliveredToken] += deliveredAmount`. **`absorbRemittance` is restricted to `msg.sender == s.buybackRemittanceReceiver`** (Codex round-11 P1 #5); without this restriction, any caller could invoke `absorbRemittance(token, amount, srcChainId)` and inflate `baseBuybackBudget` to any value, draining VPFI from the reward pool by committing fraudulent buybacks. The diamond stores the receiver address on first install via a one-shot governance setter; subsequent changes go through the standard governance multisig.
...
// Seconds-since-epoch of the user's most recent 0→positive
// transition. Used by the elapsed-time min-history gate
// instead of inclusive day buckets (Codex round-11 P2 #4 — bucket
// arithmetic lets a user stake just before midnight and satisfy a
// 3-day gate after ~24 hours).
...
// One-shot per (user, version, destination) record for the
// sweep predicate. Bit is set when the sweep pushes a fresh
// TierUpdated to that destination for that user at that
// version. Cleared implicitly when the version changes (new
// version → all bits effectively false). (Codex round-10 P1 #4
// + round-11 P2 #1 — per-destination granularity ensures new
// mirrors added between sweep calls aren't permanently skipped.)
...
// RETIRED (#1770) — declared but permanently unread and unwritten.
...
// shipped (see the note under "Sender authentication" above). Kept
// only so removing it does not shift the fields below it.
...
| 2   | **Cross-chain — `VaipakamRewardMessenger` extension** | Add `TierUpdated` + `VersionBumped` message kinds; Base outbound on tier-change AND on governance version bump (eager broadcast); mirror inbound handlers write `userTierCache` + raise `s.currentTierTableVersion`; mirror-side fee-path reads `effectiveBps` from cache directly (NOT from local tier-table constants); **extend the messenger's payload-size gate to accept the existing 4-word REPORT/BROADCAST shape AND the new 8-word `TierUpdated` shape (`kind`, `user`, `effectiveTier`, `effectiveBps`, `computedAt`, `nonce`, `tierExpirySec`, `tierTableVersion`) AND the new 2-word `VersionBumped` shape (`kind`, `newVersion`)** (Codex round-4 P1 #5 + round-8 P1 #1 + round-11 P1 #3); **route business-peer authentication through the messenger's existing `channelPeer` mapping, NOT raw `Any2EVMMessage.sender`** (Codex round-4 P1 #4); full fork-test on Base Sepolia → Sepolia mirror.                                                                                                                              |
