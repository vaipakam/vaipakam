...
|------|----------|-------|-----------------|--------|
| **`SwapToRepayIntentFacet.commitSwapToRepayIntent`** | **`commitSwapToRepayIntent`** | the debt-sized **lot** is pulled to the diamond while the loan stays Active (#2322 — the rest of the collateral stays in the vault, liened, and the borrower position is locked); the lot returns on cancel, closes on fill | **decrement by the lot at commit; increment-restore by the lot on every teardown path (`cancelSwapToRepayIntent`/`cancelExpiredIntent`/force-cancels); on fill-settlement (`LibSwapToRepayIntentSettlement`) top the lien up to cover the borrower's collateral claim (`collateralAmount − consumed`), tombstoning only a zero claim** | **IMPLEMENTED ✅** (#569 Gap B, partial custody per #2322). Originally recorded as "full collateral pulled … release on fill … MISSING". |
