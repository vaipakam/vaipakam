...
| 8 | Operator | Confirm the keeper's cron trigger is present (it is removed only if step 7 took the write-free path), then flip `KEEPER_ENABLED=true` on `vaipakam-keeper` after the validation window. The flag alone arms nothing without a schedule to run on. |
