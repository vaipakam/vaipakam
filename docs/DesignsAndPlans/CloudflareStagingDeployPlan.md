...
|---|---|---|---|
| **vaipakam-www** (was `vaipakam-labs`) | `vaipakam.com` (apex, canonical) + `www.vaipakam.com` (301 to apex). **`labs.vaipakam.com` is gone** — it was this row's "today" host before the labs → www cutover; as of 2026-08-27 it has no Workers binding and no DNS record at all, so nothing answers there and no redirect fires | Marketing site, docs, "Launch Vaipakam" button → **`app.vaipakam.com/`**. #1959 ported the last two destinations and flipped `APP_TARGET` in `apps/www/src/lib/appUrl.ts`, so users following that button land on the current app. NOTE the flip lives in the SOURCE: apps/www is its own Worker, so a `vaipakam-www` build made before the flip still sends users to the legacy host — confirm the deployed marketing site carries it before reading a `vaipakam-app` deploy as reaching users. The `/recover` guide links stay on the legacy host on purpose (same-origin recovery state, not a missing route). Static, wallet-free. | No |
| **vaipakam-app** | `app.vaipakam.com` | The connected app — wallet connect, the intent-first Home at root (NOT a dashboard), Offer Book, loan flows, Claim Center, the VPFI fee-discount vault at `/vpfi` (deposits and withdrawals for fee tiers only — the app exposes no purchase surface of any kind), plus the wallet-free NFT verifier at `/nft`. NOTE (#1959): `/analytics` and `/protocol-console` ARE now served here — both were ported from the retired `apps/defi` Worker and the marketing site links them here. Test them against this Worker; the earlier instruction not to has been superseded. `/nft-verifier` was the legacy path for the verifier; here it is `/nft`. | No |
...
| 7 | Both | Smoke-test the app deployment end-to-end — its emitted `workers.dev` URL from step 6. `app.vaipakam.com` IS bound now, but the `workers.dev` URL remains the right target here: a deploy publishes there first and the custom domain can lag or sit on an older version, so testing the domain risks smoke-testing a different build than step 6 produced (#1854) — against `agent.vaipakam.com` + `indexer.vaipakam.com`, with `KEEPER_ENABLED=false` (no autonomous liquidation). NOT fully alert-only: `runDailyOracleSnapshot` signs on `KEEPER_PRIVATE_KEY` alone and will broadcast on staging regardless — if the window must be write-free, remove the keeper's trigger in the Cloudflare dashboard (*Settings → Trigger Events*) — editing `wrangler.jsonc` after step 5 has already deployed the active schedule changes nothing live. Allow for propagation before treating it as stopped; see `apps/keeper/README.md` (#1466). **HOLD (#1896): skip this step's keeper-trigger actions entirely.** The keeper already has no trigger — step 5 deploys `"crons": []` — so there is nothing to remove, and the restore instruction below must NOT be followed: it would re-arm the every-minute invocation that #1896 exists to stop, defeating this hold and step 8. The window is write-free for the keeper by default. Original instruction, for after the hold lifts: **if you take that path, restore the trigger at the end of this step** — step 8 only flips `KEEPER_ENABLED`, so a Worker left with no schedule would make step 9's observation window look quiet while every pass is in fact disabled. |
...
| 10 | Both | **SUPERSEDED — already done, and not the way this row describes.** It named `vaipakam-labs`, a service that no longer exists (renamed `vaipakam-www`), and would bind BOTH hosts as Worker Custom Domains. Only the apex is bound that way (verified 2026-08-27: `vaipakam.com` → `vaipakam-www`); `www.vaipakam.com` is a **Bulk Redirect** to the apex, so binding it to the Worker would REPLACE that redirect and give the site two indexable origins — the opposite of the apex-canonical setup. `vaipakam-hf-watcher` is decommissioned and `api.vaipakam.com` no longer resolves. Retained as the record of what step 10 was; do not execute it. |
