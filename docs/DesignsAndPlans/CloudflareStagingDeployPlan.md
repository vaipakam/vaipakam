...
|---|---|---|---|
| **vaipakam-www** (was `vaipakam-labs`) | `vaipakam.com` (apex, canonical) + `www.vaipakam.com` (301 to apex). **`labs.vaipakam.com` is gone** — it was this row's "today" host before the labs → www cutover; as of 2026-08-27 it has no Workers binding and no DNS record at all, so nothing answers there and no redirect fires | Marketing site, docs, "Launch Vaipakam" button → **`defi.vaipakam.com/` today, NOT `app.vaipakam.com/`**. `APP_TARGET` in `apps/www/src/lib/appUrl.ts` still reads `legacy`, held there by the VPFI deposit-anchor step, so users following that button land on the legacy app — which cannot be rebuilt to carry new contract addresses (its source was deleted in #1854). Do not read a `vaipakam-app` deploy as reaching users until that flip lands. Static, wallet-free. | No |
