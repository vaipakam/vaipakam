...
| 5 | Operator | `wrangler deploy` for `apps/{indexer,agent}`, and **`pnpm --filter @vaipakam/keeper run deploy`** for the keeper — NOT a bare `wrangler deploy` there (#1896): the package script carries `--keep-vars`, and without it wrangler deletes every var absent from `wrangler.jsonc`, which is exactly the `FRONTEND_ORIGIN` and optional `LIQ_*` / `SPLIT_*` / `PARTIAL_LIQ_*` tuning §4.4 just provisioned. This activates crons + binds `indexer.vaipakam.com`. **HOLD (#1896): the keeper no longer has a cron to activate** — `apps/keeper/wrangler.jsonc` commits `"crons": []` deliberately, because the Worker was being terminated for exceeding CPU on ~100% of invocations. Deploying it is still correct (it keeps the script and bindings current); it simply leaves the keeper unscheduled. Do not "fix" the empty list here. |
