...
| **vaipakam-app** | `app.vaipakam.com` | The connected app — wallet connect, Dashboard at root, Offer Book, loan flows, Buy-VPFI, Claim Center, plus the wallet-free NFT verifier at `/nft`. NOTE (#1854): `/analytics` and `/protocol-console` were NOT ported — they remain on the retired `apps/defi` Worker, and the marketing site links them there deliberately, so do not test or redirect them against this Worker (#1959). `/nft-verifier` was the legacy path for the verifier; here it is `/nft`. | No |
