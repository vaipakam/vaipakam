...
> bounds a canonical payout), **#1219**'s A/C owner fork (its legal
...
| **M6** — perks (#1204, `SpendGatedPerk` enum entry, legal glance first) + bonds (#1219 — glance DISCHARGED; open: FOUR owner decisions — A/C fork, capacity terms, unbond option, C's fee parameters). **HARD DEPENDENCY: #1566 ships BEFORE any #1219 deposit surface activates** — without the delivered-bound custody isolation, a deposited bond is shared-balance value an already-live reward claim can spend (refundable operator principal paid out as rewards); this is an implementation-and-activation edge on #1219 itself, NOT covered by the M7 governor-arming gate | #1204 / #1219 |
...
| #1204 / #1219 | Keep; note the RL-5 release-train commitment; #1219's glance is DISCHARGED — the open ask is the A/C owner fork (+ C's fee parameters) |
