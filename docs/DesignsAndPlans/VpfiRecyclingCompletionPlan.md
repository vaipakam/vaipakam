...
| **M5 — PARTLY SHIPPED; NOT complete.** Dashboard views (`selfFundingRatio`, `platformRetained`, runway, `netEmission = freshDrawdown`) + public surface. **`platformRetained` HAS NOT SHIPPED** — it appears in neither `apps/defi/src` nor `apps/indexer/src` (nor does `getRecycleBackingSnapshot`, its only source); only the contract getter exists. **#1525 therefore BLOCKS M5 rather than following it**, and an earlier revision of this row closed the milestone over exactly that gap after #1524 deferred it. What HAS shipped: #1487 + #1496 (contract slices — the `GovernorDayPoolStamped` EVENT widened by TWO fields; event arguments are not struct members and shift no storage slot), #1507 (indexer consumer), **#1508** (day-0 — this is where `recycledCreditedPreLaunch` was appended at the STORAGE struct TAIL, `45dd7880a`, so an in-place upgrade cannot shift a later slot), #1513 (schema + read surface; operator capture PASS split to **#1523**), #1524 (the public `/analytics` surface, `c2ff56a16`). #1218 is closed, which tracks the METRIC definition rather than the milestone. **What the surface is FOR is carrying the platform's refusals through rather than resolving them** — the read layer spends real effort declining figures it cannot stand behind, and all of it is undone by a page that renders a refusal as a zero or a dash. Four rules ratified into TokenomicsTechSpec, each earned by getting it wrong first: a figure the wire format publishes unconditionally is NOT thereby safe to DISPLAY unconditionally (mirror absorption is a running partial before finalization — a machine consumer reads `stamped` beside it, a reader looking at a table cannot); a caveat is shown only where its condition holds, judged AT THE PRECISION THE SURFACE DISPLAYS; a positive amount is never presented as zero, and a published share rounds in the direction that cannot overstate; and a surface that cannot trust its input refuses ITS OWN figures without substituting a zero for the value it rejected. Remaining M5 follow-ups: **#1525 (blocking)**, #1523 (versioned captures + promote; carries the unfixed post-apply-backup gap), #1515, #1510 | #1218 |
