...
4. **`feeEntitlementEnabled`** — only at the M2 joint-cutover gate,
   ...
   **NOT asserted by PR-9/#1356** — an earlier revision of this line said it
   ...
   target Diamond (GovernanceRunbook M7 Step 1). **And PR-6 is NOT live**:
   #1354/#1381 delivered the repayment and early-close families only — the
   five recovery entry points and refinance's stored-lender keying are open
   at #1947 (the prepay-SALE terminals were listed here and are REMOVED —
   the borrower's residual bears that fee, not the lender).
   > **ORDERING SUPERSEDED (r4, see the §1a M7 row):** with PR-6
   > (#1354/#1381) landed **for the repayment and early-close families only
   > — #1947 reopened the rest**, the protective intent of "`D*` armed" as an
