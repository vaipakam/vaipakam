...
| **M4 C1–C4 + Mode B** — surplus knob (#1567) + batched repatriation **Mode A** (#1568) + **Mode B stranded-delivery recovery (#1584)** + local keeper-budget credit (#1569) + TokenomicsTechSpec §9 (#1570). See §M4 — C2's scope CHANGED with §3.6a, and **#1584 is a completion dependency**: M4 is NOT done without it | #1222 tail |
...
- **C2 — batched repatriation → #1568. Mode A only.** §3.6a establishes that
  ...
  *With **Mode B — owned by #1584**, not this card:*

  > **Mode B is #1584.** It briefly had no owner at all: an earlier revision
  > said it "ships with #1434", which is false — #1434's M3 row covers the
  > delivered-funding and zeroed-day prerequisites and then retrying mirror
  > pricing, naming no reverse transport and no recovery state, while #1568 is
  > Mode A only. Both could have closed with none of §3.6a's reverse
  > transport, wire mode-discriminator, stranded-recovery reservation or
  > one-shot entitlement built. Cut as its own card rather than folded into
  > #1434, which is already large and whose semantics differ.
  >
  > **#1584 carries a cross-card accounting dependency with #1434** (Codex
  > #1578 r4 P2), and it is the fund-safety-shaped one: neither return mode
  > may restore the append-only 69M interaction-pool headroom, and Mode B must
  > **net `rewardBudgetArmedFreshReceived` where that particular late receipt
...
flowchart LR
  ...
  M1b -.-> M3
  M3 --> M4[M4 Phase C' C1..C4 + ModeB #1584]
...
| #1222 | Adopt the parked B1–B4 cut with §M3's two corrections (B1 two report fields; two-pass funding in B2/B3); #1331 stays absorbed by B2. **The Phase-C tail is C1–C4, not C1–C2** (#1567–#1570), **plus Mode B (#1584)** which no card owned until #1578 r3, and **C2's scope changed** with §3.6a — see §M4 |
...
3. **Cross-chain**: recycle-at-source + netted remittance live on every
   ...
   reachable; surplus tooling available (M4) — **including Mode B
   stranded-delivery recovery (#1584)**, without which a late compensation
   has no return path and M4 is not done; watcher **DEPLOYED** and
