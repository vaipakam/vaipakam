...
FINALIZE a not-yet-applied day, REMIT it to every destination, and only then call
the payable `broadcastGlobal` / `broadcastGlobalTo` to propagate `D*` — remit
