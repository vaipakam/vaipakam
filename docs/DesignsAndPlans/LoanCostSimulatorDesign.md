...
Three headline numbers at accept time (and on Loan Details), each
expandable to an itemized breakdown:
...
| **Close late (grace end)** | above + grace-window interest continuation + late fees (1% first day + 0.5%/day, capped 5%; NFT-rental variant capped to buffer) |
| **Liquidated** | above + 2% treasury handling + dynamic liquidator incentive (≤3%) + tier liquidation discount (per-tier bps) + fallback premium branch where applicable, expressed as estimated collateral-value loss |
...
each scenario class (on-time, late, liquidated, rental); COVERAGE.md row
added per the verification directive.
