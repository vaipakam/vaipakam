...
> ⚠️ **These surfaces still exist in the tree** until #687 lands — and across the **whole CCIP mesh**, not just the Diamond: the sale is `VPFIDiscountFacet` buy config **plus the mirror-chain path** (`VpfiBuyAdapter` / `VpfiBuyReceiver` / `ConfigureVPFIBuy.s.sol`); the yield is `StakingRewardsFacet`; plus "5% APR" copy. #687 must remove the issuer sale **everywhere**, or a mirror-chain buy path survives. Phase 1 is **blocked on the actual removal work** — do not treat the high-risk paths as already gone.
...
**Recommendation.** ADOPT a **retroactive, snapshot-based, no-cost** VPFI airdrop with **NO pre-announced eligibility criteria** — this is the archetype. **AVOID** any bounty/referral/promotional-service distribution where Vaipakam receives marketing value, or that is announced in advance (recreates the Tomahawk fact pattern and the foreseeability problem). **Gate the claim through the platform's compliance controls** — the airdrop distribution (direct transfer or Merkle claim) must run through the same **sanctions screening** the rest of the protocol uses (don't ship an ungated claim that pays flagged addresses); per CLAUDE.md the sanctions oracle must actually be wired post-deploy (see §9.2) for that screening to be live.
...
**Lower-confidence / counsel-gated (carried from §9.2):** this is **not** a settled "service rewards are safe" position — the verifier refuted the categorical claim, users still supply principal/collateral + pay/earn fees and can foresee the reward once live, and the mechanics are an open counsel question (§6). Treat the live program as a counsel-reviewed design, not a green light; and **#687/Card-C reward-hardening is a hard prerequisite** — the *current* code doesn't yet enforce the completed-transaction tie (entries register at `initiateLoan`, accrue by a duration-derived `endDay`).
...
- **Card B — Retroactive airdrop + progressive-decentralization roadmap.** Snapshot design, governance-utility wiring (vote on collateral/risk/fee switch), parameter-immutability schedule. **Eligibility criteria + snapshot period stay PRIVATE until after the snapshot is taken** — the public card (and any pre-snapshot comms) must NOT reveal foreseeable criteria or farming targets, or the no-consideration airdrop carve-out evaporates (§4.1/§9.2).
...
Also pending from #687: reconcile the freed **1%** (sale) + **24%** (staking) supply pools — reduce the 230M cap or reallocate to earn pools. **Include the Exchange / Market-Making allocation too** (TokenomicsTechSpec reserves ~12%; the public whitepaper shows ~14%): an operator-held market-making pool **contradicts** the Phase-1 "no team-seeded liquidity/market-making" posture (§9.2), so it must be **retired or explicitly deferred to a DAO-governed Phase 2** — not silently kept while sale/staking are removed.

---

## 9. Phase 1 (Admin-Controlled) — Minimal-Footprint Design (DECIDED)

The platform launches under **admin control** (parameters, treasury, and operations all held by the operator; governance and decentralization come later). This section is the **decided** tokenomics for that phase, optimized to **minimize mandatory legal/compliance overhead** — i.e. structured to *reduce* the obligations that would require a registration, license, or ongoing counsel engagement. This is the design **goal, not a self-certified clearance**: residual US/EU/UK exposure depends on the §9.3/§9.4 analyses (and optional geofencing), which remain mandatory while distribution is open to those users.

### 9.1 Governing principle
Admin control is the **"efforts of others"** prong of Howey at its strongest. Therefore, during this phase the design **minimizes the profit-expectation surface** the token presents — it cannot be reduced to literal zero while a transferable ERC20 with a published Phase-2 roadmap exists (§9.3), so the goal is to carry **as little profit expectation as possible**, with the target identity a **consumptive "digital tool,"** not an investment. Every value-accrual mechanism (buyback-and-burn, fee-switch/real-yield, vote-escrow, governance) is **deferred to Phase 2**, gated on actual decentralization. Adding any of them while the admin holds the keys *increases* exposure; doing less with the token is the cheaper and safer path.
...
| Governance | **None live** in Phase 1; admin-controlled. Not marketed as a future value driver. *(Caveat: this doc's own public Phase-2 buyback/fee-share/governance roadmap (§8/§9.5) is issuer-attributable forward-value content — keep Phase-2 value-accrual details counsel-gated / out of marketing so the roadmap itself isn't read as future-value-driver messaging.)* |
| Distribution | **No issuer sale:** (a) usage-based interaction rewards — a **variable usage rebate** tied strictly to *completed* lend/borrow transactions, never labelled "APR/yield". **Hard prerequisite — the completed-transaction tie is NOT in the current code:** `LoanFacet.initiateLoan` registers reward entries immediately and `LibInteractionRewards` accrues by a duration-derived `endDay` that can be claimed before a clean close (even on a still-open/overdue loan). So **Card C / #687 reward-hardening MUST land first, OR interaction rewards stay disabled until that gate exists** — do not launch the rewards on the current code. **Lower-confidence design, not settled:** users supply principal/collateral and pay/earn fees and can foresee the reward once live, so it is **outside** the zero-consideration airdrop posture; its defensibility rests on counsel-reviewed mechanics (§6) — *argued* as a service/usage reward, **not** "no investment of money"; (b) optional **no-consideration** retroactive airdrop with no pre-announced criteria (the genuinely zero-cash-in path — itself gated per §2/§4.1). |
...
| Compliance scaffolding | Keep existing **free** measures: on-chain ToS (LegalFacet), sanctions screening, retail KYC-off. **Sanctions screening is a LAUNCH PREREQUISITE, not automatic:** per CLAUDE.md the sanctions oracle **fails open until `ProfileFacet.setSanctionsOracle(...)` is wired post-deploy** — that wiring must be on the go-live checklist (and the airdrop claim must route through it, §4.1). "Nothing added" applies to the **token-distribution** surface only — **not** a protocol-wide conclusion: if the independent MSB/VASP/BSA analysis (§9.3) finds covered lending activity, AML-program / KYC obligations there are a separate matter. |
| Buyback subsystem (#687-C) | **Confirm DORMANT — a Phase-1 blocker.** The Treasury buyback-and-redistribute subsystem stays operator-unactivated (no allowed-token / budget-credit / intent-commit on EITHER chain role — see #687/#707 excision plan). Listed here so the Phase-1 go-live checklist includes the dormancy assertion alongside the sale/yield removals. |
