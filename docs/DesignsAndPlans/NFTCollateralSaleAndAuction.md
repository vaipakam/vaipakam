...
- The `apps/agent` `/opensea/collection/{slug}` proxy (§14.6 + Block A.7) **MUST execute a sim-transfer pre-flight check whenever the fee schedule returns any non-zero fee leg** (elevated from "SHOULD optionally" per Grok finding 3 — this is the primary mitigation for the documented poisoning vector and shouldn't be opt-in). The proxy uses `eth_call` to simulate a 1-wei transfer to each fee recipient via the principalAsset. Recipients that revert get flagged in the response body so the dapp can surface a hard warning. The borrower can still choose to publish anyway (their right; see §14.4's "no on-chain allowlist" reasoning), but the dapp must show the warning + require an explicit confirmation click before passing the flagged recipients into the canonical order. For fee-free collections (`feeLegs.length == 0` after the API filter), no pre-flight runs — the proxy short-circuits.
