...
- **Required per-token agent config (Round 5.1 errata).** The agent's recipient-validating-token allow-list (described next) MUST carry, for each entry, **the resolved balance-slot identifier** (a single uint256 for straight-mapping layouts; the ERC-7201 namespace hash for namespaced layouts; a small `(slot, packOffset)` tuple for packed-struct layouts). Operators populate the slot via one of three methods at config-time (NOT at request-time, since slot computation requires either source or storage probing):
  1. Read the token's verified Etherscan source + extract the mapping declaration position;
  2. Use `cast storage <token> <holder_addr> --rpc-url <rpc>` to probe the slot interactively against a known-balance holder address (binary search across slots; the matching slot returns the holder's known balance);
...
- **For the testnet / Anvil rehearsal deploy** (where there is no real multisig and the admin is a dev EOA): the script deploys a small `BatchCaller` contract (a 5-line `forge.s.sol` helper that just forwards two arbitrary calls in one tx) and calls IT with the bundled payload. This preserves the same-tx-batch invariant on local + testnet rehearsals so the deploy-script pattern matches mainnet behaviour. Skipping this and "just running both in a broadcast block" produces a working testnet rehearsal that gives FALSE confidence the mainnet deploy will be atomic.
- **Reference implementation:** see `contracts/script/multicallDeploy.s.sol` + `contracts/script/utils/BatchCaller.sol` (TO BE ADDED in Block A — first time we need this pattern; will set the precedent for B.4's second iteration and for any future recorder extension).
