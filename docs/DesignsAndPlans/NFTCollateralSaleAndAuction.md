...
The vault's `setCollateralOperatorApproval` and the diamond's listing facet validate against `executor.approvedConduits(conduit)` via a view call before granting operator approval to the conduit. The executor's `validateOrder` re-checks the same predicate at fill time so that a conduit removed AFTER a borrower's sign-time write cannot still route a fill — defense-in-depth against a future-compromised conduit.
...
- **Attack surface is narrower than the original prose suggested (Round 5.1 errata — Codex P2 line 446).** A plain `IERC20.transfer` does NOT invoke the recipient contract's `receive()` or any other recipient-side hook — only native-ETH transfers do, and only for `receive()`/`fallback()`. The only way an ERC20 transfer can revert based on the recipient is if the token itself implements recipient-validation logic: **blocklist-style tokens** (USDC's OFAC blocklist; some stablecoins' compliance hooks; tokens with frozen-account mechanisms). For the typical unhooked ERC20 (DAI, WETH, vanilla wrappers, most lending-protocol tokens), a transfer to ANY non-zero address succeeds and the "ghost listing" vector via transfer-revert structurally cannot apply. The dapp / agent SHOULD surface this distinction so borrowers and operators don't infer false safety from a pre-flight passing on a token where the check is structurally a no-op.
- **Sim-transfer pre-flight MUST use state-overrides on the sender's balance** to be feasible at all (Round 5.1 errata). A naive `eth_call` from the agent (a zero-balance address) hits the principalAsset's sender-balance check first and reverts there — producing a false positive on every recipient regardless of whether the recipient itself would have rejected. Corrected pattern: the agent issues `eth_call` with a `stateDiff` / `stateOverride` payload that pre-funds the sender with > `sum(feeLegs.amount)` of the principalAsset (by writing into the ERC20's balance-slot for that sender; the slot is computed via the standard `keccak256(abi.encode(holder, slot_index))` recipe for mapping-typed balance storage). Then a 1-wei transfer to each recipient actually exercises the recipient-side path. Implementation reference: viem's `simulateContract` + `stateOverride` parameter, or the matching ethers `provider.call({ stateOverrides })` pattern.
- The `apps/agent` `/opensea/collection/{slug}` proxy (§14.6 + Block A.7) **MUST run this state-override sim-transfer pre-flight for every non-zero fee leg WHEN the principalAsset is on the agent's recipient-validating-token allow-list** (the agent maintains a small per-chain allow-list of tokens whose `transfer` can revert based on recipient — USDC, USDT, BUSD, etc. — populated from each chain's deployment registry at deploy time; this list is editable via the agent's wrangler config so operators can adjust as new blocklist tokens land). For tokens NOT on the list, the pre-flight is structurally a no-op and the proxy SHOULD return a neutral "not applicable for this principalAsset" signal so the dapp's UI doesn't render a false-confident "✓ safe" badge. The borrower may still publish either way; this is informational defense-in-depth, not a gate.
...
The "live read at fill time + observable re-sign banner on `TreasuryFeeBpsSet` events" model keeps the executor's coverage check as the single source of truth and acknowledges drift via UX, not via stale-param settlement.
...
A "tight back-to-back tx" pattern is NOT sufficient — even one block between the two tx's leaves a window where every listing mutation reverts. The deploy script MUST execute both state mutations **in the same on-chain transaction**:

- **For the post-handover (governance-timelock) deploy:** the operator submits ONE governance proposal that bundles both calls via a multicall payload (the timelock's `executeBatch` if using OZ TimelockController; a multisig `multiSend` if using Gnosis Safe; etc.). The proposal queues + executes once both targets pass the same governance window.
- **For the pre-handover (admin-multisig) deploy:** the Forge script orchestrates a single `vm.startBroadcast` that emits a multicall transaction to the multisig executor, OR — more typically for testnet rehearsals — the deploy script's admin EOA simply executes both calls in sequence within one Forge `broadcast` block + the Anvil/test-chain RPC guarantees same-tx atomicity per the script's batched send.
- **Reference implementation:** see `contracts/script/multicallDeploy.s.sol` (TO BE ADDED in Block A — first time we need this pattern; will set the precedent for B.4's second iteration and for any future recorder extension).
