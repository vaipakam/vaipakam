...
- The `apps/agent` `/opensea/collection/{slug}` proxy (§14.6 + Block A.7) **MUST run this state-override sim-transfer pre-flight for every non-zero fee leg WHEN the principalAsset is on the agent's recipient-validating-token allow-list AND the allow-list entry has a populated `balanceSlot`** (the agent maintains a small per-chain allow-list of tokens whose `transfer` can revert based on recipient — USDC, USDT, BUSD, etc. — populated from each chain's deployment registry at deploy time; this list is editable via the agent's wrangler config so operators can adjust as new blocklist tokens land). For tokens NOT on the list — OR on the list but missing a `balanceSlot` (e.g. a newly-added entry not yet populated) — the pre-flight is structurally a no-op and the proxy MUST return a neutral "not applicable for this principalAsset" signal (NOT a false-confident "✓ safe" badge). The borrower may still publish either way; this is informational defense-in-depth, not a gate. Implementation reference: viem's `simulateContract` + `stateOverride` parameter, or the matching ethers `provider.call({ stateOverrides })` pattern.
...
- **For the testnet / Anvil rehearsal deploy** (Round 5.1 errata — Codex P2 line 773 — corrected): the simplest naive helper (a `BatchCaller` contract that just forwards two arbitrary calls in one tx) does NOT work in isolation, because `DiamondCutFacet.diamondCut` and the executor's `_authorizeUpgrade(...) onlyOwner` would treat the helper as `msg.sender` — which is NOT the diamond owner or the executor owner, so both targets would revert and the rehearsal "succeeds" by failing-loud (defeating the purpose). The rehearsal MUST mirror the mainnet ownership structure: deploy a minimal multisig (Gnosis Safe in 1-of-1 mode with the dev EOA as the single signer is the cheapest valid option; the same Safe address is set as the Diamond owner via `IDiamondLoupe.owner` / `IERC173.transferOwnership` AND as the executor's `OwnableUpgradeable` owner at proxy-init time). The deploy script then encodes the diamondCut + UUPS upgrade calldata as a Gnosis `multiSend` payload and submits it via the Safe's `execTransaction` — ONE on-chain tx, owner is the Safe, both targets pass their ownership checks. This is the same call shape mainnet will use (only the signer set differs); the testnet rehearsal proves the batched atomicity property AND the ownership routing in one shot.
- **Reference implementation:** see `contracts/script/multicallDeploy.s.sol` (TO BE ADDED in Block A — first time we need this pattern; will set the precedent for B.4's second iteration and for any future recorder extension). The script depends on a Gnosis Safe deploy script (testnet / mainnet) that already exists in the deploy plumbing for other admin surfaces.
