...
| **Admin / utility / chrome pages (`AppLayout`, `AdminDashboard`, `VaultAssets`, `VaultRecover`, `NftVerifier`, `DataRights`, `PublicDashboard`)** | Routing shell + operator + utility shape | A.6, A.8 | — | Out of scope for this UX direction pass. `AppLayout` is the shared routing shell (nav / chrome); chrome reworks are a separate concern from the per-page rework this ADR scopes. Admin + utility surfaces don't need the retail-DEX visual idioms. Standardise BPS + gas idioms only across the row. | Out of scope |
