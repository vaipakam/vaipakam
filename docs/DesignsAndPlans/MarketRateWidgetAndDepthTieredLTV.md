...
- Step 6 — per-chain slippage census + audit + risk sign-off: **PARTIAL.** The census tool (`contracts/script/SlippageCensus.s.sol` + `docs/ops/SlippageCensusGuide.md`) landed in `9a8ce69` — operators can now produce the per-chain CSV the audit consumes. The audit + risk-committee sign-off remains human / governance work and is still OPEN.
