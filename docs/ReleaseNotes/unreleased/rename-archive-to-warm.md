...
**The three application services must move first, and be seen to have moved.** They read and write the shared database, and their running configuration is whatever was last deployed — not whatever the repository says. Until each is redeployed, they keep using the old database while a replacement backup service happily backs up the new, empty one and reports success. Retiring the old backup at that point leaves the data that is actually being written unprotected, and retiring the old database then destroys it. A green backup run proves the backup works; it says nothing about whether it is pointed at the data anyone is using.

The check that settles it is the data itself rather than any configuration readout: the new database should be visibly filling — the indexer rebuilding chain history into it — while the old one stops changing. Configuration endpoints can report what is *configured* rather than what is *running*, so they are not sufficient evidence here.
