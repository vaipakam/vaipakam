...
classified entries in the order they were classified, measured against
two ordering counters that only ever grow (the headroom figures the
platform already keeps can be corrected downward, which is why the
ordering could not read them), and a correction of an earlier entry
shifts every later entry's place in that order. Unspent value moves with
