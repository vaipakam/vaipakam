...
The Korean Basic guide carried a section no other edition had: it described a
withdraw control on the Rewards page. There is no such control there — the
same action lives on the VPFI page, and the Korean guide already describes it
correctly in that chapter. The stray section has been removed, so Korean
readers are no longer told to look somewhere the button has never been.
...
separately; it is roughly nineteen thousand characters of technical prose,
which is a translation project rather than a repair to the check.
