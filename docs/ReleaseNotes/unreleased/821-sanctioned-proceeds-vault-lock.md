...
close-outs (full repayment, time-based default, HF-based liquidation) could
**revert** — because depositing that party's share routes through the receiving
vault, which is screened. A flagged lender could brick repayment; the unflagged
counterparty couldn't be made whole until the flag lifted.
...
- **Frozen — nothing leaves.** While the wallet is flagged, the assets in its
  ...
  position NFT to a clean wallet and claiming from there (a loophole that's now
  closed). A genuine protocol sale of a position re-points it to the buyer, so a
  legitimate buyer's funds settle to their own vault and are unaffected.
...
- The **NFT Verifier** now warns when a position's current owner is flagged, so a
  prospective buyer knows the position's payout is frozen and currently
  unclaimable.
