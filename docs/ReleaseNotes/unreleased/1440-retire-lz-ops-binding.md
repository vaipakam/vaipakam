...
- The restore rebuilt every credential but nothing restored the
  ...
  duties run at all. They are not in the archive and are not committed, so a restore that follows only the credential steps finishes with the signing key present and every autonomous path dark, indefinitely and silently. How they are held was checked against the live deployment rather than assumed: the keeper's main switch is a per-Worker **secret**, not the plain configuration value an earlier draft of this note described, and the runbook restores all three the way secrets are restored. That also retires this note's other claim — that an ordinary redeploy wipes them. It does not; secrets are not rebuilt from committed configuration. The consequence that does matter is the opposite one: a secret's value cannot be read back afterwards, so the offline capture is the only record that will ever exist of what these were set to.
