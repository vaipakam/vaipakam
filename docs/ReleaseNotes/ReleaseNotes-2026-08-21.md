...
Today's release is mostly about lenders being told things they previously had to
work out for themselves. The position page gains a card that names every way out
of a lending position and what each one costs — including waiting, which is
listed first because for a lender it is usually the right answer. The rest of the
day's work is the machinery that keeps such a card honest: the card now publishes
whether it has finished deciding, and the post-deploy review reads that answer
instead of inferring one from silence.

A theme runs through the three review-tooling entries below, and it is worth
stating plainly because it cost real time this week. Each describes a check that
was passing without ever having run the thing it claimed to check — a comparison
the live chain never reaches, an assertion against a preview build rather than
the deployed one, a wait that concluded "nothing to see" from a timeout. A green
check that has quietly stopped checking is worse than a missing one, because it
buys confidence it has not earned. The fixes all take the same shape: make the
thing report its own answer, and read that, rather than inferring it from what
did not happen.

The last entry is a correction rather than a feature. Public pages were still
promising borrowers a fee rebate that the current fee model cannot deliver.
