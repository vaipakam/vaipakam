...
function getProtocolTrackedVaultBalance(address user, address token) external view returns (uint256);
...
- **Per-Chain Flip Discipline:** `setDepthTieredLtvEnabled(true)` and `setDiscountPathEnabled(true)` remain independent manual Timelock / Safe-governed rollouts per chain after audit sign-off. Operators may stage them separately. Emergency controls remain available through per-asset pause, master switch disable, discount-path disable, keeper-side environment flag removal, and anomaly auto-pause.
