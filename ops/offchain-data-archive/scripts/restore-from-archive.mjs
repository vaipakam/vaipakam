...
export function convertD1(archive, outDir, { lzDatabase = 'vaipakam-lz-alerts-db' } = {}) {
  ...
  // Clear-only batches for era-gated tables the archive omits. Without
  // these the converter emits nothing for such a table and a selective
  // restore silently keeps whatever the live database already holds —
  // possibly the fabricated rows the restore is meant to remove.
  for (const name of eraLossClears) {
    const dir = path.resolve(outDir, 'd1');
    mkdirSync(dir, { recursive: true, mode: 0o700 });
    secureStagingDir(path.resolve(outDir));
    const file = path.join(dir, `${name}.sql`);
    const sql =
      `-- ${name}.sql — generated by restore-from-archive.mjs (#1513).\n` +
      `-- The archive carries NO rows for this table. That is expected for\n` +
      `-- an archive predating it, and suspicious otherwise — either way the\n` +
      `-- live table must not keep rows this archive cannot vouch for.\n` +
      `DELETE FROM "${name}";\n`;
    rmSync(file, { force: true });
    writeFileSync(file, sql, { mode: 0o600, flag: 'wx' });
    entries.push({
